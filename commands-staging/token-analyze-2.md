...
8. 判断主序按 split-run §3.2：casebook 过闸 → 聚类合并裁决 → 临时实体 → 无下限成员完整性扫描 → 反证检查 → `freeze` 落盘 → 正式 entity_identity_gate → 判级 → needs_adjudication 裁决 → 阵营演变重放 → 状态评估 → A4（外部异构路按 §3.3 收紧）→ A5 即收工（A6 复盘仅我明确要求时跑）。
