...
description: >-
  受支持链的机构级代币筹码/庄家分析与既有报告独立复核：按声明范围采集重放、
  识别及标注庄级实体、重建阵营持仓演变与流转、核算 V3/V4 流动性及可证 LP 权益，
  交付经对抗复核的自包含 HTML。
  正式候选链为 Ethereum/BSC/Base/Solana；各链六项可执行能力须解析到真实实现或证据：
  链身份会话、冻结目标、会计/供应量、R9 正式纵切片、错链负测、失败产物门禁。
  四链已有真实 R9 纵切片证据；任一 target 缺失或脱离全量 suite 即退出正式发布。
  Robinhood EVM 仅保留 exploration 支持；Arbitrum 仅保留探索支持；两链均不得进入正式交接或审计发布路径。
  全新链须先补齐采集、对账、身份门禁适配器与目标链标签表才可正式发布。
  当用户问代币筹码分析/结构、既有报告复核、庄家数量/类型/行为、关联地址/老鼠仓/控盘、
  庄家是否离场、大户动向或 LP 收益，或提到 holder analysis、鲸鱼追踪、代币尽调时使用。
  gmgn-token 用于快速单项查询；本 skill 是数小时深度分析工程。
  只查价格/K线/热榜/新币列表不要用本 skill。
---

<!-- skill-version-source: VERSION; skill-version: 7.0.3 -->

# 代币筹码分析（Token Chip Analysis）

固定交付“三问一异常”：①有几个庄；②每个庄什么类型；③各阵营在已声明且过闸的历史范围内持仓占比如何演变；④本次特有发现（确无发现写“无”）。占比以总供应为分母，锁仓/销毁单列（Solana sol-rows 图一按净供应，真烧毁轨不堆叠）。交付自包含 HTML；监控包只在用户确认买入后补。核心信条：**不对账的分析是猜测，未经反驳的结论是自嗨。**

## Codex 运行适配

- **同步**：开工跑 `sync-from-cc.sh`；仅 rc=0 放行，否则按 `SYNC.md` 处理。
- **映射/复核**：CC 路径映射到 Codex，工具取等价物；禁照抄模型别名、运行 CC workflow 或以同体系 Codex 冒充外审（仅算内审）。
- **资产与边界**：保留三项 Codex 独有资产；main 是共有方法权威。−1 按 `split-run.md` 做机械层，禁裁决/冻结/判级/A4/A5，交接即停。
- **密钥/提交**：key 按登记读取且不外泄；自有变更仅写 `CHANGELOG-codex.md`，全测通过即提交。
