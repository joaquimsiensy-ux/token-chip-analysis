...
**刀 1——机械活外包子代理（卸上下文；模型不降级，3.39.1 起）**：
1. 机械阶段一律派子代理执行，**模型一律 `model: 'opus'` + `effort: 'high'`**（用户 2026-07-28 定，推翻 v3.1 的 sonnet 降级档）。**外包的收益从此只有一项：子代理不背主线几十万上下文**——单价不再省，别再拿"换便宜模型"当外包理由，也别因为省钱把哪一路改回 sonnet。写法：Workflow 的 agent() 用 `opts.model='opus', opts.effort='high'`；**Agent 工具没有 effort 参数**（schema 只有 model），直调时只能写 `model: 'opus'`、effort 继承会话档位——需要钉死 high 的路优先走 Workflow。`fast` 无 per-agent 开关（`/fast` 是会话级），不必尝试传。**外包清单**：标准脚本跑批与重试循环（采集/余额扫描/CSV 加工）、对账三查的执行侧、标签库批量 lookup、图表脚本执行、数据完整性验证、背景调研 fan-out、逐地址溯源 fan-out、**P0 实体素材装配**（每 P0 一路并行，模板见 research-workflows.md §二b；完整版与 easy 版同办）。**禁止外包**：聚类判定、实体定性、对抗复核裁决、报告撰写——一切需要"判断"的环节留在主线亲做（质量底线，铁律 6）。
