...
1. 机械阶段一律派子代理执行，**模型两档制（用户 2026-07-28 定）**：**机械型 → `model: 'opus'` + `effort: 'high'`**（sonnet 降级档已作废，单价不再省——别拿"换便宜模型"当外包理由，也别因省钱把哪一路改回 sonnet）；**判断型 → 保持主模型：model/effort 都不传，继承主会话**（判断质量不降级）。**外包的收益只有一项：子代理不背主线几十万上下文**。写法：机械型在 Workflow 的 agent() 用 `opts.model='opus', opts.effort='high'`，判断型两参数全不传；**Agent 工具没有 effort 参数**（schema 只有 model），直调机械型只能写 `model: 'opus'`、effort 继承会话档位——需要钉死 high 的路优先走 Workflow。`fast` 无 per-agent 开关（`/fast` 是会话级），不必尝试传。**外包清单·机械档（opus+high）**：标准脚本跑批与重试循环（采集/余额扫描/CSV 加工）、对账三查的执行侧、标签库批量 lookup、图表脚本执行、数据完整性验证、逐地址溯源 fan-out（按 schema 批量抓地址页）、**P0 实体素材装配**（每 P0 一路并行，模板见 research-workflows.md §二b；完整版与 easy 版同办）。**外包清单·判断档（继承主模型）**：背景调研 fan-out（综合评估）、对抗复核的怀疑者与完整性批评（复核另有 1 路**外部异构怀疑者走 codex/GPT-5.6-sol**——非 Claude 子代理、不占本档，见 research-workflows §2）。**禁止外包**：聚类判定、实体定性、对抗复核裁决（汇总仲裁）、报告撰写——一切需要"判断"的环节留在主线亲做（质量底线，铁律 6）。
