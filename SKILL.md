...
description: 对任意链上代币（EVM/Solana/Hyperliquid/Filecoin 及新链）做机构级庄家链上行为分析——全量数据采集重放、庄级实体识别与标签划分（项目方/大庄/小庄/离场庄/刷量地址）、各阵营持仓演变、重点实体流转路径图、对抗复核、自包含 HTML 报告（用户确认买入后可补生成观察哨与监控 JSON 附录）。当用户问"某代币的筹码分析/筹码结构/庄家行为分析"、"有几个庄/庄家什么类型"、"庄家/项目方/做市商在吸筹还是砸盘"、"有没有关联地址/老鼠仓/单一实体控盘"、"庄家是不是跑了/弃盘了"、"该不该买/该不该卖/解锁抛压大不大"、"看看某代币的链上持仓/大户动向"，或提到 holder analysis、鲸鱼追踪、代币尽调时使用。与 gmgn-token 的区别：gmgn-token 是快速单项查询；本 skill 是数小时深度分析工程。只查价格/K线/热榜/新币列表不要用本 skill。
---

# 代币筹码分析（Token Chip Analysis）

对一个代币的项目方/庄家/做市商回答三个固定命题（v5.0 三问框架，2026-07-30 用户修订；前身 v2.0 四问框架）：**①有几个庄？（按标签体系计数：项目方 / 大庄 / 小庄 / 离场庄 / 刷量地址，见 playbook §6a；不分 P0/P1，狙击集团标签已废止）②每个庄什么类型（单地址明牌/多地址互转·gas同源/伪装分散·指纹一致）？③各阵营全历史持仓占比如何演变（占总供应量，锁仓/销毁单列；建仓后动没动、拉升期有没有出货）？** 原问 4 项目方背景调查已删除（创始人/社媒/水军整路不做；解锁日程情报保留服务问 3）。交付一份每条结论可独立验证的**自包含 HTML 报告**（图 1/图 2 前置于 TL;DR 顶部 + 每个当前持仓 ≥20% 总供应或 ≥20% 流通的大庄/项目方一张全周期流转路径图）。建仓成本不再是固定命题（§6b 降为按需工具）。**监控包默认不随报告生成，用户确认买入后按需补**（v3.2；细则见阶段 5 与 monitoring-package.md）。

**三问是下限不是上限（开放条款）**：链上任何不属于三问的显著结构性异常——暴跌/暴涨归因、假量对倒矩阵、流动性池异动、治理/权限异动、跨链桥异常等——必须单列章节报告，并在 TL;DR 增设"本次特有发现"条目（确无发现时明写"无"，这也是结论）。禁止因"框架未覆盖"而略去；报告骨架是最小集，允许按标的插入特有章节（见 report-template.md）。
...
5. **数据源取用**：API key 从 `~/.claude/api-keys.md` 登记文件直接取用（全局自动加载；已登记通道——含付费额度内的 HyperSync Starter——按 pipeline 决策树直接用）；**新增**数据源免费优先（缺了才向用户索取或走 auto-register-api）；运行时只写工作目录 config.json，永不写死进 skill 目录。
...
阶段1 并行采集：全量链上数据(后台) + 标签 + 价格（vesting 标的加解锁情报轻量路）
阶段2 对账关卡（硬性，不过不进分析）
阶段3 分析：地址标注 / 金库归因 / 庄级实体识别与标签划分 / 大户排查双闸 / 演变重放
阶段4 对抗复核（必做）
...
另核定两件事（v1.3）：①**标的是否带解锁表/vesting**（tokenomist/dropstab 有记录，或链上有锁仓合约/多签托管）——有则问 3 必须包含"未来 6–12 个月解锁日程与量级"小节（要求见 report-template.md）；②**开工版本自查**：读 CHANGELOG 首个版本号并在计划里注明，交付前重读一次——版本号变了说明 skill 被并行会话更新过，向用户提示框架可能已迭代（多会话并行的版本竞态防护）。
...
三路并行：①**全量链上数据**（最耗时，最先启动，后台跑；采集脚本标配=限速可调/退避重试/断点续传/失败段补扫/冒烟小样本先行）②地址标签与安全面（GMGN、浏览器标签页）③价格（CoinGecko / binance.vision）。**vesting/解锁表标的加一路解锁情报轻量 agent**（tokenomist/dropstab 多源交叉，见 research-workflows §1 路线 1——下一次大解锁的时间和量是问 3 解锁小节的核心输入）。（v5.0：问 4 删除，背景调研 workflow 整路退役；research-workflows §1 其余路线按需作分析工具，不再默认启动）
...
方法学全部在 `references/analysis-playbook.md`，按序做：地址身份标注（官方标签→外部证据→行为特征三级兜底）→ 金库与核心实体逐笔归因 → 关联聚类（多证据边+服务枢纽剔除）→ **实体身份硬闸（v4.2 强制，IQ/LPT/PYTHIA 托管误判三案根治）**：实体表冻结前跑 `scripts/report/entity_identity_gate.py --state … --chain … --snapshot …` 产出 `identity_gate.json`，对每个实体地址+≥1% 大仓做标签双源（CSV 主库+address-book 手工层，label_lookup 已自动并源）、Solana ed25519 曲线判定、托管假设三查；INFRA_IN_ENTITY（设施/CEX 混入实体）/PDA_UNRESOLVED（off-curve 无解释）/BIG_UNLABELED（大仓未过托管假设）三类 flag 逐条填 resolution（查了什么、结论是什么）——**build_html G8 会校验此闸，flag 未解决报告编不出来**；币安 Alpha 在架的 Solana 标的同时做 Alpha 集齐率判别（easy-workflow E0b 步骤④同款，完整版同责——PENGUIN 教训曾只入 easy 未入本版，PYTHIA 案因此复发） → **庄级实体识别、标签划分与类型三分类**（§6a，v5.0 不分 P0/P1：项目方无论份额；大庄=当前 ≥20% 总供应或 ≥20% 流通；小庄=当前 ≥5% 总供应或 ≥10% 流通；离场庄=峰值 ≥10% 总供应或 ≥15% 流通且当前非庄；刷量地址单独标签；狙击集团标签已废止，发射窗协同实体按普通门槛判级；合并口径含全部疑似关联地址）→ **其他大户排查前置双闸**（§6a：其他大户线降为当前 ≥0.1% 总供应或 ≥0.2% 流通，逐个过批量排查层才准归阵营；每个已识别实体做不设持仓下限的成员完整性扫描）→ 全量转账重放出各阵营占比演变序列（阵营划分标准见 §6a）→ 庄家当前状态评估（§7）→ 质押/留存修正；建仓成本仅按需算（§6b 降为工具）；CEX 净流×价格作为演变解读工具按需用（防内部调仓伪影）。（v5.0：项目方背景调查已随问 4 删除。）数据先验结构再分析（榜单唯一性断言、多档抽查），批量脚本先 2 个样本验证编解码再放量、绝不吞异常。

## 阶段 4：对抗复核（必做）

执行序（细则与 prompt 骨架的唯一权威源=playbook-evidence-wording §10 + research-workflows §2，此处只列主干）：**扰动敏感度前置**（EVM 案，`cluster_sensitivity.py --dir <案目录>`，sensitivity_report.md 作复核输入；FRAGILE/STABLE 字样只进复核材料禁进报告正文）→ **惯犯揭盲**（实体冻结后 `label_lookup.py --unseal` 取封存命中，与实体划分互证/互斥）→ 本地反例自查脚本前置 → **N 路怀疑者 agent**（给数据文件路径让它**自己重算**，不是审阅文字；强制构造备择解释）+ 1 完整性批评角色查报告缺口（必查全史极值清单）+ 1 路**外部异构怀疑者**（codex/GPT-5.6-sol 单进程横扫全部结论）→ 判定三档 CONFIRMED/WEAKENED/REFUTED（**必须实际核查，"理论上可能"不算推翻**）→ 修订顺序先修数据管线再修文案，图表措辞同步改 → 修正记录印进报告附录。

## 阶段 5：报告

报告本体先写 `报告.md` + `charts/*.png`。**三张标准图必配**（阵营占比演变/庄级实体vs价格/价格与关键事件），直接调 `scripts/report/standard_charts.py` 的三个函数——规格与配色已固化，不要每次重新设计；**图 1/图 2 放 TL;DR 顶部（问 1 直答上方）**。**每个当前持仓 ≥20% 总供应或 ≥20% 流通的大庄/项目方必配一张全周期流转路径图**（v5.0 门槛；`scripts/report/lifecycle_flow.py`，样图 references/examples/lifecycle-flow-sample.png）。结构与措辞纪律见 `references/report-template.md`（三问逐条直答 + 标签体系 + 代币数量带【总量X%】 + 正文零地址 + 局限性独立成章）。然后 `python3 scripts/report/build_html.py --md 报告.md --out 报告.html` 出自包含 HTML（PDF 仅当用户点名，用 md2pdf.py）。质检：build_html 退出码 0（缺图会打 WARN 拒绝交付）+ 浏览器目检（图全显/表格无错位）。**附录四件套**（验证步骤/标签↔地址对照/复核修正记录/来源）——附录 B 地址对照任何情况下不可省（正文零地址的可验证性支点）。**监控包默认不做（v3.2）**：观察哨/两档监控建议/appendix.json 在用户确认买入后按 monitoring-package.md「买入后监控包」节补生成（新会话可执行，材料全在落盘产物；report-extract 四键/sentinel 纪律等格式标准原样不变），报告末尾带固定句"如决定买入，回复一声即可补生成监控包"。**默认交付另落一份 `analysis-state.json`**（appendix 的机器子集：token/whale_groups/vault_addresses/addresses 骨架+camp_share_series，无监控文案——/token-update 的实体表原料，防日后从报告文字反抄地址；schema 见 report-template「默认交付的机器状态文件」节）。交付前 checklist 见 report-template.md 末节。
...
对新标的做**筛查级**筹码分析（v3.12 新增，场景=币安 Alpha/现货初筛候选批量过一遍找类庄家盘）：分析引擎与完整版同强度——全量采集、对账三查、深度关联全套（金库归因/gas 溯源/行为指纹/女巫深挖/标签判级/大户排查双闸）、对抗复核路数**一分不减**；砍掉的只有完整报告（v5.0 起问 4 已从框架整体删除，不再是 easy 与完整版的差异点），交付缩为**两件套单页 HTML**（图 1 含价格右轴 + 按实体结构细分的阵营快照表 + 3–5 行判定块）+ analysis-state.json。判定块只给"是否值得跑完整版"的参考意见，**绝不自动升级**——用户人工决策后新会话跑 /token-analyze 同目录衔接（数据/聚类/判级直接继承，只补背调+完整报告+扩面复核）。全流程见 `references/easy-workflow.md`（E0–E7）。一币一会话铁律不变，跨币汇总不进分析会话。
...
**刀 1——机械活外包子代理（卸上下文；两档模型制。本节是外包档位的唯一权威源，research-workflows/update-workflow 只引用不重复）**：
1. 机械阶段一律派子代理执行，**模型两档制（用户 2026-07-28 定）**：**机械型 → `model: 'opus'` + `effort: 'high'`**（sonnet 降级档已作废，单价不再省——别拿"换便宜模型"当外包理由，也别因省钱把哪一路改回 sonnet）；**判断型 → 保持主模型：model/effort 都不传，继承主会话**（判断质量不降级）。**外包的收益只有一项：子代理不背主线几十万上下文**。写法：机械型在 Workflow 的 agent() 用 `opts.model='opus', opts.effort='high'`，判断型两参数全不传；**Agent 工具没有 effort 参数**（schema 只有 model），直调机械型只能写 `model: 'opus'`、effort 继承会话档位——需要钉死 high 的路优先走 Workflow。`fast` 无 per-agent 开关（`/fast` 是会话级），不必尝试传。**外包清单·机械档（opus+high）**：标准脚本跑批与重试循环（采集/余额扫描/CSV 加工）、对账三查的执行侧、标签库批量 lookup、**其他大户批量排查层**（标签库/惯犯库/指纹扫描/funder 批量溯源）、图表脚本执行、数据完整性验证、逐地址溯源 fan-out（按 schema 批量抓地址页）、**重点实体素材装配**（对象=达到流转图门槛的实体，即当前 ≥20% 总供应或 ≥20% 流通的大庄/项目方，每实体一路并行，模板见 research-workflows.md §二b；完整版与 easy 版同办）。**外包清单·判断档（继承主模型）**：对抗复核的怀疑者与完整性批评（复核另有 1 路**外部异构怀疑者走 codex/GPT-5.6-sol**——非 Claude 子代理、不占本档，见 research-workflows §2）。（v5.0：背景调研 fan-out 已随问 4 删除。）**禁止外包**：聚类判定、实体定性、对抗复核裁决（汇总仲裁）、报告撰写——一切需要"判断"的环节留在主线亲做（质量底线，铁律 6）。
...
| **★图表脚本静默跳过非标准阵营名**（自定义名如"项目方·官方系"→ 图只画出命中标准名的少数阵营，无任何报错） | `standard_charts.plot_camp_evolution` 按 `CAMP_ORDER` 白名单过滤 series 键；阵营名**必须**用标准名（项目方/大庄/小庄/离场庄/刷量地址/CEX托管/流动性池/其他大户/散户/桥锁仓/锁仓销毁；"狙击集团"仅旧数据重绘 legacy）。**出图后必须目检图例条数 == 你传入的阵营数**（GMX 实测传 8 个只画出 2 个） |
...
- `data-pipeline-evm.md` — BSC/Base/Arbitrum 数据管道**路由索引**（节→分册对照表）
...
- `data-pipeline-evm-recon.md` — evm 分册3 对账与重放（§5 对账 gate 四件套/§11 公共数仓准入与分工/§12 DuckDB 亿级重放缩图引擎/§13 时间抽查第二源）
- `data-pipeline-solana.md` — Solana 数据管线**路由索引**（IO 实录核验版；节→分册对照表）
- `data-pipeline-solana-scan.md` — solana 分册1 扫描与判别（§0a 双 RPC 互补矩阵/§0b 死亡名单/§1 全量扫描/§2a 托管判别五步法/§3a 签名投毒坑/§4 辅助数据面/§5 观测边界）
- `data-pipeline-solana-capture.md` — solana 分册2 采集与重建（§8 SQD 通道/§9 锚点法/§10 快照对比增量/§11 长币龄混合重建/§12 销户覆盖审计/§13 采集加速工程/§14 日级余额快照重建）
...
- `analysis-playbook.md` — 链无关方法学**路由索引**（三问总纲+节→分册对照表）
- `playbook-supply-recon.md` — 分册1 供给与对账（§1 分母口径/§1b 多链分母判据/§2 对账 gate/§8 cohort 留存质押/§8b 嵌套质押穿透）
- `playbook-entity-cluster.md` — 分册2 实体识别与聚类**路由索引**（节→分册对照表）
- `playbook-entity-cluster-methods.md` — 实体分册1 标注/归因/聚类（§3 标注三级兜底/§4 逐笔归因/§6 聚类硬规则含半枢纽与代买枢纽裁决/§6.5 节拍指纹/§6.6 否证检验）
- `playbook-entity-cluster-tiering.md` — 实体分册2 标签体系与判级（§6a 标签门槛与类型三分类/其他大户排查前置双闸/峰值与分母口径/合并指纹库/场景范式/资金通道判据/世代阵营法）
- `playbook-entity-cluster-cost.md` — 实体分册3 成本工具（§6b 配价方法优先级/流量存量双口径/染色分摊/退出深度比/出货美元核算优先级）
- `playbook-state-anomaly.md` — 分册3 状态评估与市场异常（§5 CEX 净流/§7 状态评估含行为归因/§9 刷量克制/§9a-9e 盘型库/§12 流动性形态取证/§13 回购链识别/§14 净持仓曲线陷阱）
- `playbook-evidence-wording.md` — 分册4 证据复核与措辞（§10 对抗复核/§11 措辞纪律）
- `research-workflows.md` — 调研 fan-out（v5.0 起背调路线废止，解锁情报路线保留）与对抗复核的 prompt 模板、任务编排纪律
- `report-template.md` — 三问报告结构与标签体系、三张标准图+全周期流转路径图规范（≥20% 大庄/项目方）、analysis-state.json（默认交付的机器状态文件）、措辞对照表、HTML 排版约定、交付 checklist
