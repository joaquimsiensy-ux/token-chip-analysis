...
- round4b 验收返工（B 同族残留）：G8 的 EVM preflight/stats 继续向下绑定 collector segment chain、确切 CSV/Parquet inputs 与 replay 输出，Solana meta 绑定 supply/GPA raw+meta/owner 输出，emit/check 均重验；shared release 的 accounting 与四查 producer 改为仓库路径白名单＋当前脚本哈希，新增受控 `adversarial_review_runner.py` 执行 receipt。存量只能重跑 production preflight/replay/scan/accounting/四查/reviewer，不得手工补字段。
- round4c 验收返工（B 同族残留）：Solana 与 EVM 修复深度对齐；从 `scan_token_accounts.py` 抽出生产同源 GPA/supply 解析函数，emit/check 离线重解析全部 raw RPC 响应，重算 account/owner/总量并逐条对账，堵住“六个手写文件＋全套自洽哈希”穿透。
- 测试：原 10 项与 round4b/round4c 同族残留的最小反例、同族变体与失败分支先红后绿；round4c 再增 1 个 suite 入口，合计 51 项全绿。成本：修复 3 轮；质量：第四轮 10/10＋验收残留全部关闭，未新增分析结论，漏检实体/传播级数字错误不适用。
