...
- **已验证观察的离线复用**：`scan_token_accounts.py <mint> --reuse-observation-bundle <source-bundle.json> --out <new-snapshot.json> --bundle <new-bundle.json> --work-dir <new-data-dir>` 使用原生 observation validator 验证源生产者、身份与所有已保存输入，再从 GPA/supply 重做账户解码、owner 聚合与三方闭合；必须与源 snapshot/holders 逐值一致。全程不建立 RPC 会话，保留原 endpoint、观察 slot、mint/activity 证明及原输入哈希，新产物明确标注 `observation_reuse.mode=verified-offline-reuse` 和源 bundle SHA/producer，不宣称发生新 live 观察。`--min-context-slot` 仍是下界；`--as-of-slot` 若提供必须等于原观察 slot，不能填更早的冻结 slot。观察晚于冻结仍按既有 runner `exact <= observed` 合约消费。
  输出 snapshot、bundle 和 work-dir 六项副产物须与源 bundle、其依赖及彼此互异；路径、symlink、hardlink 别名在 quarantine 前拒绝，源数据零写入。链式复用将祖先依赖继续绑定为 inputs；缺失/篡改证据拒绝复用。已确认输出安全后验证失败会隔离旧 PASS snapshot/marker 并写 ERROR；不传复用参数时维持原 live 行为。源文件未保存的 mint pre/post 原字节与活动 RPC 明细只继承原生产者已验证的观察证明，不凭空补造原始数据。
