...
6. **全量转账重放出各阵营占比演变序列**（阵营划分见 §6a）：分母＝当期净供应序列，**逐时点 assert Σ阵营＝100%±容差**，改过名册跑反向断言（casebook S-03）。三道配套硬闸（v6.8.0 三道互补防线，schema 权威定义 scan-schemas.md）：
   ...
   - **覆盖真空声明（用户 2026-08-01 确认接受）**：v6.8.0 删除 camp_jump_audit.py（阵营序列骤变归因闸）后，系统不再有"从最终阵营序列反向发现未解释大变化"的输出侧报警器——wave/flow 覆盖不了标签重分类、分母变化、慢速迁移类异常；无法归因的骤变按判断层义务写进报告"局限性"，本轮不做替代闸（不承诺永久）。仅存的输出侧轻量信号＝阵营重放产出时标记"单日阵营变动 ≥10pp"的日子，作为峰值逐笔触发日之一（无归因义务，见 tiering"峰值判级口径"条，2026-08-02）。
