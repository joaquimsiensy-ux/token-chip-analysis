...
6. **全量转账重放出各阵营占比演变序列**（阵营划分见 §6a）：分母＝当期净供应序列，**逐时点 assert Σ阵营＝100%±容差**，改过名册跑反向断言（casebook S-03）。三道配套硬闸（v6.8.0 三道互补防线，schema 权威定义 scan-schemas.md）：
   ...
   - **资金流异常扫描（硬闸）**：`python3 scripts/report/flow_anomaly_scan.py` 产 `flow_anomaly_report.json`——汇集点（14 日窗多来源进货 ≥2% 且 ≥5 源，Q1/3yMk 型枢纽）＋分发点双模式（脉冲滑窗／慢速批发全史 ≥500 收方 ≥2%，H9 派发器型）。
   - **候选裁决闭环＋溯源闸（硬闸）**：两扫描器全部候选按 `adjudication_validator.py`（template 起草 → 成员级逐条裁决〔等额组必查 top_sender_global_out_degree 识设施撞衫〕→ `validate --entity-file` 八类拒绝〔含 verdict 语义交叉约束与 linked_entity 名册绑定〕）闭环，**裁决完毕前历史大户兜底桶不准关闸**——"有兜底桶"≠"桶内被检验过"（casebook S-04）；实体表冻结前对临时实体表跑 `entity_source_trace.py`（祖先子图正向模拟：两锚点构成＋direct_upstream 进货单——Q1 教训：19 根 W1 藤裸露在进货单上无人看——＋FIFO/LIFO 敏感性上下界，翻转即 exit 2），新支路补候选回裁决环，`freeze` 机器强制四重前置（严格 verify＋裁决带名册＋溯源内容级重查＋哈希绑定，自报值一概不作数——v6.8.1 codex 复核后加固）。分段执行时跑批归 −1、裁决与溯源归 −2（split-run §1.3/§3.2）。
   - **覆盖真空声明（用户 2026-08-01 确认接受）**：v6.8.0 删除 camp_jump_audit.py（阵营序列骤变归因闸）后，系统不再有"从最终阵营序列反向发现未解释大变化"的输出侧报警器——wave/flow 覆盖不了标签重分类、分母变化、慢速迁移类异常；无法归因的骤变按判断层义务写进报告"局限性"，本轮不做替代闸（不承诺永久）。
