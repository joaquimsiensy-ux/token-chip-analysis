...
6. **全量转账重放出各阵营占比演变序列**（阵营划分见 §6a）：分母＝当期净供应序列，**逐时点 assert Σ阵营＝100%±容差**，改过名册跑反向断言（casebook S-03）；**历史清零层检测**——重放全期 max 仓位而非只看现仓（track set 按现仓筛会漏整个历史波次，casebook C-01 出处案）。
