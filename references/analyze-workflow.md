...
6. **全量转账重放出各阵营占比演变序列**（阵营划分见 §6a）：分母＝当期净供应序列，**逐时点 assert Σ阵营＝100%±容差**，改过名册跑反向断言（casebook S-03）。三道配套硬闸（v6.8.0 三道互补防线，schema 权威定义 scan-schemas.md）：
   ...
   - **候选裁决闭环＋溯源闸（硬闸）**：两扫描器全部候选按 `adjudication_validator.py`（template 起草 → 成员级逐条裁决〔等额组必查 top_sender_global_out_degree 识设施撞衫〕→ validate 六类拒绝）闭环，**裁决完毕前历史大户兜底桶不准关闸**——"有兜底桶"≠"桶内被检验过"（casebook S-04）；实体表冻结前对临时实体表跑 `entity_source_trace.py`（两锚点构成＋direct_upstream 进货单——Q1 教训：19 根 W1 藤裸露在进货单上无人看），新支路补候选回裁决环，`freeze` 机器强制三重前置。分段执行时跑批归 −1、裁决与溯源归 −2（split-run §1.3/§3.2）。
