...
15. 图 1 是否已经 `select_fig1_series()` 白名单机器闸并产生 `fig1_legend_receipt.json`：收据的实绘 camps 必须与绘图函数同源，非标准名 exit 2，豁免集合按 state 已绑定的 producer `series_format` 取自 `stack_exempt_for`：`sol-rows` 的真烧毁轨「锁仓/销毁」与 `burn_cum_pct` 记为 `non_stacked_metric`；`evm-dict` 仅排除 `burn_cum_pct`，其「锁仓/销毁」仍为在账持仓；`sol-anchor-rows` 不排除灰桶；无格式的历史重绘保持旧规则，overlay 必须记标签与组成 camps（判例：casebook/supply-accounting.md S-10）。
