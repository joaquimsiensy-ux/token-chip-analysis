...
15. 图 1 是否已经 `select_fig1_series()` 白名单机器闸并产生 `fig1_legend_receipt.json`：收据的实绘 camps 必须与绘图函数同源，非标准名 exit 2，豁免集按 state 绑定的 producer `series_format` 由 `stack_exempt_for` 派生：`sol-rows` 豁免 `burn_cum_pct` 与「锁仓/销毁」（真烧毁轨，净供应分母外，图一按净供应标注、不堆叠仅在报告披露）；`evm-dict` 仅豁免 `burn_cum_pct`（其「锁仓/销毁」仍是在账堆叠桶）；`sol-anchor-rows` 保留在账灰桶堆叠；无 `series_format` 的历史重绘保持旧规则，豁免原因记 `non_stacked_metric`，overlay 必须记标签与组成 camps（判例：casebook/supply-accounting.md S-10）。
