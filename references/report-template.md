...
下表是**速查副本**——认定规则、峰值取法、合并口径细则、其他大户排查前置双闸的唯一权威源=playbook-entity-cluster-tiering §6a（改门槛须三处同步：tiering/本表/SKILL.md 阶段 3）；本节管报告怎么写。

| 标签 | 认定标准（实体合并口径=含全部疑似关联地址，同一时刻合并计算） |
|---|---|
| **项目方** | 金库、创始人、团队、基金会、vesting 托管、项目方做市仓等一切与项目方相关的实体，**无论持有份额** |
| **大庄** | **当前**持仓 ≥20% 总供应 或 ≥20% 流通 |
| **小庄** | **当前**持仓 ≥5% 总供应 或 ≥10% 流通（未达大庄线） |
| **离场庄** | **同一时刻历史峰值** ≥10% 总供应 或 ≥15% 流通，且当前既非大庄也非小庄 |
| **刷量地址** | 对倒刷量执行地址（集群），单独打标签；可证明与其他集团关联时标复合标签"大庄#N·刷量地址""离场庄#N·刷量地址" |

- **狙击集团标签已废止（v5.0）**：发射窗协同实体按普通门槛判入大庄/小庄/离场庄，协同建仓行为写进该实体小节；识别与合并方法全保留（tiering §6a 废止条款）。
- **报告只对以上标签实体展开分析**。其他大户（当前 ≥0.1% 总供应或 ≥0.2% 流通、未入任何标签；v5.0 门槛下调）必须先过排查前置双闸（tiering §6a）才准归阵营；排查后仍独立者与散户只作为阵营出现在标准图 1 中，正文不逐个开小节，阴性排查小节汇总交代排查覆盖数。
...
**特有章节现成模板——死币复活盘/CEX 拉盘专题**（2026-07-13 定）：标的命中 playbook §9a 盘型（老币长期零链上吸筹却突然暴拉）时，特有章节按 §9a 五阶段指纹逐段取证成章（钱包预置→点火弹→洗量机器→创建者借势出货→二次脉冲），并把 §9a"可捕捉信号排序"落成第六章观察哨条目。
...
| # | 图 | 函数 | 放置位置 | 要点 |
|---|---|---|---|---|
| 1 | 各阵营持仓占比演变（100% 堆叠面积）+ 价格右轴 | `plot_camp_evolution` | **一、TL;DR 顶部** | 阵营=标签体系固定配色顺序（红=项目方/橙=大庄/金=小庄/棕=离场庄/粉=刷量地址(有仓才列)/蓝=流动性池/青=其他大户/绿=散户/灰=锁仓销毁；紫=狙击集团仅旧数据重绘 legacy）；y=占总供应量%；数据=全量转账重放快照序列；其他大户与散户只在此图出现；**price_series 必传**（v3.12，2026-07-21 用户定：价格+筹码对照）——右轴黑线白描边，默认线性、量程>30 倍自动切对数（图例条目自带"(对数,右轴)"标注），完整版与 easy 筛查版都传；仅旧报告基线重绘可不传。**混合重建标的（锚点插值/截断流水+快照封口）加两条**：①末日封口造成任一桶跳变 ≥2pp 的，图注**逐桶**注明"±Xpp 为口径切换（稀疏观测→全量快照）非当日转账"——只解释其中一桶、放任更大跳变被读成末日大转账即违规（GOAT 案 -12/+13pp 跳变仅解释了主实体的 2.8pp，外部复核抓出）；②作图脚本输入必须是**修正后终版数据文件**——数据修 bug 出新版后旧文件当场改名失效（evidence-wording §10 第 12 条），防作图脚本静默读脏版（GOAT compose_evolution 读去重前发射窗致 LP 桶高估 0.83pp）（来源：GOAT(Solana) GPT5.6 外部复核采纳，2026-07-24） |
...
- **宏口径边界：`{{e.peak_share}}`=日末序列峰值，日内事件占比禁用宏**：peak_raw 来自日末快照序列，闪电过手型实体（单笔吃下→当日部分回吐）的**日内峰值高于日末峰值**（EGL1 案发射窗协同实体单笔买入 47.11% vs 日末峰值宏 40.21%，TL;DR 与正文两处误用宏被外部异构复核抓出）。规则：单笔买入/日内持有语境的占比一律手写并标注"单笔/日内"口径；报告含此型实体时，日末峰值与日内峰值两口径并列写清（另见流转图 footnote 块级峰值声明纪律）（来源：EGL1(BSC) redo2 @CX 复核，2026-07-28）
...
| "3 个庄" 直接断言 | "项目方在场；大庄#1；小庄#1、离场庄#1（其中伪装分散型为高度疑似）" | 按标签体系逐项计数，类型③上限"高度疑似" |
