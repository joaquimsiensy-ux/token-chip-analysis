...
- camp_share_series：固定为 `{"dates":[...],"series":{"阵营名":[...]}}` 对象（**这是 figures_from_facts.py fig1 的直接输入契约**，逐日对象列表是旧文档笔误，引擎从不支持；每个阵营数组长度必须与 dates 完全相同）；阵营键用当前标签体系名（缺的阵营省略；旧文件里的"狙击集团"键仅旧数据重绘 legacy）
