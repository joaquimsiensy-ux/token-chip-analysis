...
**症状**：v2 把"HTTP 200 且流完整读完但零行"和"HTTP 异常/连接中断"一起归为 `last is None` 走失败重试,6 次后记 `gaps['scan-fail']`。这些纯空 slot 段**永远补不回来**（重扫还是零行）,于是任何以 `gaps == []` 为完成判据的调度器都判不到完成——BONK 六分片共 **365 段**此类空洞,watchdog 每 20 分钟重启一次分片、S00R 被重启 **59 次**、空转约 **24 小时**。
...
**症状**：旧版收尾无条件 `all_edges = list(old_edges)` + 载入全部 parts + `sorted(set(all_edges))`。BONK 三个分片各 **3,900-5,900 万行**,按 165 字节/边算峰值 **13-19GB**,16GB 机器必 OOM；更糟的是 `gzip.open(cache_fp,"wt")` 边算边写,OOM 落在写入中途会留下**损坏 gz**,下次启动触发"缓存损坏——重新全量",几小时工作作废。开局那句"把旧缓存全量 load 成 list"本身也是同一个 OOM 点。
...
- **定位（验收后降级）**：❌全程采集第二引擎**禁用**（代码已就位于 fetch_sqd_transfers_v2 `--hypersync` 开关，运行时打硬警示，仅限吞吐实验/对照——双引擎 1 万 slot 实测 1,175 slots/s ≈ 纯 SQD 2.2-2.8 倍，GA 后重验收即可启用，对账脚本已收编 `scripts/solana/hypersync_recon.py`——改 MINT/FRM/TO 按三区各跑一轮+Helius 终审定责，3.18.1）✅摄取前沿附近的对照源/指纹查询（fee_payer 服务端过滤仍是 SQD 没有的能力）仍可用。
...
- **⚠混合分段提议已评估否决（2026-07-22 @CX 交叉复核，勿再重议）**："近期段 HyperSync+历史段 SQD"不成立：滚动窗口 196 天是**覆盖范围不是准确范围**，实测干净区仅前沿约 20h（且是单次抽样非承诺），14h 外即见静默暂态洞；洞静默（单跑 HS 无法自知缺数据）→证明某段完整的唯一办法=SQD 重拉该段对账，HS 等于白跑。codex 补充两条硬论点：①供给对账兜不住成对缺行（一笔转账借贷双缺时供给仍守恒），完备性验收必须落到边集合一致；②认证完成时间 T≥max(S_SQD,H)+差集修复,SQD 全量扫描恒为关键路径,双引擎不缩短认证耗时。唯一理论净收益架构（SQD 只出键清单+HS 出载荷）依赖 SQD 服务端摘要能力,现实不存在,不立项。
