...
2. **decode 通道坑**：`getTransaction` 直连 `api.mainnet-beta` **恒 429**，必走 clash 代理（`decode_txs.py --proxy http://127.0.0.1:7897`）；requests.Session 连接复用比 curl 逐发快约 3 倍。金额只用 raw integer，输出 `deltas_raw/pool_balance_raw + decimals`，UI 字段仅为精确十进制字符串；缓存及断点输出绑定 mint/pool/RPC，`decode_fail` 不算 done。dRPC 免费层 Solana 需付费（`chain is not available on freetier`），别用。
