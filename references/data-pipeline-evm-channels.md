...
| **GT 逐池 TVL 伪影 + V4 幽灵仓 + LP 归属定池属** | GT 的 V4 逐池 TVL 多处鬼影（近空池显示数十万美元 TVL）；V4 头寸可转移 ⇒ 按 (pool,tx_from) 聚合会出"幽灵仓"——V4 外部 LP 硬口径只有 **receipt 净现金流**（逐地址投入/提取实额）。**池子算不算体系自有，以当前 LP 归属为准**（V3 看 NFT tokenId 持有人份额、V4 看 receipt 净额构成），"体系曾在此做过市"不算：体系 LP 连本带费撤离后的池=外部池，须从控制口径摘出（QUQ 案 Pancake 池摘出，67.4%→64.6%） | （QUQ LP 补查，07-22） |
