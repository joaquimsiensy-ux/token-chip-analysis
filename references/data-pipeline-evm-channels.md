...
| **V3/V4 LP 费口径鸿沟 + swap 回执速查** | V3 有独立 Collect 语义，但 `collect−burn=费` 只有在同仓位、同边界且处理期初 `tokensOwed`/跨窗结转后才成立，且买卖双边各收；V4 普通 `ModifyLiquidity` 事件不公开本金与费的拆分，然而核心调用会分别返回 `principalDelta`/`feesAccrued`，池费可按逐 Swap 真实 fee 计算，仓位费可按逐 tick 活跃流动性重放，当前未结算费可由 `feeGrowthInside` 计算。**“事件不直接给”≠“算不出来”**；四层口径与公式统一见 `lp-fee-accounting.md`。swap 回执速查三招：①V4 腿特征=代币对手方是 PoolManager 单例，直连池合约收付=V3 系（Uni/PCS 再按 Swap topic 分家）；②池费率必须从池状态/Swap 事件读取，动态费禁用静态假设；③路由抽成验证=各拆腿输入之和 vs 用户付出额；路由按笔实时比价，同一标的先后两笔可走不同池组合 | （来源：QUQ(BSC) LP 手续费专项复核，2026-07-24；修正 2026-07-22 旧条） |
| **GT 逐池 TVL 伪影 + V4 幽灵仓 + LP 归属定池属** | GT 的 V4 逐池 TVL 可能有鬼影；V4 头寸可转移，按 `(pool,tx_from)` 聚合会出“幽灵仓”。官方 PositionManager 头寸优先按 `ownerOf(tokenId)` + NFT Transfer 历史 + `(owner,tickLower,tickUpper,salt)` 状态链确定归属；自定义 manager/复合金库再用 receipt、内部 trace 与份额账本穿透。receipt 净现金流只证明投入/提取，不等于手续费账本。**池子算不算体系自有，以目标时点可证 LP 归属为准**；“体系曾在此做过市”不算，体系 LP 已撤离的池须从当前控制口径摘出 | （来源：QUQ(BSC) LP 补查，2026-07-22；归属口径修正 2026-07-24） |
