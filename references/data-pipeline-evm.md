...
> Arbitrum 已有 `labels-arbitrum.csv` CEX-only 初版，resolver 不再因缺表进入 degraded mode；但基础设施/协议/桥/池等静态覆盖仍为空白，剔除仍依赖现场 `getCode`、协议识别与行为守门员动态闸。其 `release_tier=exploration` 不变，只能产探索结果，不得封 A4/A5 seal 或编译正式 analysis。
