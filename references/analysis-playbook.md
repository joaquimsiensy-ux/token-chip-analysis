...
**历史来源**：方法条目的案源与恢复记录见 CHANGELOG；正文行尾继续用全名＋日期标注。
...
**建仓成本＝按需工具（2026-07-14 用户定）**：出货获利、浮盈状态等结论需要成本参照时按 §6b 就地计算引用，不单设章节、不强制每实体四件套。
...
**判定纪律——阈值样本只能"找事"，不能"定案"**：金额门槛只用于缩小扫描面；任何净额、盈亏、净持仓或是否离场的判定都必须回全量流水重算，否则截断偏差可把方向写反（判例：casebook/supply-accounting.md S-09）。

§1 §2 是所有结论的数据地基；§10 §11 是发布前的质量闸门，任何一问的答案未过 §10 对抗复核、未按 §11 措辞纪律改写，不得进入报告。
