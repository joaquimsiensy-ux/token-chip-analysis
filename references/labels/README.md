...
| 文件 | 用途 |
|---|---|
| labels-eth.csv（含 privacy 子表） | ETH 主表；tornado-user 隐私层由 resolver 自动合并 |
...
- **serial-actor（惯犯庄家层）**：历史案定性的收割集团地址，由 appendix.json + analysis-state.json 双源回灌。库内定性属于案内自评，多数未经用户复核；命中后先判案源成色，未经复核的只按**线索级**消费，报告禁用“实锤”、改写“历史案标记”。**不剔除、不禁边**，跨案命中只提高深查优先级；结论未经用户复核不入库，`accumulate_offenders.py --apply` 仅随用户明确要求的复盘执行。组级收纳可能含一次性执行地址，远端成员必须按“提示不定罪”复核。
- **跨案身份冲突防线**：候选地址同时是主库设施身份（cex/infra/bridge/dex/bundler/paymaster/mev）或命中 benchmark 设施金标时，必须写 `sources/serial_conflicts_<日期>.json/.md`；**primary/goldset-infra 级冲突地址必须在 `--apply` 与手动 add_labels 两条路径被硬闸拦在 serial_actors.csv 外，secondary/cross_chain 仅提示**。被拦地址逐条三选一裁决：①案源实体误吸设施→修 whale_groups 并重跑；②主库标签错→走 curation override；③确属庄家自建设施→人工单独 add_labels，并在 evidence 写裁决依据；无 `--force` 绕闸。事故与机制见 `casebook/entity-clustering.md` E-02，curation 操作见 `labels/MAINTENANCE.md`。
