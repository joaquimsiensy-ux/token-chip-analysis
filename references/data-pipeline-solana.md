...
> **来源声明（2026-07-12 修订；2026-07-31 标注反转）：本文档原自 IO 分析最终报告反推，2026-07-12 经用户找回的 IO 会话实录逐条比对核验，并据实录补入反推不可能推出的坑（双 RPC 互补矩阵、方法级屏蔽、签名列表投毒等）。scan 分册规则**默认＝实录已确认**（原 `[VERIFIED·IO实录]` 逐条标注已反转为默认、不再标），仅例外才标注：**
