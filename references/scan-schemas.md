...
**camp_share_series 数值面（compile_state 无条件校验，与 --series-source 无关）**：桶名白名单＝`standard_charts.CAMP_ORDER_MODERN`（∪ `burn_cum_pct` 豁免键；legacy 名与实体级自造桶名新报告一律拒），图一实绘集合同样按 `series_format` 派生豁免；全值有限；按 `series_format` 派生的实际堆叠桶值域 [0,100]，堆叠外披露桶只验非负有限；日期轴统一 UTC 解析后严格递增无重复（naive 视为 UTC，带时区先换算再比）。
