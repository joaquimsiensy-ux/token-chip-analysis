...
**formal 必经与探索豁免（F-C1 消化轮）**：`state_from_facts.py` 默认（formal）编译 **--series-source 必填**——缺席 BLOCK exit 2，来源绑定不挂可选参数；显式 `--exploration` 才可不带，产物 provenance 落 `series_binding="exploration-unbound"` 非正式标记。formal 绑定产物落 `series_binding="producer-sidecar"`＋`camp_series_sidecar` 块；`source.provenance` 不得预置这两个键（只能编译器按验证结果生成）。下游 `audit_release_gate`（new-analysis）复验：state 含 camp_share_series 就必须是 producer-sidecar 绑定、且绑定的序列实物（案根与 data/ 两层）sha 一致——exploration 产物与无标记手编 state 在发布闸拦死。绑定路径的闭合按 sidecar `denominator` 单式严判（净分母族只认非 burn 之和、total 族只认全桶之和，两族不得互救；无口径信息的手填路径保留双式宽判）。登记面命中是结构化校验：supply_truth.json 本身须过 schema（supply-truth-receipt/v3）/verdict/exit_code 三验且 sha 必须命中 `inputs.replay_stats.sha256` 特定字段位（sol 侧 reconcile 收据同理验 schema）——"含某个 sha 字符串的任意 JSON"不算登记面。
