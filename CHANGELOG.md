...
- **9.2.1**（2026-09-25）修复标签 CLI 盲化漏封：按惯犯结构化标记精确匹配并整行封存混合行，补离线集成回归；其他出口登记后续单，档位 修。
...
## [9.2.2] - 2026-09-25 — replay_duck --only-addrs 哈希分桶流式补算

- **出处与根因**：QUQ(BSC) 0922 案补算资源失败（调度方提供），旧入口在地址过滤前物化全部原始行和去重事件，并执行全局冲突聚合；本工程只改地址子集补算路径。
- **改法**：原始行改为 VIEW，复用逐通道保留行 COUNT，以完整去重键哈希分桶；同键同桶，因此桶内完整键冲突检查与全局查重等价。每桶先检查全部保留行，再过滤端点、去重并汇总增量，最后跨桶合并 `(a,b)`；正常每桶两条覆盖源范围的查询，以 IO 换内存。峰值窗口、回退、收据构造与原子写出保留原文。
- **验证**：见本工程 `maintenance/repair-20260925-replay-only-addrs-scale/W1_equivalence.txt`、`W1_timing.txt`。主夹具双格式均为 2,000,000 原始行、1,999,000 唯一键、1,055 补算地址；K=8 最大桶 250,811 行，K=8/K=1 结果均与真实基线收据全等，全量业务产物与绑定分别核验。CSV 基线/K=8/K=1 墙钟 10.03/21.82/11.26 s，v2 为 1.77/4.82/1.78 s；RSS 与完整执行计划见证据。另测 4,999,000 唯一键宽字符串单桶冲突聚合成功，墙钟 6.64 s、进程 RSS 2,269,954,048 B（getrusage）；`time -l` 的 RSS 查询被沙箱拒绝，指定测量方式列为环境未完成项。
- **成本与边界**：分桶阶段 K=8/K=1 分别 16/2 条源查询，另计预检摘要、reject、保留行 COUNT、v2 值域探测、maxlen 与失败取样；查询数不等于完整文件字节读取倍数。`SEG_ROWS=5_000_000` 为目标平均规模非上限，偏斜只告警，`CHIP_REPLAY_SEG_ROWS` 仅测试。默认桶规模已补上述宽键实测，不能外推亿级或偏斜上限；全量路径计算未动。生产变更 +92/−10、新增私有 helper 1、CLI 参数 0；定向测试与施工记录见 `W1_done.md`。合并后案卷须用最终合并版脚本重跑 `--only-addrs` 再过发布闸，旧收据不得沿用或手改 sha。

## [9.2.1] - 2026-09-25 — 标签 CLI 惯犯结构化标记盲化补漏

- **改法**：`label_lookup.py` 新增纯函数 `serial_marked()`；严格布尔 `serial=True`、去空白后的惯犯类别、解析后的风险旗标、按 `+` 拆分的来源项任一精确命中即整行封存。非空非法风险类型保守封存，list 仅为辅助函数输入兼容；JSON 与文本共用过滤结果，单链未命中及跨链行为保持原约定。
- **代价与边界**：混合行即使已被人工改成设施类别，A2–A3 仍整行封存，暂时看不到其设施身份、`no_merge`、`exclude` 及其他有效标签，A4 揭盲恢复；带历史标记不等于当前确为惯犯，须揭盲后人工判断。不改 resolver 的 `serial` 定义或显式 `--unseal` 路径。其他消费者与 resolver 出口已登记于 `maintenance/repair-20260925-label-blind-leak/README.md`「后续单候选」，本版只修 CLI。
- **验证**：新增判据正反例与临时合成 CSV 集成测试，覆盖参数盲化 JSON、环境变量盲化、文本、非盲化对照及跨链 misses 边界，加入 `run_all.py` 显式清单。实际运行结果见本工程 `W1_done.md`。
- **成本与质量**：生产逻辑改动 1 个文件，未联网、未读取真实案目录；修复前合成数据已复现 4 个 CLI 场景漏封。完整测试结果与提交记录见完工报告，不预填通过结论。
...
- **改法**：按 W3→W1→W4→WR-a→W2→WR-b 顺序：curl_json 压缩；可信来源驳回继承、原始时效及来源副本绑定；α/β 候选状态探针并入 census（β 搜索不变）；repair 四协议登记 15822564…、旧 3f89aab1… 保持 ACTIVE；find-known-map 离线轻筛、确定性排序与扫描故障优先 exit 1，−1 非 resume 开工查图及导出回填纪律；probe 两协议登记 d4adc0c8…（WR-b），旧 c4980c98… 保持 ACTIVE。resume 不加载或叠图且不保留驳回继承。
- **字节与测试**：记录见 [W3_done.md](maintenance/repair-20260924b-sol-stage1-speed/W3_done.md)、[W1_done.md](maintenance/repair-20260924b-sol-stage1-speed/W1_done.md)、[W4_done.md](maintenance/repair-20260924b-sol-stage1-speed/W4_done.md)、[WR-a_done.md](maintenance/repair-20260924b-sol-stage1-speed/WR-a_done.md)、[W2_done.md](maintenance/repair-20260924b-sol-stage1-speed/W2_done.md)。W3 生产 +2/−0；W1 探针 +130/−7、校验器 +352/−5；W4 生产增删合计 52 行；WR-a 登记 +32/−0。W2 生产 +156/−1、测试 +238/−0；相对 W2_BASE references +1,716 B、commands +121 B，SKILL 8,021→8,021 B，索引 173 B。W2 六项允许的离线检查通过，probe/export 24/24 组（含新增 find 4 组）；W1/W3/W4 的禁读受限项仍依各报告移交。调度方本机：docs_lint/changelog_lint PASS；run_all 150/151（唯一红＝test_stage2_reseal 验收 worktree 缺失，环境项）；producer 登记守卫 0 FAIL；WR-a/WR-b 真实入口验收 PASS（[WR-a_formal_entry.md](maintenance/repair-20260924b-sol-stage1-speed/WR-a_formal_entry.md)、[WR-b_formal_entry.md](maintenance/repair-20260924b-sol-stage1-speed/WR-b_formal_entry.md)）；收官 review PASS（P0/P1 空，P2/P3 已按 W5 修正）。
- **成本-质量指标**：W2 生产文件 1、测试文件 1、新公开子命令 1、新私有 helper 4；沙箱外部网络调用 0。W3 联网单 slot 对照：传输字节 4,516,539→736,729（gzip，规范化结果摘要相同，fable_probes P5）；W4 合并查询对照（fable_probes P3）：header 在 probe-only／census-only／combined 三方全等，transactions combined＝census-only，instructions combined＝probe-only；完整生产链路吞吐收益未证明；W4 离线 workers=1/4 逐候选 slot 为 SQD census=1、旧 probe=0、Helius getBlock=1。W2 新增轻筛拒绝 38 类非法形态，并测损坏 JSON/UTF-8、文件读取失败及部分扫描错误；不重算二进制摘要、不解压、不逐 slot 分类。全工程轮次数、Bash 调用数、交付用时、初稿关键结论数、复核判定分布、漏检实体数与传播级数字错误数未记录；不预填线上收益。
...
- **版本与请求**：新增公开接口 `--adopt-pending` 与可选持久化字段 `header.adopted`，记次版本。Solana 请求统一引用 `endpoint_identity.SOLANA_MAX_SUPPORTED_TX_VERSION=1`，修复请求模板只保留一份；升上限或改模板字段语义须同步修改 producer 版本钉、登记哈希并记 CHANGELOG。AST 守卫拒绝请求字典内写死数字上限。
...
- **范围与纪律**：同 7.1.2——只修"同一事实多处说法不一"与"文档写法与 scripts/ 实际行为不符"；结构/门禁有效性/复杂度/fail-closed/代码质量/上下文预算一律不动（用户 09-19 裁决）。原则：删除 > 修改 > 新增、只改文本、需改代码者说明原因等用户审批（本期零例）。动机：7.2.0→9.0.1 六个修复工程代码 +2806/−118 而 references/SKILL/commands 零改动。
- **工艺**：两路并行只读盲审（a 路＝全范围术语表法，R5 起改逐文件全量兜底扫；b 路＝`git diff 4cbfe48..HEAD -- scripts/` 变更区专审，对每个改动脚本反查文档提及点）→Fable 逐条亲核两侧原文与代码写出点→Fable 写工单（锚一律目标文件整行原文、`grep -n -F -x` 恰 1 处、字节本机逐字 UTF-8 模拟）→codex 只读复核（退回即改版、可选压缩一律吸收）→codex 写模式施工→Fable 新旧文本计数＋三组字节＋九项守卫＋白名单 diff 验收→commit/push→下一轮。原判据＝两路各连续两轮 0 条；实际 b 路 R10–R13 四轮 0、a 路 R12 0 后 R13 又报 1 条 minor，用户 09-19 裁决 R13 修完即收官。
- **修复要点（按轮）**：R1 9 条——钱包公式限 strict、facts/state_source 分工、exit 3 改换源重跑、峰值宏口径、图 2 收据、`--pct` 显式、token-analyze-2 删"第⑤条"（已部署同步）等；R2 3 条——Multicall3 补 `--chain`、evm-sources 章节号、report_facts.json→facts.json、Solana 预检命令补 `--exploration --out`；R3 3 条——scan-schemas 七处（删 `--live-canary` 位图对表、sqd_blockhash 可 null、shared_map 回退对象类型放宽、canary.slots 长度口径）；R4 4 条——research-workflows 裸数组模板、labels/README labels_meta、robinhood decimals `or 18` 与 quote_decimals 回退口径；R5 3 条——split-run −1 示例补 `full`、S-04 "无 sig"禁令收窄为旧五元组、bloXroute §3.3 改为 scan_bloxroute_seg 实际行为（删旧脚本两行）；R6 2 条——Alchemy 续采删"按 tx hash 去重"（重放键含事件序号）、聚类措辞锁定收窄为共用出纳型；R7 3 条——monitoring-package 重编译命令补"含宏加 `--facts`"、删已定论不存在的 SQD 专属端点待办、3a 标题删错误计数；R8 4 条——扫描器不按 dataSize 过滤、accounting-gate/v2 句限定 EVM（Solana 为 v1）、删三处失效 Plan B/§8 引用、C-06/E-12 六处引用改不带路径惯例；R9 3 条——监控包正式重封改走 `stage2_closeout reseal --from a4`（显式 `--seal-files` 须含原 extras，重走 A5；原指引直接 a4 finalize 被 terminal 检查硬拒，blocker）、删 Etherscan 免费层仅 ETH 的错误断言、address-book 失效条名改指 C-07；R10 1 条——G8 离线重放契约存量句改"producer 哈希须匹配当前扫描器"（原"实物完整可直接 emit"过不了 identity_snapshot_receipt 哈希校验，blocker）；R11 2 条——快照示例 `--rpc` 改完整 URL、VIRTUAL 军资案例删"差 8 倍"；R12 两路 0；R13 1 条——销户抽样退出码句删无条件 exit 1、改"漏边优先 exit 2"（audit_closed_accounts 先判 LEAK_FOUND/2 再判 INVALID_SAMPLE/1）。
- **字节与产物**：21 个文件、+50/−52 行；references 三组 glob 930076→929092（净减 984 B）、SKILL.md 8021 不变、commands-staging 8798→8789；每轮九项守卫全绿；`invariant_manifest`/`contract_manifest` 不动。
- **范围外残留（登记不修）**：`scripts/solana/fetch_sqd_transfers_v2.py:26`、`:1301` 帮助文字仍写"拿到专属端点后 --url 换掉"，该脚本对自身全文计算 `collector_sha256`，改帮助文字即改采集器身份，留待日后升版一并处理（工单 R7 §2b）。
- **工艺教训**：工单锚必须整行＋`grep -F -x`（R3/R4 片段锚各退回一次）；工单"代码行为"断言须亲核写出点（R7 D1 `--facts` 老案无 facts.json、R9 D1 显式 `--seal-files` 整体替换 extras、R11 D1 Helius 默认分支、R13 D1 :95 同款联动，均由复核揭穿后改版）；写工单的 python 勿在双引号 f-string 内套中文双引号（R9 一次工单未写出却派了复核，任务自行报缺失无副作用）；哨兵 grep 配额须用 `hit your usage limit` 全句（`usage limit` 会撞自己提示词回显）。
- **成本-质量指标**：codex 任务 50 次（盲审 28 含 R2 两次重派、复核 12、施工 12、误派 1）；Fable 亲核 38/38 条属实（另 1 条待确认项亲核升格、1 条待确认项裁定不属漂移）、0 条误报采纳；工单复核退回 9 次均在派工前拦下；外部网络调用 0；不运行真实案卷判断。
...
- **成本-质量指标**：生产逻辑文件 5（net、rpc_batch、price_check、stage2_closeout、facts_gate）、新公开入口 0、新增产物输出键 3（收据 `price_file_sha256` 1；facts `token.circulating_supply_raw`、`token.circulating_supply_source` 2）、新增 SUITE 入口 0；外部网络调用 0（RPC 与第二源全部替身）；不运行真实案卷判断链，判断结论不自动变更。
...
- **F05**（`facts_gate.py`；`audit_release_gate.check_facts_decimals`）：`gate_check` G2 加每实体 `peak_raw≤total_raw`；`derive_facts` 对 `peak_overrides` 证据文件读 JSON，须为 `{entity_id:{peak_raw,peak_date}}` 且与申报相等（hash 只证文件没变，不证值来自文件）；`peak_date` 严格 `YYYY-MM-DD`（`fromisoformat` 回写相等），provenance `anchors.current` 显式给出 `date` 时不得晚于该日（无 date 不设上界）；新增 `check_facts_decimals` 只挂 `_run` 的 new-analysis 分支：solana 家族取 `accounting_mode.checks.decimals`，evm 家族取 balance 对账收据绑定的 verify_recon config `decimals`（复用已缓存 witness），取不到或与 `facts.token.decimals` 不等即拒；共享 `check_facts_vs_ledgers` 源码未改；stage2 收口/reseal 未新增 decimals 核验，但沿既有 `derive_facts`→`gate_check` 调用同步采用峰值上界、日期与证据校验（迁移后须重建 stage2 收口收据）。`test_report_facts` 用例 16–21（证据不一致／一致／峰值超总供应／坏日期两形／晚于当前锚点／证据为数组），`test_review_20260804_p105` decimals a/b 两例，batch_d 与 audit_release_gate 夹具补 `decimals`。
- **工艺**：每段工单先 codex 只读复核（F06 一轮、F04 两轮、F07 两轮、F05 两轮，回复均为退回意见），按意见修订（F06 v2、F04/F07/F05 v3）后 codex 写模式施工，Fable 本机验收（白名单 diff 对照工单、定向测试、字节）后 commit，再 codex 常规盲审：F06 r1 PASS、F04 r1 PASS、F07 r1 FAIL（1 minor：迁移命令漏必填 `--out-dir`，纯文本）→r2 PASS、F05 r1 PASS。只读复核与盲审并行派、施工串行；施工中不 commit；施工与盲审同碰一文件时串行。
...
- **档位与迁移说明**：按 09-18 计划记 7.2.1（修）——四段皆既定契约内加固，无新入口/schema；codex 工单复核 E-R1 持异议：F05 把原先接受的自由格式 override 证据（如 `{"note":…}`）改为强制 `{entity_id:{peak_raw,peak_date}}` 并要求迁移，属输入契约收紧，按 CHANGELOG 头部规则倾向 8.0.0（主）。**待用户追认或改判**（改判只需改四处版本号与条目标题）。存量迁移代价：①`block_precision_followup.json` 缺绑定字段、`producer.sha256` 与当前引擎不符、或 `channels` 未命中案内唯一实物/哈希不符时，须用当前引擎 `replay_duck.py --channels <通道清单> --out-dir <补算工作目录> --only-addrs <产物目录>/needs_block_precision.json --only-addrs <产物目录>/trigger_days.json` 重跑补算（升级引擎即重跑；`--only-addrs` 自 R09 起已写出这些字段，闸侧此前不消费）；②`peak_overrides` 自由格式证据文件须改写为 `{entity_id:{peak_raw,peak_date}}` JSON，重算证据 sha256、更新 `state_source.facts_inputs` 的 override 绑定，再依次重建 facts、图 2 收据、A4、stage2 收口、A5；③`state_source.facts_inputs.decimals` 须与链上观测一致，填错即 BLOCK。旧案在 HEAD 下 BLOCK 由钉版机制承接（`~/.claude/skill-pins/`）。
...
- **A＝R08**（`figures_from_facts.py`）：`_load` 用 `parse_constant` 拒 NaN/Infinity 字面量；图 2 对账对 pct 全序列做类型＋有限值检查（拒 bool/非数/非有限/字符串宽容）；`mode_check` 把输入解析失败收敛为 FAIL 退出，并在两输入文件在场时尝试覆盖 FAIL 收据（写失败打印"收据未更新"仍失败退出；series 非 list 等既有不产收据分支保留）；`dumps_fig2_series` `allow_nan=False`。`test_repair_batch_c` 新增 R08 13 例（完整模块 244 checks）。
...
- **D＝R09**（`audit_release_gate.check_daily_peaks` 重写；`peaks_daily.py`；`replay_duck.py --only-addrs`）：rglob 定位 `peaks_summary.json`（跳隐藏/`_history`/符号链接），零份 return、多份拒，伴随文件相对产物根解析；summary 新增 `needs_block_precision_file/sha256` 与实物咬合（旧产物＝升级重跑）；needs 各档 ∪ 触发日 `active_candidates`（缺项/null 不作零候选）非空时须 `block_precision_followup.json`（schema `block-precision-followup/v1`、engine replay_duck.py、inputs 必绑 needs sha、触发日候选非空时还须绑 trigger sha、addresses 覆盖每址 `{peak, peak_blk}`，peak>0 ⇒ 非负整数区块、peak==0 ⇒ null，两侧小写归一）；生产者 `--only-addrs`（needs 字典/trigger_days/地址列表，可重复）只算并集地址块级峰值（窗口 SQL 与全量逐字同源、无门槛、无事件补 0/null），收据写首个输入所在目录，整段跳过 pass1/merged/pass2、坏事件分支也不写全量 `replay_stats.json`，坏 JSON/空并集/坏形状 exit 2。原六条错误文案逐字保留。`_r09_case_1..13`、`followup_case`、peaks_daily needs sha 断言。`data-pipeline-evm-recon.md:132` −13 B、`playbook-entity-cluster-tiering.md:149` +9 B。APU 0801 对照：改前闸返回 `[]`（子目录产物被绕过），改后报 needs 哈希未登记；该案再发布前须重跑 peaks_daily＋`--only-addrs` 补算。闸不验 followup `channels` 与全量输入同源，登记 P13 另单。
- **工艺**：每段工单先 codex 只读复核（A 四轮、B 两轮、C 三轮＋C7 两轮、D 四轮），通过后 codex 写模式施工，Fable 本机验收（白名单 diff、定向测试、字节、存量案对照、reseal 同步 worktree）后 commit，再 codex 常规盲审：A r1 FAIL（打包偏差）→r2 PASS、B r1 PASS、C r1 FAIL（1 minor 类型校验）→C7→r2 PASS、D r1 PASS。只读复核与盲审并行派、施工串行（用户 09-17 定）。
- **字节与测试**：references 930070→930061（−9 B），SKILL.md 8021、commands-staging 8798 不变；invariant_manifest 登记两 producer/consumer schema 与两处 overwrite_single，minimum_counts 下限抬至实际计数（81/118/61）；本机 run_all 151/151（含 reseal 21/21，验收 worktree 同步 HEAD 后）；九项守卫全 PASS。登记不修台账 `maintenance/repair-20260917-p0-four/code_change_pending.md` P1–P13（含 P2 预筛 0.1% vs 1%、P3 APU trigger_days 键、P4 override 数学正确性、P7 `load_json` 1e999、P10 上限来源、P12 producer sha 只记录）。
- **档位说明**：按 2026-09-17 批准计划记 7.2.0（新入口＋持久化契约扩展）；codex 复核 E-01 持异议：facts.json 进 `NEW_ANALYSIS_REQUIRED` 且无 provenance 绑定块的旧 facts 必红，属不兼容契约变更，应记 8.0.0。旧案在 HEAD 下 BLOCK 由钉版机制承接（`~/.claude/skill-pins/`）。**待用户追认或改判。**
- **成本-质量指标**：生产逻辑文件 6（figures_from_facts、audit_release_gate、facts_gate、stage2_closeout、peaks_daily、replay_duck）、新公开入口 2（1 子命令＋1 选项）、新持久化 schema 2、新增 SUITE 入口 0；外部网络调用 0；不运行真实案卷判断链，判断结论不自动变更；存量案 APU 0801 再发布前须补 `state_source.facts_inputs` 重 build 与峰值补算。
