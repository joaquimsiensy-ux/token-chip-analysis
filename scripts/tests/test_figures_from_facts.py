...
def check_sync_entry_contract(td):
    """Keep the existing direct-call API and reject malformed provenance."""
    sys.path.insert(0, REPORT_DIR)
    import standard_charts

    root = Path(td).resolve() / "sync-entry-contract"
    root.mkdir()
    series = {"ts": [datetime(2026, 1, 1), datetime(2026, 1, 2)],
              "项目方": [60.0, 50.0], "散户": [40.0, 50.0],
              "锁仓/销毁": [5.0, 7.0]}
    positional, keyword = root / "positional.png", root / "keyword.png"
    standard_charts.plot_camp_evolution(
        series, str(positional), "TEST", "占净供应量", None, None, "sol-rows")
    standard_charts.plot_camp_evolution(
        series, str(keyword), "TEST", "占净供应量", series_format="sol-rows")
    assert positional.stat().st_size > 10000
    assert sha256(positional) == sha256(keyword), "Both supported calls must render identically"

    for idx, malformed in enumerate((["broken"], "broken", 123, True)):
        case = root / f"malformed-{idx}"
        case.mkdir()
        state = deepcopy(STATE)
        state["provenance"] = malformed
        state_path, png = case / "analysis-state.json", case / "fig1.png"
        state_path.write_text(json.dumps(state, ensure_ascii=False))
        result = run(["fig1", "--state", str(state_path), "--out", str(png)])
        assert result.returncode != 0 and "provenance 非对象" in result.stderr, result
        assert not png.exists() and not (case / LEGEND_RECEIPT).exists()

    for state in ({}, {"provenance": None}, {"provenance": {}},
                  {"provenance": []}, {"provenance": ""},
                  {"provenance": 0}, {"provenance": False}):
        assert standard_charts.fig1_series_format(state) is None


def main():
    with tempfile.TemporaryDirectory() as td:
        check_producer_stack_contract(td)
        check_sync_entry_contract(td)
