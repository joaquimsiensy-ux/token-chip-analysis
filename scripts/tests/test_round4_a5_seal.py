...
def main():
    with test_vertical_slices():
        ...
        with tempfile.TemporaryDirectory() as td:
            d = Path(td)
            (d / "charts/final").mkdir(parents=True)
            (d / "a4_seal.json").write_text(json.dumps({
            ...
            seal = d / "a5_report_seal.json"
            create_seal(d, report, d / "a4_seal.json", seal)
            assert not validate_seal(seal, report, d / "a4_seal.json")
            original = json.loads(seal.read_text())
            old = dict(original)
            old["schema"] = "a5-report-seal/v1"
            seal.write_text(json.dumps(old))
            assert any("schema" in x for x in validate_seal(
                seal, report, d / "a4_seal.json"))
            seal.write_text(json.dumps(original))
            a4_original = json.loads((d / "a4_seal.json").read_text())
            a4_old = dict(a4_original)
            a4_old["schema"] = "a4-seal/v3"
            (d / "a4_seal.json").write_text(json.dumps(a4_old))
            assert any("a4-seal/v4" in x or "A4" in x for x in validate_seal(
                seal, report, d / "a4_seal.json"))
            (d / "a4_seal.json").write_text(json.dumps(a4_original))
            report.write_text("swapped")
            assert any("Markdown" in x for x in validate_seal(
                seal, report, d / "a4_seal.json"))
            report.write_text("# R\n![x](charts/final/x.png)\n")
            (d / "charts/final/x.png").write_bytes(b"tamper")
            assert any("图" in x for x in validate_seal(
                seal, report, d / "a4_seal.json"))
