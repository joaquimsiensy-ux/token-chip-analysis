#!/usr/bin/env python3
"""全周期流转路径图（lifecycle flow）——重点实体必配图（v5.0 门槛：当前持仓
≥20% 总供应或 ≥20% 流通的大庄/项目方；v2.0.0 起为标准图）。
...
- 每个达门槛实体（≥20% 的大庄/项目方）必配一张，覆盖全生命周期：
...
NODE_STYLES = {
    ...
    "sniper":  dict(fc="#e9def2", ec="#7d5a9e"),   # 发射窗协同（legacy，"狙击集团"标签 v5.0 废止）
