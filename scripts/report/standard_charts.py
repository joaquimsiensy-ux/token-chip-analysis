...
def fig1_exclusion_reasons(series_format=None):
    """Keep the Codex helper name while using the shared producer contract."""
    return fig1_excluded_series(series_format)
...
def fig1_series_format(state) -> str | None:
    ...
    if not isinstance(prov, dict):
        raise camp_series_provenance.SeriesProvenanceError("provenance 非对象")
    if "camp_series_sidecar" not in prov:
        return None
