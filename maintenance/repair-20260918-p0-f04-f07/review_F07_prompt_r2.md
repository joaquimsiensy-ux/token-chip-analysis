# 工单 F07 复核提示词（只读，r2：对 v2 复核；r1 退回项见 review_F07_reply_r1.md，本轮重点核 v2 对每条退回的处置是否闭合、以及 v2 新增内容有无引入新问题；r1 已判通过的项只需确认 v2 未改坏）
...
2. 只读、离线、不 commit、不改任何文件。报告全文打印到 stdout，首行固定为 `# 工单F07复核：通过` 或 `# 工单F07复核：退回`。退回时逐条给出：编号（F07-R2-NN）、工单位置、事实（附 `grep -n -F` 或代码原文）、修订建议。通过时也要列出你实际核过的项。
3. 这是对**修复计划**的复核，不是攻击式验收：目标是拦住工单里"想得不周"的地方——锚点错、修法有副作用、既有测试会变红、同族入口漏了、迁移代价没说清。

## 任务
复核 `maintenance/repair-20260918-p0-f04-f07/workorder_F07.md`（v2）。工作目录＝本仓库根（HEAD 含本工程工单 commit；`scripts/` 与 311e6c4 逐字节相同，行号按 311e6c4 核）。逐项核：
...
g) **r2 专项**：①`PEAKS_DAILY_PRODUCTS` 去掉 `trigger_days.json` 后，"改名 summary 但 needs 在场"仍被拒、"原始 trigger_days.json 在 data/"不再误判——用例 14/20/21/22 是否覆盖；单次 `rglob("*.json")` 与原四次 rglob 的过滤等价性；②`channels` basename rglob 恰 1：调度方实核 APU 0914 案 `channels.json` 在案根唯一、另有 `channels_preflight.json` 不同名——确认 `_find_peaks_dirs` 过滤规则同样用于 channels 查找时不会命中 `.duck_tmp`/`_history` 下副本；③§1.4 双 `--only-addrs` 说明与 `replay_duck.py:334-368/:411` 一致；④用例 21/22 的断言写法是否会被其他既有错误文案误伤（列出 `build_case(historical=False)` 正例在 `gate.run` 下的已知错误集合，应为空）。
