# 工单 E（v2，融合 codex 复核 r1 五条 E-R1-01…05）：版本落地 7.2.1 —— repair-20260918-p0-f04-f07 收官（四条主轨 P0：F06 exclude 标签重推 INFRA / F04 图 2 收据发布期重算 / F07 峰值产物定位＋followup 绑定 / F05 facts 峰值-日期-decimals 校验）

> 性质：**纯文档/元数据工单**，零生产代码、零测试代码、零手册改动。四段均为既定契约内的闸加固（消费侧从"信收据/字段自报"改为"按实物重算或绑定"），无新公开入口、无新持久化 schema，**档位＝7.2.1（修）**。存量迁移代价写进条目"档位与迁移说明"。
> v2 变更（codex r1 五条，见 `review_E_reply_r1.md`）：E-R1-01 日期上界写明生效条件；E-R1-02 stage2 联动范围改准；E-R1-03 图 2 手写收据表述收窄；E-R1-04 followup 迁移按实际条件不按版本截断、override 迁移补后续步骤；E-R1-05 工艺叙述改为"按意见修订后施工"。codex 档位意见（倾向 8.0.0，F05 证据契约收紧）写进条目"档位与迁移说明"待用户追认或改判，不阻断本单。
