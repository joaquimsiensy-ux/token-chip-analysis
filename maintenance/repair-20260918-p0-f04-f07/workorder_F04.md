# 工单 F04（v1）：new-analysis 发布闸对图 2 收据做发布期重算 —— repair-20260918-p0-f04-f07 第二段
...
- 0.8 不跑 `run_all.py`。定向跑：`python3 -B scripts/tests/test_repair_batch_c.py`、`test_figures_from_facts.py`、`test_audit_release_gate.py`、`test_a4_gate.py`、`test_repair_batch_d.py`、`test_repair_g1_cross_target.py`、`test_review_20260804_p105.py`、`test_stage2_closeout.py`（`dry_run_touches_nothing` 依赖外部 worktree 属已知 F12，仅此项失败注明）、`python3 -B scripts/tests/invariant_scan.py`。除注明项外全 PASS。
...
3. `F04 真跑 check 同源收据仍放行（GREEN→GREEN）`：ws pct `[27.8]`；`run([sys.executable, str(fff), "check", "--facts", ..., "--series", ...], cwd=td)` rc 0；读 `figure2_check_receipt.json` 喂消费者 → `errs == []`。
4. `F04 输入缺席不重算不重复报错`：ws 不写；手写收据 series.path 指 `ws.json`（不存在）→ `errs` 恰 1 条且含"不在案根"，且无"重算"字样。GREEN→GREEN（验 §1.4）。

**既有用例 `_r08_case_12` 须同步订正**（它在 `:1440-1452` 之后断言"基线消费者接受手写 PASS 收据"作为 RED 前提；F04 落地后该断言反转）：把该处"消费者接受（`errs == []`）"的 `check(...)` 改为断言 `any("重算失败" in x for x in errs)`，用例名文案对应改为"消费者对 NaN 手写收据直接拒（F04）"；后半段（跑 check → 收据 FAIL → 消费者报"非 PASS"）保持。开工先 `nl -ba` 读 `:1452-1465` 确认该断言的准确行与文案，写进 done。

RED 证据：改生产代码前逐个调用 `_f04_case_1..4` 与订正后的 `_r08_case_12`，记录 1/2 的 AssertionError 原文（含命令与被测文件 sha256）。
...
- 发布闸 import `figures_from_facts` 连带 matplotlib：一次性成本；若日后要拆，把 `fig2_check_errors`/`_pct_value_ok`/`_load` 迁到无绘图依赖模块属另单。
- `stage2_closeout` 强检查未成必经：本段以重算闭合"错误数据放行"，closeout 必经化属 7.1.0 遗留清单。
- 收据 schema 不升版：重算使旧 PASS 自然失效，不需要 v2。
