# 施工 F06 提示词：按 workorder_F06.md 最新版施工（写模式）
# 派工基线 HEAD＝ad93d95
...
2. 工单全文＝`maintenance/repair-20260918-p0-f04-f07/workorder_F06.md`（以文件头版本号为准），逐字执行其 §0–§3。§0.1 的基线校验命令须实际运行并把输出贴进 `F06_done.md`。
3. 只改工单 §0.3 白名单文件；只动工单指定的片段；锚不唯一或行号不符即停工汇报（不猜、不扩）。工单里标注"开工核实"的项先核实并把结果写进 done。
4. 先红后绿：新用例先在改动前跑取 RED 证据写 `F06_red_evidence.txt`，再改生产代码。
...
7. 中途遇到工单与代码事实冲突、或需要动白名单外文件：停工写 `F06_done_attemptN_stopped.md` 并在 stdout 说明，不自行扩权。
8. 完成后：把 §3 要求的完成报告写到 `maintenance/repair-20260918-p0-f04-f07/F06_done.md`，并在 stdout 首行打印 `# 施工 F06：完成` 或 `# 施工 F06：停工`，随后打印 `git diff --stat` 与 §1.1 三个字节数。
