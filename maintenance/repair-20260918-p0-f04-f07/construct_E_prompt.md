...
2. 工单全文＝`maintenance/repair-20260918-p0-f04-f07/workorder_E_version.md`（以文件头版本号为准），逐字执行 §0–§4。开工先跑 `git status --short`（须为空）与 `git rev-parse --short HEAD`（须为首行标注多恰一个提交），贴进 `E_done.md`。
...
5. 完成后：报告写 `maintenance/repair-20260918-p0-f04-f07/E_done.md`，stdout 首行 `# 施工 E：完成` 或 `# 施工 E：停工`，随后打印 `git diff --stat`。
