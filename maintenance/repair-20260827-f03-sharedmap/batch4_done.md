...
- 基线保持 `HEAD=main=cdc4f87f8e3ee4d181760cb8455d688f23049f20`；未执行任何 git 写操作。
- `scripts/lib/producer_history.py` 已紧跟现有 `be415db3…` 两条之后登记 `c4980c98…` 的 coverage/pointer 双协议 `ACTIVE` producer。
- `VERSION`、`pyproject.toml`、`SKILL.md` 已同步到 `6.52.15`；`CHANGELOG.md` 已增加活跃索引一行和归档详述一段。
- 定向准入守卫全部通过；全量 `run_all.py` 实际为 `137 PASS / 2 FAIL`、退出码 1。两项失败均在业务断言前因沙箱禁止绑定 `127.0.0.1` 而退出，不能记作全绿。

## Diff 摘要

- `scripts/lib/producer_history.py`：新增 2 条六字段 producer 登记，协议分别为 `sqd-solana-coverage/v1` 与 `sqd-solana-coverage-pointer/v1`。
- `VERSION`、`pyproject.toml`、`SKILL.md`：`6.52.14` → `6.52.15`。
- `CHANGELOG.md`：新增 6.52.15 活跃索引和归档段，覆盖 live 根因、失败三分级、分段覆盖声明、杀变异守卫、审计字段、盲审与 suite 数量。
- `maintenance/repair-20260827-f03-sharedmap/batch4_done.md`：本报告。
- 未修改任何代码逻辑、测试、契约或资产。

报告写入前的 tracked diff stat：
...
~~~text
 ...
 VERSION                         |  2 +-
 pyproject.toml                  |  2 +-
 scripts/lib/producer_history.py | 16 ++++++++++++++++
...
~~~text
$ git rev-parse HEAD
cdc4f87f8e3ee4d181760cb8455d688f23049f20

$ git show cdc4f87f8e3ee4d181760cb8455d688f23049f20:scripts/solana/sqd_coverage_probe.py | shasum -a 256
c4980c984b08d27f5a7e46db50f97c9c16e47ea491f37a459b3773f939218769  -

$ PYTHONDONTWRITEBYTECODE=1 python3 -c '<producer_history exact-row assertion>'
PASS producer_history exact rows: 2 protocols, ACTIVE, six-field shape

$ PYTHONDONTWRITEBYTECODE=1 python3 scripts/tests/changelog_lint.py
PASS: 版本号唯一（豁免 2 组历史撞号存档）、顺序正确；活跃 57 条 + 归档 139 条

$ PYTHONDONTWRITEBYTECODE=1 python3 scripts/tests/test_version_consistency.py
PASS: M-03 version metadata consistent at 6.52.15

$ PYTHONDONTWRITEBYTECODE=1 python3 scripts/tests/test_anchor_plan_v3.py
PASS test_01_v3_full_positive
PASS test_02_bad_source_enum_rejected
PASS test_03_final_source_in_matrix_rejected
PASS test_04_final_source_forbidden_keys_rejected
PASS test_05_day_end_block_shape_rejected
PASS test_06_tx_with_balance_source_rejected
PASS test_07_balance_without_source_rejected
PASS test_08_kind_text_is_not_semantic
PASS test_09_v2_fixture_full_compatibility
PASS test_10_v2_projection_asserts_before_stripping
PASS test_11_strict_xor_rejections
PASS test_12_producer_history_and_default_boundary
PASS test_13_duplicate_keys_rejected_on_both_authority_paths
PASS test_14_v3_rejects_v2_historical_producer_hash
PASS test_15_schema_dispatch_v2_field_and_enum_type_fail_closed
anchor-plan v3: 15/15 PASS

$ PYTHONDONTWRITEBYTECODE=1 python3 scripts/tests/test_sqd_consumer_v4.py
...
- 无新增生产语义问题。
- 全量中的两项 localhost bind `PermissionError` 是既有沙箱环境限制；它们发生在业务断言前，见下方完整原始输出。
- 既存未跟踪输入 `maintenance/repair-20260827-f03-sharedmap/workorder_batch4.md` 未改。
...
========================================================
      PASS  changelog_lint.py        PASS: 版本号唯一（豁免 2 组历史撞号存档）、顺序正确；活跃 57 条 + 归档 139 条
...
FAIL(rc=1)  test_batch3_evm_vertical_slice.py (无输出)
      ...
      PASS  test_version_consistency.py PASS: M-03 version metadata consistent at 6.52.15
