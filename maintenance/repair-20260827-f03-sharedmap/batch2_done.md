...
- 开工 `HEAD=f0469a376c0101759f260dafb1678c00ff785d65`，与工单指定基线逐字一致。
- 本批未执行 `add/commit/checkout/rebase/merge/push` 或其他 git 写操作，也未联网。
- 只修改登记面、版本面、CHANGELOG，并新增本报告；代码、测试逻辑、契约和资产均零改动。
- 开工前工作树唯一脏项为未跟踪的工单输入 `maintenance/repair-20260827-f03-sharedmap/workorder_batch2.md`；本批未修改、移动或删除它。

## Diff 摘要

- `scripts/lib/producer_history.py`：紧跟既有两条 `bccf1802…` 登记之后，为提交 `f0469a376c0101759f260dafb1678c00ff785d65` 中 SHA-256 为 `be415d…7bea` 的冻结探针新增两条 `ACTIVE` 登记，分别对应 `sqd-solana-coverage/v1` 与 `sqd-solana-coverage-pointer/v1`；旧登记零改动。
- `VERSION`、`pyproject.toml`、`SKILL.md`：三处版本由 `6.52.13` 同步升为 `6.52.14`。
- `CHANGELOG.md`：新增 6.52.14 顶部活跃索引与正文归档段，覆盖 F-03 出处、身份/重验/回退闭环、两消费 validator、D1 裁决、CT-SQDGAP-35、资产零改动复用、两轮盲审和 SUITE 138→139。
- `maintenance/repair-20260827-f03-sharedmap/batch2_done.md`：本报告。
...
```text
git show f0469a376c0101759f260dafb1678c00ff785d65:scripts/solana/sqd_coverage_probe.py | shasum -a 256
```
...
与工单指定 SHA-256 逐字节一致。producer 全表的 git 可复算守卫位于 `test_anchor_plan_v3.py::test_12_producer_history_and_default_boundary`，定向运行 15/15 PASS；collector 全表的同类守卫 `test_collector_history.py` 也在定向与全量运行中通过。

## 定向守卫

- `changelog_lint.py`：PASS；活跃 56 条、归档 139 条。
- `test_version_consistency.py`：PASS；三版本面一致为 6.52.14。
- `test_anchor_plan_v3.py`：15/15 PASS；producer_history 结构、状态和默认边界通过。
- `test_collector_history.py`：PASS；每条登记均可由 git 复算。
- `test_sqd_consumer_v4.py`：PASS。
- `test_f03_sharedmap_reuse.py`：9/9 PASS。
- `git diff --check`：退出码 0，无输出。

## 工单外发现清单

1. 全量仍有两项沙箱环境失败：Solana/EVM R9 vertical slice 都在 `ThreadingHTTPServer(("127.0.0.1", 0), ...)` 绑定时得到 `PermissionError: [Errno 1] Operation not permitted`。两项都发生在业务断言前，与本批登记/版本/CHANGELOG 改动无调用关系；按白名单纪律只记录，不修。
2. 开工前已有未跟踪 `maintenance/repair-20260827-f03-sharedmap/workorder_batch2.md`。它是本批权威工单输入但不在施工白名单，本批保持零改动；因此最终 `git status` 会如实显示该既有输入，不能把它冒充为本批新改动，也不能越权删除。
3. 当前沙箱禁止 `ps/pgrep` 读取进程表；等待全量测试期间只读诊断得到 `operation not permitted`。该限制不影响 runner 自然完成及其最终输出。
4. 未发现其他新的工单外代码问题。
...
退出码：`1`。Suite size：139。结果：137 PASS / 2 FAIL；两项均为上述 localhost bind 沙箱限制。
