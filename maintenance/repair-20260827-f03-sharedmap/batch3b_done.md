...
- 两项点名发现已消化；基线保持 HEAD=main=9b1c4b5c3a72afc2787473cf684a14db03e4ace2，未执行任何 git 写操作。
- 发现 1 已修：retry mismatch 仍触发整体回退，但不再进入 unverified_ranges，也不再增加 recheck_stats.unverified。
- 发现 2 仅加测试、未改其生产防线：部分复用与纯 full 两个端到端用例都断言案区间外/跨界 recheck 行不得声明 coverage，scan_ranges 全部被限制在案区间内。
- 定向结果：F-03 shared-map reuse 15/15；SQD coverage probe 12/12。
- 本机 run_all.py 原样结果为 137 PASS / 2 FAIL，退出码 1；两项失败仍是在业务断言前因沙箱禁止绑定 127.0.0.1 而退出，不能记作全绿。

## RED 证据

### 发现 1：批 3 现态真实 RED

先只扩展 test_retry_rescues_range_and_retry_mismatch_falls_back，未改生产代码。命令退出码 1：

~~~text
Traceback (most recent call last):
  File "<string>", line 1, in <module>
    import runpy; ns=runpy.run_path("scripts/tests/test_f03_sharedmap_reuse.py"); ns["test_retry_rescues_range_and_retry_mismatch_falls_back"]()
                                                                                  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^^
  File "scripts/tests/test_f03_sharedmap_reuse.py", line 400, in test_retry_rescues_range_and_retry_mismatch_falls_back
    assert info["unverified_ranges"] == []
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
AssertionError
~~~

### 发现 2：受控 mutation RED

本发现的工单前提是生产代码方向正确，因此新增行为断言在真实批 3 代码上应直接 GREEN，无法诚实制造“真实生产现态 RED”。为证明回归测试能锁住防线，测试进程内模拟删除案外 recheck 降级效果：把成功且越出案区间的 recheck 行恢复为 counts_coverage=true，不改磁盘上的生产代码。两个点名端到端用例均按预期 RED，命令退出码 1：

~~~text
EXPECTED RED mutation: test_partial_rate_limit_end_to_end_full_repairs_failed_ranges
AssertionError: outside recheck rows included cross-boundary 100..163 and fully-outside 180..180 with counts_coverage=true
EXPECTED RED mutation: test_exclusion_failure_falls_back_and_case_can_degrade_to_pure_full
AssertionError: fully-outside successful recheck rows had counts_coverage=true
~~~

真实批 3 生产代码上的新增边界断言随后直接通过：

~~~text
{"probe_id": "adbca6969b85b1bd", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
{"probe_id": "ae8a7b737c2bf2f3", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
PASS case-bounded recheck assertions: partial reuse + pure full
~~~

## 修复与契约核对

- scripts/solana/sqd_coverage_probe.py：仅在重试分类循环把兜底 else 收紧为 elif outcome == "request-failed"。retry mismatch 只进入 mismatch_slots，最终仍以 recheck-mismatch:<slot> 整体回退。
- scan-schemas 的逐字段语义保持一致：
  - verified：最终得到完整响应且逐 slot 等于资产的连续请求区间数；
  - unverified：首轮 request-failed 且末尾统一重试仍 request-failed 的连续请求区间数；
  - retried：首轮 request-failed 后进入末尾重试的连续请求区间数。
- retry mismatch 夹具的最终统计为 verified=2、unverified=0、retried=1；unverified_ranges=[]。
- 部分复用夹具把案件收窄为 120..175：成功 recheck 100..163 跨下界，180..180 完全案外；两者发布账本 counts_coverage=false，scan_ranges 无越界条目。
- 纯 full 夹具案件为 170..171：案外成功 recheck 行全部 counts_coverage=false，scan_ranges 无越界条目。

## GREEN 证据

~~~text
PYTHONDONTWRITEBYTECODE=1 python3 -c 'import runpy; ns=runpy.run_path("scripts/tests/test_f03_sharedmap_reuse.py"); ns["test_retry_rescues_range_and_retry_mismatch_falls_back"](); print("PASS retry rescue and retry mismatch classification")'
PASS retry rescue and retry mismatch classification

PYTHONDONTWRITEBYTECODE=1 python3 -c 'import runpy; ns=runpy.run_path("scripts/tests/test_f03_sharedmap_reuse.py"); ns["test_partial_rate_limit_end_to_end_full_repairs_failed_ranges"](); ns["test_exclusion_failure_falls_back_and_case_can_degrade_to_pure_full"](); print("PASS case-bounded recheck assertions: partial reuse + pure full")'
{"probe_id": "b67b8a8fa63f0a74", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
{"probe_id": "cc7c189fa87d31a2", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
PASS case-bounded recheck assertions: partial reuse + pure full

PYTHONDONTWRITEBYTECODE=1 python3 scripts/tests/test_f03_sharedmap_reuse.py
{"probe_id": "7e316d8d8bcc3c36", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
{"probe_id": "563f99f1fde4f42c", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
{"probe_id": "41732b8536ac74ee", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
{"probe_id": "f9053d75def3139f", "status": "published", "verdict": "NO_KNOWN_NONCE_OMISSION_DETECTED"}
PASS F-03 shared-map reuse: 15/15 groups

PYTHONDONTWRITEBYTECODE=1 python3 scripts/tests/test_sqd_coverage_probe.py
...
批 3 工单“全部 recheck 段 unverified”按“案内全部可复用区间均被 non-canary unverified 覆盖”解释执行；该解释已获裁判方追认。canary 所在区间重试仍 request-failed 时仍必须整体回退，不与该解释冲突。

## Diff 摘要与白名单

批 3b 增量：

- scripts/solana/sqd_coverage_probe.py：重试分类段 1 行语义收紧。
- scripts/tests/test_f03_sharedmap_reuse.py：retry mismatch 增加完整审计断言；两个点名端到端夹具增加案外/跨界账本与 scan_ranges 断言，净增 38 行。
- maintenance/repair-20260827-f03-sharedmap/batch3b_done.md：本报告。

当前相对 main 的整棵未提交树仍包含批 3 既有改动，原样 stat：

~~~text
 assets/sqd-solana-coverage-map/README.md  |   2 +-
 references/scan-schemas.md                |   9 +-
 scripts/solana/sqd_coverage_probe.py      | 148 +++++++++---
 scripts/tests/test_f03_sharedmap_reuse.py | 366 +++++++++++++++++++++++++++++-
 4 files changed, 484 insertions(+), 41 deletions(-)
~~~

批 3b 未触碰批 3 既有 README、scan-schemas、batch3_done.md、batch3_red_evidence.md、两份 workorder；未触碰 validator、契约/快照、VERSION/CHANGELOG/pyproject/SKILL、producer_history 或冻结资产。

## 工单外发现

- 无新增生产语义问题。
- 全量中的两项 localhost bind PermissionError 是既有沙箱环境限制，详见下方原始输出。
- 附加展示 validator 结果时，第一次直接 JSON 序列化其 bytes 型 counts 触发 TypeError；改为只打印 ok/reasons 后通过。这是取证命令展示错误，不是 validator 或资产缺陷。

## run_all.py 最终一次完整原始输出
