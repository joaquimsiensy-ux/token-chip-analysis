# 工单 F01（v1）：主价格文件非有限值 fail-closed、收据序列化拒 NaN、−2 收口逐点重算价格状态 —— repair-20260918d-p1-f01-f04 第二段
...
> 修法：①生产者 `_load_series` 解析后任一点非有限 → `[fatal]` 退出 1（主价格文件是报告图 1 正式输入，含 NaN 本身就是坏文件，抽查前先清洗；存量案卷价格文件实证 0 命中，台账 Q4）；②`:175` 第二源非有限按 SKIP（对照不可得，台账 Q1）；③`:199` `allow_nan=False`（收据层纵深防御）；④closeout `price_receipt_errors` 逐点用 `main_price/second_price` 按 `price_check.py:175-180` 同规则重算 status 并要求与声明一致，主价非有限正数直接拒；阈值常量复制自 `price_check.py:43`、不 import（台账 Q2）。
...
- 0.4 **不改**：`price_check.py` 的 `_daily_close`（含 CSV 日期截断取值口径，上轮用户裁决不修）、`second_llama`/`second_binance`/`_get`、`--second` 选源与同源提示（review F05 本轮不修）、阈值数值、既有退出码；`stage2_closeout.py` 的 `PRICE_POINT_STATUSES`、汇总 verdict 规则（`:269-277`）、`price_file_sha256` 绑定（`:281-287`）、`workorder_errors` 其余全部、`load()`（`:62-63`，`json.loads` 默认解析 NaN 字面量为 float，正好让 closeout 读到坏收据后拒）；`scripts/tests/test_stage2_reseal.py`；任何 `references/`、`SKILL.md`、`commands-staging/`、`VERSION`、`pyproject.toml`、`CHANGELOG.md`、`contract_manifest.json`、`invariant_manifest.json`（开工用 `python3 -B scripts/tests/invariant_scan.py` 证实无需登记）。
- 0.5 行号均指施工前基线（868d3f61）；锚 `grep -n -F` 恰 1 处且行号一致，不符**停工**。删除 > 修改 > 新增。
- 0.6 离线；不 commit、不 push、不部署；禁 stash/checkout/reset。
- 0.7 先红后绿：§2.3 三段新用例在改生产代码前**逐段独立执行**取 RED 写 `F01_red_evidence.txt`（每段各自 try/except，段 6b 失败不得截断 6c/6d 取证）。
...
- `:18`（锚 `  时间戳 >1e12 自动判毫秒。`，唯一）改为 `  时间戳 >1e12 自动判毫秒；任一点非有限（NaN/inf）即 [fatal] 退出 1，先清洗主价格文件再抽查（F01）。`
- `:84`（锚 `    out = [(t // 1000 if t > 10 ** 12 else t, p) for t, p in out]`，唯一）之后插入：

```python
    bad = [t for t, p in out if not math.isfinite(p)]
    if bad:
        sys.exit(f"[fatal] 价格文件含非有限值（NaN/inf）{len(bad)} 点（首个 ts={bad[0]}）：主价格文件先清洗再抽查")
```

- `:175`（锚 `        if p2 is None or p2 <= 0 or p1 <= 0:`，唯一）改为 `        if p2 is None or not math.isfinite(p2) or p2 <= 0 or p1 <= 0:`
- `:199`（锚 `            json.dump(out, f, ensure_ascii=False, indent=1)`，唯一）改为 `            json.dump(out, f, ensure_ascii=False, indent=1, allow_nan=False)`

说明：①CSV 路径 `float("NaN")`/`float("inf")`/`float("-inf")` 与 JSON 路径 `NaN`/`Infinity`/`-Infinity` 字面量（`json.load` 默认接受）都落到 `:84` 之后的检查；`sys.exit(str)` 退出码 1，与 `:61/:66/:83` 同款；②`p1` 经 ①保证有限，`:175` 只需补 `p2`；③`allow_nan=False` 在 ①②之后只剩不可能路径，属纵深防御（若触发则 `ValueError` 退出 1，可接受）。
...
说明：①`round(…, 2)` 与 `price_check.py:179` 逐字同规则，边界 5.00/15.00 两侧判定一致；②非 dict 元素由既有 `statuses` None→FAIL 汇总拒，本块跳过；③主价非有限时 `continue`，该点自报 status 仍进汇总——最多多出一条汇总诊断，不矛盾；④`p2 is None` 与非有限/≤0 同落 SKIP，与 `price_check.py:175`（改后）一致；⑤block 内只用既有 `errors`/`workorder_error`，返回值不变。

### 2.3 `scripts/tests/test_stage2_closeout.py` —— 三段新用例（改既有函数，不新增函数）

`:659`（锚 `    # 7 内联纯申报对象拒；内联带 receipt（ARC 形态）放行`，唯一）之前插入：

```python
    # 6b F01：主价格文件含 NaN → 生产者 [fatal] 退出 1、不写收据
    write(case / "price_nan.json", [[1767225600, float("nan")], [1767312000, 1.0], [1767398400, 1.0]])
    assert write_price_receipt(case, second_price=1.0, prices="price_nan.json", out="price_nan_checks.json") == 1
    assert not (case / "price_nan_checks.json").exists()
    # 6c F01：收据主价被改成 NaN（status 仍 PASS）→ 消费者拒"有限正数"
    ...
    # 6d F01：收据第二价被改成 2.0（偏差 66.67%）但 status 仍 PASS → 逐点重算不一致
    assert write_price_receipt(case, second_price=1.0) == 0
...
说明：①`write()`（`:28-29`）用 `json.dumps` 默认 `allow_nan=True`，写出 `NaN` 字面量；`update()`（`:159-162`）读改写同款；②6b 用独立文件名 `price_nan.json`/`price_nan_checks.json`，不污染主夹具与工单绑定；`_load_series` 在 `price_check.py:157` 早于任何写盘；③插在段 6（ALL_SKIP）之后、段 7 之前：此时顶层 `price_source_checks` 仍在场，`rebind()` 可用；6c/6d 各自先重写 PASS 收据；段 7 起始 `:660` 亦重写收据，不受影响；④`rebind()`/`update()`/`sha()`/`write_price_receipt()` 均为既有 helper。

RED 证据（改生产代码前逐段独立执行，写 `F01_red_evidence.txt`）：6b 基线返回 0 且收据落盘（RED）；6c 基线 errors 为空（RED）；6d 基线 errors 为空（RED）。每段记录返回码/errors 原文。
...
- 第二源非有限按 SKIP（Q1）；阈值常量复制不 import（Q2）；review F05 同源双源不修（Q3）；存量代价 0 命中、APU 暂不重跑（Q4）。
