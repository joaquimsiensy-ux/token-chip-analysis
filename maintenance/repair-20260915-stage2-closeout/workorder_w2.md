...
2. **只新建一个脚本** `scripts/report/stage2_closeout.py`。其余改动全部落在现有文件：`audit_release_gate.py`、`figures_from_facts.py`、`a4_gate.py`、`build_html.py`、`scripts/tests/run_all.py`；新建测试 `scripts/tests/test_stage2_closeout.py`。
...
6. 现有测试 `test_a4_gate.py:608/:638` 断言 build_html 输出含 `封口后被改动`；`test_audit_release_gate.py`、`test_distribution_gate.py`、`test_round4_a5_seal.py`、W1 新增的 `test_reopen_cycle.py`/`test_a4_limits_extract.py`——改完全部必须仍 PASS。全套 `python3 scripts/tests/run_all.py` 约 11 分钟，`nohup … > /tmp/run_all_w2.log 2>&1 &` 落盘后读回真实结果，不得凭部分输出宣称 PASS。
...
2. 新子命令 `fig2-series`：`--entity-series <案内 entity_series.json> --keys TE-01,TE-02 --labels-from facts.json --out whale_series.json`。输入格式 `{dates:[YYYY-MM-DD…], <key>:[pct…]}`（APU 0914 与 BTW `data/replay/entity_series.json` 同款）。输出裸数组 `[{entity_id, label, ts, pct}]`：`label = facts.entities[key].label`（key 不在 entities、series 缺 key、`len(pct)!=len(dates)` → exit 1）；`ts` 保留 `YYYY-MM-DD` 字符串；`pct` 原值。实现为纯函数 `build_fig2_series(entity_series_obj, facts_obj, keys) -> list` ＋ `dumps_fig2_series(lines) -> bytes`（`json.dumps(ensure_ascii=False, sort_keys=True, separators=(",",":"))`，确定性），closeout 用它重放。
   - 同时写 `<out 同目录>/whale_series.provenance.json`：`{schema:"fig2-series-provenance/v1", entity_series:{path,sha256}, facts:{path,sha256}, keys:[…], out:{path,sha256}}`，path 为案根相对路径，无时间戳。写法 tmp+fsync+os.replace（同 `:248-253`）。
...
3. **合并线本版不支持**：`fig2-series` 不提供 merge 参数；工单 `fig2.merge_groups` 在场即 BLOCK（见 E3）。理由：APU/BTW 两案无实例，`check` 末点对账也只认单实体。
...
**E1-9 fig2_series 细则**：固定读案根 `whale_series.json` 与 `whale_series.provenance.json`（不解析工单 `fig2.whale_series` 字段的文字，APU 实况该字段是带说明的串 `whale_series.json（案根，由 entity_series.json 三键转换）`）。①两文件都不在场 → BLOCK"未生成 whale_series：运行 `figures_from_facts.py fig2-series` 生成（历史案迁移办法同此）"；②旁车 schema 须 `fig2-series-provenance/v1`，`entity_series/facts/out` 三项 path 案内、sha 与实物一致；③**重放核验**：用旁车 `entity_series`、`facts`、`keys` 调 `build_fig2_series`＋`dumps_fig2_series`，字节 sha 须等于 `out.sha256`（防手改 whale_series）；④`figures_from_facts.fig2_check_errors(facts, whale_series, DEFAULT_TOL_PP)` 无错；⑤**与工单连线**：whale_series 的 `entity_id` 集合 == 工单 `fig2.lines[].entity_id` 集合，且 `keys` 集合相同（空数组不再"0 条 PASS"，工单 lines 非空而序列空即 BLOCK）。
...
2. 机械字段重算等值：`bindings.report_md.sha256`（amendments 为空时＝报告实物 sha；非空时＝首条 `before_sha256`）；`bindings.a4_seal_sha256`；`bindings.entity_freeze_revision == len(entity_freeze.revisions)+1`（APU 3+1=4）；`bindings.facts/state/rounds/final_distribution_png/final_distribution_scan{path,sha256}`；`bindings.distribution_terminal{status,round_n}` 与 rounds terminal 一致；`report_image_refs` 与 `IMG_RE` 重取的有序清单（保留重复）一致。
3. **amendments 链完整性**：每条必备字段 `before_sha256/after_sha256/trigger/exact_change{before,after}/approved_by`（APU 实况字段）；首条 `before == bindings.report_md.sha256`；逐条 `amendments[i].after_sha256 == amendments[i+1].before_sha256`；末条 `after == 报告实物 sha`。任一断链 BLOCK。
4. fig2：`lines[].entity_id ∈ facts.entities` 键（写展示名 → `WORKORDER BLOCK: fig2.lines[i].entity_id: facts 键 != 展示名`）；必画下限＝facts.entities 中 `label.strip()` 以 `项目方`/`大庄`/`小庄`/`离场庄` 开头的实体（APU {TE-01,TE-02,TE-04}；BTW {e_proj}）；label 缺失/空串 → BLOCK"实体 <id> 无标签，无法判定必画"；`required_entity_ids ⊇ 下限`，`lines` 的 entity_id 集合 ⊇ `required_entity_ids`（允许额外线，BTW 另两条合法）；每条 `series_source{path,key,sha256}`：案内、sha 一致、key 在文件里；`merge_groups` 在场 → BLOCK"本版不支持合并线"。
5. flow：分母两种：总供应 `facts_gate.Facts.total_raw`；流通 `facts.token.circulating_supply_raw`（可选键，缺失则 detail 记 NOTE"未声明流通量，门槛仅按总供应判"，不 BLOCK）。下限＝label 以 `项目方`/`大庄` 开头且 `current_raw/总供应 ≥ 20%` **或** `current_raw/流通 ≥ 20%`（report-template.md:176-178）；`eligible_entity_ids ⊇ 下限`；逐张清单兼容 `flow.charts`（APU）与 `flow.items`（BTW），两键都无 → BLOCK；**图中实体集合 == eligible 集合**（split-run.md:163 双向相等；`eligible=[e_proj], items=[]` 即 BLOCK）；eligible 允许多于下限（承载关键结论的额外图）。APU 实况：下限空、eligible 空、charts 空 → PASS。
...
schema `stage2-closeout/v1`：`{schema, verdict, report_md{path,sha256}, workorder{path, frozen_sha256, report_image_refs_sha256, amendments_count}, facts{sha256}, state{sha256}, entity_freeze_revision, a4_seal_sha256, rounds{sha256, terminal_round_n, status}, whale_series{path,sha256}|null, skill_commit, checks[], pending_for_stage3[], amendment_chain[]}`。
- `workorder.frozen_sha256` ＝ 工单剔除 `stage2_selfcheck`、`amendments`、`report_image_refs` 三键后 `json.dumps(sort_keys=True, ensure_ascii=False, separators=(",",":"))` 的 sha256（−3 唯一获准改动的三处不入冻结哈希）；`report_image_refs_sha256` 单独记；`amendments_count` 单独记。无时间戳。BLOCK 也写收据。不进 `REQUIRED_BY_PROFILE`。
- `--receipt-only`（−3 前置自检）：只复验——report sha（有 amendment_chain 按最后一条 after_sha256）、workorder frozen_sha256＋report_image_refs_sha256＋amendments_count、facts/state/a4_seal/rounds sha、freeze revision、**whale_series sha、skill_commit**（当前 `git rev-parse HEAD` 须等于收据值，否则文案"skill 已换版，重跑 stage2_closeout"）、verdict==PASS。任一不符 exit 2，文案"closeout 收据与现场漂移: <项>，退回 −2 重跑 stage2_closeout"。

### E6 `amend`

−3 每写一条 `amendments[]` 后运行。前置（任一不过 exit 2 不写）：收据在场且 verdict=PASS；**工单未获准字段不变**：当前 `frozen_sha256` == 收据值；`amendments_count` == 收据值＋1；链：新条 `before_sha256` == 收据当前报告 sha（首次对 `report_md.sha256`，之后对 `amendment_chain[-1].after_sha256`），`after_sha256` == 报告实物 sha，必备字段齐。通过后**全量重跑 E1 十一项**（不做子集选择，逻辑最少），全部 PASS 才追加 `amendment_chain[] {before_sha256, after_sha256, verdict}`，更新 `report_md.sha256`（保留原稿锚在工单 `bindings.report_md`）、`report_image_refs_sha256`、`amendments_count`；否则 exit 2 不写。
...
2. **补 facts/state/identity 准备**（`build_case` 不产这些，原在 `test_a4_gate.main:370-389`；`v_ok.json` 在 `:412-414`）：从该处复制写法，但实体 label 改为 `"大庄#1"`、`current_raw="300"`、`peak_raw="300"`（总供应 1000 → 30% ≥ 20%，供 flow 正例）；state `whale_groups[0].label` 同步；identity `state_sha256` 按 `add_camp_series` `:272-275` 的重绑法算。若 facts_gate 因 facts↔state/balances 不一致报错，按报错调整夹具数值，**不改闸**。
...
4. 写 `entity_series.json {dates:[…], "e1":[…最后一点=30.0]}` → `fig2-series --keys e1` 产 whale_series＋旁车；写最小 `flow_e1.json` spec（可复用 `test_a4_gate` 现有 flow 夹具写法，若无则最小合法 spec）；`fill-workorder` 生成骨架后补 fig2（lines=[e1]，required=[e1]）、flow（eligible=[e1]，charts=[{entity_id:e1, spec{path,sha256}, out:"charts/final/flow_e1.png"}]）、fig1/fig3 最小合法值。
5. 报告：在终态图引用行后追加按 E2 规则渲染的图注段（含 HHI）。构造最小 `adversarial_review.json {claim_registry:{path:"a4_claims.json", sha256:<真>}}`（供 #3 与用例 3/13）。
...
9. `amend_chain_updates_receipt`：改报告一处措辞并追加合法 amendment → `amend` exit 0，`amendment_chain` 长 1，`--receipt-only` PASS；再改工单 `fig2.required_entity_ids` 后 `amend` → exit 2 文案含 `frozen_sha256`。
...
12. `fig2_series_replay_binds_workorder`：`fig2-series` 产物 → #9 PASS；手改 whale_series 一位 → #9 BLOCK（重放 sha 不符）；工单 lines 加一个序列里没有的实体 → #9 BLOCK。
...
- **APU 0914 完整 closeout 挪到 W3 验收**：W1 改扫描器后 APU 两份 scan 绑定的扫描器整文件 SHA（`holder_distribution_scan.py:650-652`，`semantic_payload` 保留）必失配，且 APU `a4_seal.sealed_files` 封了 `distribution_scan.json`——重跑 initial scan 会改封口件 sha，须重新 finalize、重开周期、重跑 final scan，这正是 W3 `reseal --from a4` 的全链。故 W2 阶段对 APU 副本只做：`fig2-series` 迁移（记录迁移范围：新增 whale_series.provenance.json、whale_series.json 被重生成）后跑 `check`，要求 **#2–#11 全 PASS**，#1 允许且只允许因扫描器 SHA/标签 manifest 失配 BLOCK（detail 逐条记录）。完整 PASS 在 W3 用 reseal 迁移副本验收。
- BTW 纯规则：用 BTW `facts.json`/`analysis-state.json`/工单结构在临时目录直接 import E3 的 fig2/flow 判定函数：required 含 `e_proj`，`flow.items` 兼容，图中实体==eligible。
