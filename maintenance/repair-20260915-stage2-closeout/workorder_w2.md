...
> 版本 **v6**（2026-09-16，codex 第五轮复核 `scratchpad/cx_reply_w2v5.txt` 确认价格三规则/fig1 四键/两案推演均成立，仅退回一条：夹具 fig2 缺 `out`，已补；v5 按第四轮 `cx_reply_w2v4.txt` 五条修订。历次复核见 `cx_reply_w2v3.txt`/`cx_reply_w2v2.txt`/`cx_reply_w2.txt`）。依据已批计划 `plan_v2_approved.md` §2.2、§2.4、§4、§6。**开工前提：W1 已合入 main**（本单调用 W1 的 `find_snapshot(case_dir, requested, stage)`、`a4_gate limits-extract` 与 `holder_distribution_scan` 的 `reopened_from_cycle` 字段）。行号一律以**开工时 HEAD 的实物**为准，下文行号是 764b60c 基线锚点，W1 已使 `a4_gate.py` 位移（`main` 已到 `:574` 附近、分派字典在 `:600` 附近），**按锚文本定位，不按旧行号施工**。
>
...
>
> **v4 相对 v3 的改动摘要**：E1-9 加逐行 `entity_id == series_source.key`、time_range 理由改正、PNG 边界补一句；E6 重放改**字节级**（`read_bytes`、编码后替换、禁换行归一化）；E3.2b 把 `bindings.report_md` 排除出通用重验（走锚＋链专用校验且 path 须等于 `--report`）、必需输入引用先查完整再重验（缺 `sha256` 即 BLOCK，别名表兼容两案）、自引用例外收窄到 `fig3.events_input`；E3.6 `display_label` 改为非空＋与 facts 不等记 NOTE（两案实况均为缩写，等值会误拒）；§F 用例 8 改为"期望串整段消失"负例并断言 #7 文案；§7 BTW 预期补 revision 实况错误检出（工单 1、重算 2）与 required 三项；§0.2 白名单允许登记新增原子写入函数；§C2 保留 `import a4_gate`；E6 措辞"冻结字段不可改"。
>
> **v5 相对 v4 的改动摘要**：E3.2b (i) 与 (ii) 用同一自引用例外（APU `fig3.events_input` 合法空哈希不得在 (i) 先被拒）、必需引用补 `bindings.price_source`＋图 2 价格引用同路径＋双源检查在场（兼容 APU `price_source_checks{path,sha256}` 与 BTW `price_source.dual_source_check` 对象）；E3.2c fig1 四键完备（`state` 须等于 `bindings.state.path`，非空 `price_csv` 须等于某个已重验引用的 path）；§F 用例 8 改测 `private_main`（把图注全部 `100.00%` 换掉）、用例 12c 展示名不等改 NOTE、夹具第 4 步补价格/成交量/事件引用与 `bindings.price_source`；§0.2 原子写入按"script＋实际含 os.replace 的函数"登记 `overwrite_single` 并去重、schema 登记扩现有数组不新增对象、`sys.exit(main())`、SUITE 150；E5 `--receipt-only` 先核收据 schema。
...
2. **只新建一个脚本** `scripts/report/stage2_closeout.py`。其余改动全部落在现有文件：`audit_release_gate.py`、`figures_from_facts.py`、`a4_gate.py`、`build_html.py`、`scripts/tests/run_all.py`、`scripts/tests/invariant_manifest.json`（**只允许两类新增登记行**：①schema 登记——`stage2_closeout.py` producer/consumer 登记 `stage2-closeout/v1`，`figures_from_facts.py` producer 登记 `fig2-series-provenance/v1`，`stage2_closeout.py` consumer 登记 `fig2-series-provenance/v1`；②`atomic_writes` 登记——格式 `{"script": <脚本相对路径>, "locator": <实际包含 os.replace 的函数名>, "semantics": "overwrite_single"}`，扫描器按 `script＋locator` 去重：一个函数内两个 `os.replace` 只一条，调用已登记 helper 的上层函数**不登记**；本单预计 `figures_from_facts.py` 一条（fig2 输出＋旁车依次替换，不构成 `dual_file_txn`）、`stage2_closeout.py` 一到两条（收据写入 helper 一条，fill-workorder 写工单一条；amend 复用收据 helper 不再登记）；schema 登记**扩现有 `schemas` 数组**（`figures_from_facts.py` producer 数组加 `fig2-series-provenance/v1` 保留原两项；`stage2_closeout.py` producer 一个对象含 `stage2-closeout/v1`，consumer 一个对象含 `stage2-closeout/v1` 与 `fig2-series-provenance/v1`），**不得给同一脚本另追加只含新 schema 的独立对象**（codex 内存推演会产生三项差异）；新脚本入口必须 `sys.exit(main())` 传返回码（另一 invariant 检查）；不扩 `formal_entrypoints`；SUITE 149→150。否则 `invariant_scan` 报 `code point missing from manifest`；W1 已登记的 3 行保留；不得删改任何既有行；若还报其他未登记项，停工写进 `w2_done.md` 请示，不得自行扩）；新建测试 `scripts/tests/test_stage2_closeout.py`。
...
③**与工单逐条等值**：工单 `fig2.lines[]` 的 `series_source.path` 全部相同且 == 旁车 `entity_series.path`，`series_source.sha256` == 旁车 `entity_series.sha256`；`lines[].series_source.key` 列表（无重复）与旁车 `keys`（无重复）集合相等；`lines[].entity_id` 无重复；**逐行 `lines[i].entity_id == lines[i].series_source.key`**（`fig2-series` 直接用 key 作 entity_id，交换两行 key 即错配，集合比较查不出）。任一不等 → BLOCK 文案指明是 path/sha/keys/重复/逐行错配。
...
⑥whale_series 的 `entity_id` 集合 == 工单 `lines[].entity_id` 集合（工单 lines 非空而序列空即 BLOCK）；whale_series 每行 `label` == `facts.entities[entity_id].label`（重放已保证，仍显式核）；`lines[i].display_label` **只要求非空**，与 facts label 不等 → NOTE"展示名为缩写"（两案六条实况全是缩写，等值会误拒；家族 3 的缺陷是 entity_id 写展示名，已由 E3.4 拦）。
⑦`time_range` **本版不消费**（两案均为 `[起,止]` 两元素，但 `fig2-series` 输出全段序列不按它裁剪，核它等于核一个未被消费的声明）：detail 记 NOTE"time_range 未核"。
**边界（写进 detail 与 `--help`，勿夸大）**：以上只闭合"案根 whale_series.json ↔ 工单选材 ↔ entity_series 实物"，**不能证明 −3 渲染的 PNG 用的是这份序列**——`plot_whale_vs_price` 接受调用方任意数组（`standard_charts.py:289`），该边界既有，−3 的 `figure2_check_receipt` 也只绑定被检查的序列文件，**同样不能证明 PNG 用了它**；本单不改绘图库，边界如实写进 detail。
...
2b. **必需输入引用先查完整，再通用重验**。(i) 必需引用（缺对象或对象缺 `path`/`sha256` 键、或 `sha256` 为 null → BLOCK 文案指明字段路径；**空哈希唯一放行形状与 (ii) 相同**：字段位置 `fig3.events_input` 且 `path == 当前工单相对路径` 且 `key == "fig3.events"`，APU 实况，(i) 阶段即放行为 NOTE，不得先拒后豁免）：`fig2.lines[*].series_source`；`flow.charts[*].spec` 或 `flow.items[*].spec`；`fig3` 的价格输入（键名别名 `price`｜`price_input`）与成交量输入（`volume`｜`volume_input`）；`fig3.events_input`；`bindings.facts/state/rounds`；**`bindings.price_source{path,sha256}`**；第 2 条按路径找到的终态 scan/图两项。另两条价格规则：**图 2 价格引用**（`fig2.price_source`｜`fig2.price`，值为字符串或含 `path` 的对象）必须在场且其路径 == `bindings.price_source.path`（BTW `fig2.price` 无哈希，靠同路径挂到已重验绑定；若自带 `sha256` 则也核）；**双源检查在场**：`bindings.price_source_checks{path,sha256}`（APU，走 2b 重验）或 `bindings.price_source.dual_source_check`（BTW，对象在场即可，**不要求带哈希**），两者皆无 → BLOCK。别名表只含两案已出现的写法，其他键名不认。(ii) 通用重验：递归遍历工单全文（`stage2_selfcheck` 与 **`bindings.report_md`** 除外——报告锚走第 2/3 条的锚＋amendments 链专用校验，且其 `path` 必须等于 `--report` 传入的相对路径；APU 实况原稿锚 ≠ 现稿 sha 而链正确，套"等于实物"会误拒并让首次 amend 全量重跑必败），凡对象同时含 `path`（str）与 `sha256` 键 → path 过围栏（第 6 条）且 `sha256(实物) == sha256`；`sha256` 为 null → BLOCK，**唯一例外**：`fig3.events_input` 且 `path` == 工单自身路径且 `key == "fig3.events"`（APU 实况）→ NOTE 不核；其他任何指向工单自身的引用不豁免。
2c. **各图最小结构**（缺一 BLOCK）：`fig1`/`fig2`/`fig3` 均为对象且含 `out`（str）；**`fig1` 四键 `state/price_csv/overlay/out` 必须存在**（`price_csv`、`overlay` 允许 `null`，APU 实况）：`fig1.state == bindings.state.path`；`price_csv` 非空时其路径必须等于工单中某个已通过 2b 重验的 `{path,sha256}` 引用的 `path`（BTW 实况＝`fig3.price_input.path`），字符串字段递归检查覆盖不到，须显式核；`fig2.lines` 非空 list、`fig2.required_entity_ids` list；`fig3.events` 非空 list 且每项含 `date`（`YYYY-MM-DD`）与 `label`；`flow` 含 `eligible_entity_ids` list 且 `charts` 或 `items` 至少一键（第 5 条）。不核更细的类型（两案写法差异大，本轮不定死）。
...
4. **实际改动＝获准改动（字节级重放）**：取 `exact_change{before,after}`，**全程 `read_bytes()`，`before/after` 用 UTF-8 编码后在字节串上查找替换，禁止 `read_text()`/换行归一化**（否则 `\r\n` 一类字节改动会被文本层吞掉而通过）。默认**反向重放**：当前报告字节中 `after` 必须恰出现 1 次，替换为 `before` 得到"前稿字节"，其 sha 须 == `before_sha256`（前稿 sha 等值即证明替换范围外无任何字节改动）；`after` 为空串或出现次数≠1 时必须传 `--previous`（其字节 sha 须 == `before_sha256`），改为**正向重放**：前稿字节中 `before` 恰出现 1 次，替换为 `after` 后 sha 须 == 报告实物 sha。两向都不成立 → BLOCK"实际改动与 exact_change 不符：正文混入未获准改动"。`approved_by`/`approval_text` 只要求非空，不作证明。
...
4. 写 `entity_series.json {dates:[…], "e1":[…最后一点=100.0]}` → `fig2-series --keys e1 --out whale_series.json` 产案根 whale_series＋旁车；写最小 `flow_e1.json` spec（可复用 `test_a4_gate` 现有 flow 夹具写法，若无则最小合法 spec）；**先写完报告正文**（第 5 步）再首次 `fill-workorder`（fill 不覆盖已存在的 `bindings.report_md`，先 fill 后改正文会锁错锚）；fill 生成骨架后补 fig2（lines=[{entity_id:e1, display_label:"大庄#1", series_source{path:"entity_series.json", key:"e1", sha256}}]，required=[e1]，**`out:"charts/final/fig2.png"`**（2c 要求三图都有字符串 `out`，fill 不填选材字段，缺键或 null 即拒））、flow（eligible=[e1]，charts=[{entity_id:e1, spec{path,sha256}, out:"charts/final/flow_e1.png"}]）、fig1 四键（`state:"analysis-state.json"`, `price_csv:null`, `overlay:null`, `out`）；写最小 `price_series.json`（`[[unix_ts, close]…]`）与 `volume_series.json`，登记 `bindings.price_source{path,sha256}` 与 `bindings.price_source_checks{path,sha256}`（最小 JSON 文件即可）、`fig2.price_source` 同路径、`fig3.price/volume{path,sha256}`、`fig3.events_input{path:"a5_assembly_workorder.json", key:"fig3.events", sha256:null}`（合法自引用）、`fig3.events` 一条、`fig3.out`——E3.2b (i) 全部必需引用齐备，否则 #10 必拒。
...
8. `caption_mismatch_rejected`：夹具是单 owner，主桶与 top-k 都是 `100.00%`、多桶为 0，改一处数字别处仍命中——负例须让**某桶的全部可接受串在整个图注段消失**——零值桶的 `0` 会被 `100.00%` 里的 0 命中，删尘埃句不成立；改测 `private_main`：把图注段内**全部** `100.00%` 替换成 `99.00%` 并确认 `100.00%` 不再出现，断言 **#7 status==BLOCK 且 detail 含"图注与终态 scan 不同源: private_main"**，不得只断言总 verdict（报告 sha 变化会让别的子项 BLOCK 冒充）。
...
12c. `fig2_duplicate_and_label_rejected`：工单 lines 重复 entity_id → BLOCK；两行交换 `series_source.key` → BLOCK（逐行错配）；`display_label` 空串 → BLOCK；`display_label` 非空但与 facts label 不等 → **NOTE 不 BLOCK**（#9 status 仍 PASS，detail 含"展示名为缩写"）。
...
- **APU 0914 完整 closeout 挪到 W3 验收**：W1 改扫描器后 APU 两份 scan 绑定的扫描器整文件 SHA（`holder_distribution_scan.py:663/:1073`，`semantic_payload` 保留）必失配，且 APU `a4_seal.sealed_files` 封了 `distribution_scan.json`——重跑 initial scan 会改封口件 sha，须先 `reopen-cycle` 解除旧 terminal、再 finalize、再重跑 final scan，这正是 W3 `reseal --from a4` 的全链（`a4_gate.py:285` 见 terminal 即拒，顺序不可反）。另外 APU `accounting_mode.json` 的 observation bundle 用原案绝对路径，普通复制到新案根会被案根包含检查拒（`shared_release_receipt.py:367/:1755`），副本迁移办法也归 W3 定义。**因此 W2 阶段对 APU 只做纯规则验收（与 BTW 同法，不跑组合 `check`）**：在临时目录 import E2/E3 判定函数，喂 APU 的 `facts.json`/`analysis-state.json`/工单/报告.md/终态 scan：fig2 required 下限得 {TE-01,TE-02,TE-04} 且 lines ⊇ required、逐行 entity_id==key；display_label 三条均为缩写 → NOTE 不 BLOCK；flow 下限空/eligible 空/charts 空 → PASS 带 NOTE；图注五桶＋top-k＋HHI 全部命中（codex 逐桶重算：76.69%/0.0005%/23.31%/0/24.51%，top-k 3.76/8.89/12.17/17.09，HHI 0.0049）。不宣称 "#2–#11 PASS"。
- BTW 纯规则：同法。required **下限** == {e_proj}（工单实际 `required_entity_ids` 三项 e_proj/e_aug_cohort/e_wash_w ⊇ 下限，合法），`flow.items` 兼容，图中实体==eligible；**`bindings.entity_freeze_revision` 工单写 1、重算 2（final scan 也写 2）→ #10 BLOCK，这是实况工单错误被正确检出，验收预期记 BLOCK，不放宽公式**；图注五桶命中（24.02%/0.0001%/75.97%/0.0056%/0.0%），无 HHI/top → NOTE 不 BLOCK；无流通量 → NOTE。
