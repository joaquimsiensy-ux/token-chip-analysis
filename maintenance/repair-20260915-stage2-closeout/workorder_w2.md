...
2. **只新建一个脚本** `scripts/report/stage2_closeout.py`。其余改动全部落在现有文件：`audit_release_gate.py`、`figures_from_facts.py`、`a4_gate.py`、`build_html.py`、`scripts/tests/run_all.py`、`scripts/tests/invariant_manifest.json`（**只允许两类新增登记行**：①schema 登记——`stage2_closeout.py` producer/consumer 登记 `stage2-closeout/v1`，`figures_from_facts.py` producer 登记 `fig2-series-provenance/v1`，`stage2_closeout.py` consumer 登记 `fig2-series-provenance/v1`；②`atomic_writes` 登记——本单新增的每个 `os.replace` 写入函数（`fig2-series` 旁车/输出、closeout 收据、`fill-workorder`、`amend`）按 `invariant_scan.py` 现有登记格式登记函数名与语义，否则 `invariant_scan` 报 `code point missing from manifest`；W1 已登记的 3 行保留；不得删改任何既有行；若还报其他未登记项，停工写进 `w2_done.md` 请示，不得自行扩）；新建测试 `scripts/tests/test_stage2_closeout.py`。
...
8. `caption_mismatch_rejected`：夹具是单 owner，主桶与 top-k 都是 `100.00%`、多桶为 0，改一处数字别处仍命中——负例须让**某桶的全部可接受串在整个图注段消失**（如把 `private_dust` 桶那句整句删掉并确认 `0`/`0.0%`/`0.00%` 三串都不再出现在图注段），断言 **#7 status==BLOCK 且 detail 含"图注与终态 scan 不同源: private_dust"**，不得只断言总 verdict（报告 sha 变化会让别的子项 BLOCK 冒充）。
