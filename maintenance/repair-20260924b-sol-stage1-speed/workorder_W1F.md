# 返修单 W1F（v1）：校验器继承 recheck 见证——完整响应中未返回块头的 slot 须按值 1 参与继承条件与跨记录冲突检查 —— 归属 W1（盲审 r2 FAIL 反例修复）
...
> ① `scripts/lib/solana_exact_validate.py:532` `_inherited_recheck_values(row, template_sha)`：核请求摘要/模板/响应见证/摘要/size/`slots_covered==end-start+1` 后，空响应分支核 `empty_response/returned_*/n_blocks` 事实并 **`return {}`（`:562`）**；非空分支 `values = {}`（`:563`）只对响应中出现的块写 `values[slot] = min(255, 2 + len(instructions))`（`:572`），完整性靠 `previous == end` 等事实核（`:574-580`）——**请求区间内未返回块头的 slot 不出现在返回映射中**。
> ② `_validated_inherited(:589)` 在 `:644-650` 合并各 recheck 行的 measured：`if slot in actual and actual[slot] != value: raise "recheck results conflict"`；`:654` 要求 `actual.get(slot) == 2`。由于①不返回隐含的 1，「空响应」或「非空但缺该 slot」的完整 recheck 既不触发冲突、也不使 `actual.get(slot)` 变为非 2。
...
- 1.2 `_validated_inherited` 的冲突检查与 `actual.get(slot) == 2` 判据不变——修 ① 后，反例 A（零 nonce 块 vs 空响应）在 `:646` 触发 `recheck results conflict`；反例 B（非空完整响应缺该 slot）同样冲突或使 `actual[slot]==1` 被 `:654` 拒。
...
## 2. 逐条施工
- 2.1 `solana_exact_validate.py`：锚 `        return {}`（`:562`，函数内唯一）改为返回全 1 映射；锚 `    values = {}`（`:563`）改为 `values = {slot: 1 for slot in range(start, end + 1)}`；`:572` 覆盖逻辑不变。
