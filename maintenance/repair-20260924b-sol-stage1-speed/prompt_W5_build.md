...
# 工单 W5（v3）：收官 review P2/P3 文档修正（find-known-map 异常分支采纳纪律＋CHANGELOG 9.2.0 归并最终验收）—— 纯文本，不升版本

> v3 变更：吸收 `review_W5_reply_r2.md` 唯一必改——§0.7 保留临时目录的补丁改为 `lambda self: self._finalizer.detach()`（空函数只拦显式 cleanup，对象回收时的 finalize 回调仍会 rmtree）。
