...
# 工单 W2（v3.2）：−1 执行者强制使用共享覆盖地图（`find-known-map` 子命令＋流程硬性＋文档）＋ 版本 9.2.0 登记 —— 本工程收官单（producer 登记走 `workorder_WR.md` WR-b）
...
> v3.2 变更（调度方派工填实，不改要求）：`<W2_BASE>`＝`97e888df0a5cfeb88a0b94285475c8845d7aac40`（W4 收官 commit，已含 W3/W1/W1F/W4/WR-a）；§0.1 改为「祖先＋生产文件相对基线无差异」形式（与 W1F/W4 同款，允许 HEAD 领先基线若干仅含 maintenance 的 commit）；事实⑥更新：WR-a 已登记 repair 现役 `15822564…`（commit `59f88b8…`）四协议 ACTIVE，旧 `3f89aab1…` 保留 ACTIVE；probe 现役仍 `c4980c98…`，当前 probe 文件哈希 `ab2371f5…`（W1 过渡）未登记、`test_producer_registry_current` 对 probe 两协议 FAIL 属预期，由 WR-b 在本单后登记最终 sha；六处整行锚已由调度方在 HEAD 按 §0.5（含 `-e`）重核全部在位；CHANGELOG 日期填 2026-09-24。
...
- 0.1 工作目录同 W1；固定 `W2_BASE=97e888df0a5cfeb88a0b94285475c8845d7aac40`；开工贴 `git status --short`（空）、`git rev-parse HEAD`、`git merge-base --is-ancestor "$W2_BASE" HEAD`（exit 0）、`git diff --quiet "$W2_BASE" HEAD -- scripts references SKILL.md commands-staging VERSION pyproject.toml CHANGELOG.md assets`（exit 0）、`git merge-base --is-ancestor cc6298b HEAD`（exit 0）。任一不符停工。**不继承** W1「相对 cc6298b 无差异」检查。
