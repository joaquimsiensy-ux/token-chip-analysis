...
# 工单 W4（v3.2）：修复生产者每候选 slot 的 SQD 请求由两次（状态探针＋census）合并为一次 —— 归属版本 9.2.0（producer 换代；登记走 `workorder_WR.md` 登记单 WR-a）

> 出处：用户 2026-09-24 裁决第 4 条「同一个候选 slot 被 SQD 探两次，看看怎么修复」。
> v3.2 变更（调度方派工填实）：`cfe2f41`＝W1 收官 commit（见 §0.1）；W1 已改 `solana_exact_validate.py`（+352）与 `sqd_gap_repair.py`（仅 `:585` 一行）、`test_sqd_gap_repair.py`（+114，main 移至 `:1321`），W1F 返修（`cfe2f41`，`solana_exact_validate.py` 两行原位替换、净行 0）后调度方再核，行号未漂。本单引用的校验器行号已按新 HEAD 重核：深验摘要字段检查 `:1949-1950` 附近（原 :1594-1606）、`_repair_state_matches:1549`（原 :1202）、`validate_repair_pointer:1260`、`_repair_getblock_params_digest:1571`（原 :1225）、现役 sha 判据 `:712-716`（原 :499-506）、`validate_repair_bundle_deep:1592`；修复脚本行号不变（`_census_body:643`、`_state_probe:917`、调用 `:1024`、`_fetch_live_slot:1022`、`_plan:596`、`_verify_ledger_rows:745`、`_verify_adopted_record:783`、`_payload_from_evidence:938` 起、`_persist_live_slot:989`、`_routea_slot:1226`、`_sqd_call_with_backoff:900`）；`test_batch8_repair_scale.py` 行号不变（`:57/:61/:316/:325`）。施工前仍按 §0.5 整行锚重核。
...
> 事实（调度方本机亲核，基线 `cc6298b`；**派工基线＝W1 收官 commit `cfe2f41`，届时行号重核**，W1 只改本脚本 `validate_coverage_state_consistency` 一函数）：
...
- 0.1 工作目录＝`/Users/uravvv/.claude/skills/token-chip-analysis`。固定施工基线 `W4_BASE=cfe2f41132bd3f162f3d356f359c9eba72a500d3`（W1 收官 commit，已含 W3/W1）；开工先设置该变量，再跑并贴进 `W4_done.md`：`git status --short`（须为空）、`git rev-parse HEAD`、`git merge-base --is-ancestor "$W4_BASE" HEAD`（须 exit 0）、`git diff --quiet "$W4_BASE" HEAD -- scripts references SKILL.md commands-staging VERSION pyproject.toml CHANGELOG.md assets`（须 exit 0）、`git merge-base --is-ancestor cc6298b HEAD`（exit 0）。任一不符停工。
...
- 1.7 `git diff --stat "$W4_BASE" -- . ':!maintenance'` 只含 0.3 白名单。
