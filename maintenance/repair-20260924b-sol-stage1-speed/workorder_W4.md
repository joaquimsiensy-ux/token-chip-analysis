...
- 2.4 模块 `:46-49` 注释旁追加 ≤4 行：「9.2.0：α 修复的状态探针并入 census 请求（`_census_body` 经 `sqd_query_body` 复用探针选择器）；新采 evidence 的 `coverage_probe_*` 与 `query_body_sha256/response_sha256` 同值；改 `_census_body` 语义须换代并登记 producer_history」。
- 2.5 测试：
