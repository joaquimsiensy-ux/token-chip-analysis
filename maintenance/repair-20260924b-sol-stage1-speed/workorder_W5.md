# 工单 W5（v1）：收官 review P2/P3 文档修正（find-known-map 异常分支采纳纪律＋CHANGELOG 9.2.0 归并最终验收）—— 纯文本，不升版本
...
- 0.3 **白名单**：`references/split-run.md`（仅 `:41` 一行内的指定片段）、`commands-staging/token-analyze-1.md`（仅 `:12` 指定片段）、`assets/sqd-solana-coverage-map/README.md`（仅 `:5` 指定片段）、`CHANGELOG.md`（仅 `:108-110` 三条内的指定片段）、完成报告 `W5_done.md`。
...
- 0.7 定向：`test_g3_docs_guards.py`、`test_review_scale_guards.py`、`test_sqd_coverage_probe.py`、`test_exemption_guards.py`（均须 exit 0）；`test_commands_deploy_sync.py` 预期 FAIL（staging 改后待调度方同步本机，报告如实写）；`docs_lint.py`/`changelog_lint.py` 读禁区交调度方。
...
- 2.2 `commands-staging/token-analyze-1.md:12`：
  ...
  新：`仅 exit 0 才采用 chosen 并带 --known-map，exit 2 且 chosen=null 才全扫，exit 1 先处理错误；`
- 2.3 `assets/sqd-solana-coverage-map/README.md:5`：
  旧：`有 chosen 必用 --known-map，完整加载失败时按探针规则回退。`
  新：`仅 exit 0 才采用 chosen 并用 --known-map（exit 1 即便 chosen 非空也不得采用；exit 2 且 chosen=null 才全扫），完整加载失败时按探针规则回退。`
...
- 2.6 `CHANGELOG.md:110`（成本-质量指标）：
  ...
  新：`W3 联网单 slot 对照：传输字节 4,516,539→736,729（gzip，规范化结果摘要相同，fable_probes P5）；W4 合并查询三组内容对照三方全等（fable_probes P3）；完整生产链路吞吐收益未证明；`
