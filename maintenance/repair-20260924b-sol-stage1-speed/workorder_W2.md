# 工单 W2（v2）：−1 执行者强制使用共享覆盖地图（`find-known-map` 子命令＋流程硬性＋文档）＋ 版本 9.2.0 登记 —— 本工程收官单（producer 登记走 `workorder_WR.md` WR-b）
...
> 事实（调度方本机亲核；**派工基线＝W3 收官 commit `<W2_BASE>`，届时行号与锚重核**）：
...
> ③ 流程文档锚（完整整行，派工时重核恰 1 处）：`references/split-run.md:41`（以 `- **A2 全部**：EVM 四查；Solana 五查＝四查＋精确重放 \`exact_reconcile\`。` 开头的整行）；`commands-staging/token-analyze-1.md:12`（`3. **范围**＝A0–A2 全部＋A3 机械子层（split-run §1.3）；…` 整行）；`references/data-pipeline-solana-capture.md:202`（`**共享地图生命周期**：已知缺陷地图只省重复探测，不替代本案证据。…` 整行，288 B）、`:107`（`- **gzip 压缩 = 21 倍**：…` 整行）、`:198`（`2. \`sqd_gap_repair.py/v1\` 只修已确认缺陷，…` 整行）；资产 README `:3` 首段（把来源限定为 `--full` 且含「首版未入库」过时叙述）。`~/.claude/commands/token-analyze-1.md` 与 `commands-staging/` 逐字一致（调度方收官后同步，仓库外）。
> ④ 版本四处：`VERSION:1`、`pyproject.toml:15`、`SKILL.md:23`、`CHANGELOG.md:13/:104` 均 9.1.1；`CHANGELOG.md:4` 档位规则；`changelog_lint.py:21-33` 日期须合法（`2026-09-2X` 不通过）；`SKILL.md` 8,021 B（`docs_lint.py:300-302` 上限 8,192 B）。
...
> ⑥ producer_history：probe 现役 `c4980c98…` 登记 coverage/v1、coverage-pointer/v1；repair 现役 `3f89aab1…` 登记 cache/v4、repair-bundle/v1、coverage-resolution/v1、repair-pointer/v1；**无 shared-map 历史条目**。本单不改 producer_history（登记单 WR-b 在本单收官后登记探针最终 sha）。
...
- 2.1 `sqd_coverage_probe.py` 新子命令 `find-known-map --from-slot N --to-slot M [--search-dir DIR]...`，入口接线在 `main(argv)` 的 export 分支旁（锚 `    if argv and argv[0] == "export-shared-map":`），独立 parser。设计：
  ...
  - stdout 仅一行 JSON：`{"chosen": accepted[0]|null, "accepted": [{path(绝对), version, generated_at, expires_at(UTC ISO-8601), overlap_slots, overlap_ratio, candidate_count, refuted_count}...], "rejected": [{path, reason}...]}`；单个坏文件进 rejected 后继续。退出码：有 chosen 0；正常无 chosen 2；运行故障 1；argparse 错误沿用 exit 2＋stderr。缺失目录记 reason、空目录允许、不可读目录不得静默当空。
  - 测试（tempfile 小资产＋固定当前时间，不依赖仓库真实地图当天是否过期）：①过期/不重叠/合格/空目录；②多份合格资产按 overlap、交集 refuted、UTC 时间、路径并列排序；③JSON 损坏、字段类型错、缺二进制、size 不符、区间不一致、路径逃逸、模板不符；④旧资产 refuted 空缺两键仍可轻筛；⑤多目录追加去重、默认目录不依赖 cwd；⑥一行 JSON 确切字段、chosen==accepted[0]、无图 exit 2 与运行错误 exit 1 区分。默认目录指向临时目录；网络调用、完整 `validate_shared_map`、二进制解压替换为一旦调用即失败的桩。保留既有 probe/export 测试确认 main 接线不破坏旧入口。
...
- 2.4 `references/data-pipeline-solana-capture.md:202` 整行替换为（≤900 B）：「**共享地图生命周期**：开工必跑 `find-known-map`；TTL 从源 coverage 发布时刻算 30 天，驳回证据的时效从其首次直接 census 证明算起、不因链式导出刷新。`refuted_slots` 来自已发布修复 census 或链式继承；仅资产内、本案复用成功区间且不在 unverified_ranges 的 slot，SQD 重查成功且实测值＝资产值＝2 且证据未过期，才记 `INHERITED_REFUTED`，不入候选，免该 slot 的修复 Helius getBlock；getBlocks 确认仍照常。此重查只证块头在、零 AdvanceNonce，不证交易集合未变（残余风险，来源可信是前提，导出不深验源案全量证据）。重查值变化整图回退，失败重试按规则处理。refuted-only 无代，自有驳回不能导出，只能带出继承部分。回填用 `sqd_coverage_probe.py export-shared-map --case-root <案目录> --probe-id <probe_id> --out <skill根>/assets/sqd-solana-coverage-map/ --repair-gid <gid>`；无代将最后一项换为 `--no-repair`；调度方验收入库。」`:107` 整行末尾追加：「（9.2.0 起 `scripts/lib/net.py::curl_json` 内置 `--compressed`，经该层的 Helius getBlock 与 SQD 请求自动协商压缩；ledger 字节/哈希按解码后内容计算不受影响）」。`:198` 第 2 条整行末尾追加：「9.2.0 起 α 修复将状态探针并入 census；无重试时每候选 slot 一次 SQD 请求，β 不变；新采 evidence 的 `coverage_probe_query_sha256/coverage_probe_response_sha256` 与 `query_body_sha256/response_sha256` 分别同值。」
...
- 2.6 版本四处 9.1.1→9.2.0。CHANGELOG `:13` 前插索引（日期由调度方在派工时填实际日期，示例 2026-09-24，173 B）：`- **9.2.0**（2026-09-24）Solana −1 提速：驳回继承、find-known-map 开工硬性、curl_json 压缩、修复 α 合并 SQD 请求；producer 登记，档位 次。`；`:104` 前插 `## [9.2.0] - 2026-09-24 — Solana −1 机械段提速`（日期与索引一致），按 `:106-109` 四条格式「出处与裁决／改法（按实际顺序 W1→W4→W3→W2＋WR 登记）／字节与测试（引用 `W*_done.md` 实际记录）／成本-质量指标」；缺失指标写「未记录」，不预填 PASS/轮次/线上收益；W2 自身行数与测试由本次完成结果回填；沙箱外部调用与调度方联网验收分别统计。
