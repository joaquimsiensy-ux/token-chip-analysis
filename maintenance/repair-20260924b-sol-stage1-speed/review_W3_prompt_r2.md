...
首行固定 `# 工单W3复核r2：通过` 或 `# 工单W3复核r2：退回`。

## 任务
复核 `maintenance/repair-20260924b-sol-stage1-speed/workorder_W3.md`（**v2**，已吸收你上一轮 r1 意见 `review_W3_reply_r1.md`；背景 `README.md`、调度方实测 `fable_probes_20260924.md`）。逐条亲核事实段行号/锚（`grep -n -F -x`）与设计。重点：r1 的 7 条是否每条被正确吸收（逐条对照 `review_W3_reply_r1.md`）；§2.2/2.3 测试原文在当前源码上是否可直接落地（注意函数内已有 `calls` 变量的重绑定说明）；事实④调度方本机实测（`fable_probes_20260924.md` §P1）表述是否准确。
