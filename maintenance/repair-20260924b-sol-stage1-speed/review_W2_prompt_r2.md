...
首行固定 `# 工单W2复核r2：通过` 或 `# 工单W2复核r2：退回`。

## 任务
复核 `maintenance/repair-20260924b-sol-stage1-speed/workorder_W2.md`（**v2**，已吸收你上一轮 r1 意见 `review_W2_reply_r1.md`；背景 `README.md`、调度方实测 `fable_probes_20260924.md`）。逐条亲核事实段行号/锚（`grep -n -F -x`）与设计。重点：①r1 的 11 条是否每条被正确吸收（逐条对照 `review_W2_reply_r1.md`）；②producer 登记已移出本单到 `workorder_WR.md`（WR-a/WR-b 两次登记），登记单本身的纪律与时序是否成立（同时复核 `workorder_WR.md`）；③§2.1 find 设计是否仍有未定形处；④§2.4 三段追加/替换原文与字节预算是否合规（自行 `wc -c` 估算）；⑤CHANGELOG 索引 173 B 与 changelog_lint 规则。
