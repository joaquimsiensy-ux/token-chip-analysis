# 工单 W3（v2）：`scripts/lib/net.py::curl_json` 开启 HTTP 压缩（`--compressed`）—— 归属版本 9.2.0（登记在 W2）
...
- 0.1 工作目录同 W1。调度方派工时填入 W4 收官提交 `<W3_BASE>` 并重核本单行号；开工记录 `git status --short`（须为空）、`git rev-parse HEAD`（须等于 `<W3_BASE>`）、`git merge-base --is-ancestor cc6298b HEAD`（须 exit 0）。**不继承** W1「生产文件相对 cc6298b 无差异」的检查。0.2/0.5/0.6 同 W1。本目录可读：`README.md`、`workorder_W*.md`、`review_W3_reply_*.md`、`fable_probes_20260924.md`。
...
```
  ...
  注意：该函数早前已有一个名为 `calls` 的变量用于重试计数（`assert len(calls) == 3`），上述代码在其之后重新绑定 `calls = []`，不影响前面的断言；若施工方认为易混淆可改名 `argv_calls`（两处同步）。
