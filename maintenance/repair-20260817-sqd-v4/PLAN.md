...
## 勘误注记（批 5，2026-08-18；原文保留）

背景第 12 行把“被吃 124,816 条边”写成纯 DISTINCT 损失，属于开工时的混合口径预判，现已由
批 4 冻结 parts 独立 oracle 推翻，原文仅作为计划历史保留。修正口径如下（完整证据见
`batch4_done.md` §6.3）：

- 冻结的 1,348 个 parts 域内，multiset 1,775,858 行、五字段 DISTINCT 1,764,356 行，机械可证
  损失为 **11,502 行 / 8,487 个碰撞组，最高 23 倍率**；
- **124,816** 只表示 ARC 两版全史边表的行数差，混入两次采集间的其他差异，禁止再表述为
  “DISTINCT 吃掉 124,816 条”。

## provenance 威胁模型与根治宣告边界（批 7，2026-08-18；原文保留）

本计划现有正文没有工单所称的独立“根治宣告条件”段，现按批 7 工单把条件和边界显式补齐：ARC 在
v4 下 A2 达到 0/0，或 47 项残差全部完成归因，仍只证明本工程声明范围内的采集、重放与对账闭合。
`collector_sha256` 是可由 git 对象公开复算的完整性哈希，不是密码学签名；producer history、逻辑摘要
和行数校验假设 `data/` 目录可信，防版本漂移、旧产物误用和改装采集器冒名，但不防具备本地写权限的
对手同时伪造边、meta 与快照。抗主动伪造需要签名或独立链上重验，属于根治宣告后的独立后续项，
不在本工程实现范围内。
