...
## 11. provenance 宣称勘误（批 7，2026-08-18；原文保留）

二次盲审工单称本文件 §3 F-02 使用了“建立归属根基”“打断自证环”等措辞；当前文件真字节并无这两个
原句，但 §3 F-02/F-03 与 §7 的 formal 身份描述仍需按更窄边界理解：登记 producer、逻辑摘要、行数与
实际边复算只建立本地文件的完整性和版本对齐，防版本漂移、旧产物误用及改装采集器冒名；它假设
`data/` 目录可信，不能证明边确实来自链上，也不防能同时落盘自洽边、meta 与快照的主动伪造者。
`collector_sha256` 是 git 可复现的公开哈希而非密码学签名。权威现役措辞见
`references/data-pipeline-solana-capture.md`“v4 provenance 的保护范围与信任前提”。
