...
| P7 | 工单 A v3 §4（codex r2） | `parse_constant` 管不住非 pct 字段里的 `1e400` 被解析成 inf | 范围外；pct 由 A2、输出序列由 A4 兜底 | 登记；C 段扩注：发布闸 `load_json` 装载 facts.json 同样漏 `1e999`，由 C2 逐键比对兜底（facts_gate 输入侧已严格拒非有限） |
