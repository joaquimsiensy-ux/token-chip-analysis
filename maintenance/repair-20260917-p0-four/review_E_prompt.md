# 工单 E 复核提示词（只读，r3：对 v3 复核；r2 唯一未闭合项 E-02-r2（§2:19 残留"先跑 changelog_lint"）已改为调度方运行；只需 `grep -n -F '先跑'` 确认为 0 处、§1/§2/§3 口径统一，其余 r2 已核项不必重跑）
...
## 任务
复核 `maintenance/repair-20260917-p0-four/workorder_E_version.md`（v3）。工作目录＝本仓库根（HEAD 含 A–D 四段及 C7 落地）。逐项核：
