# 工单 C（v1）：R07 facts.json 由三账自动生成＋发布闸/收口重算 —— repair-20260917-p0-four 第三段
...
> 内容基线：HEAD `8ead156`（含 A/B 两段）。本段白名单文件与 HEAD 逐字节相同；`references/`、`SKILL.md`、`commands-staging/` 与 4cbfe48 相同。行号均指施工前基线。
> 设计要点（调度方已实证，复核方请核）：①发布闸**不**把 facts.json 加进 `NEW_ANALYSIS_REQUIRED`——全仓六个跑 new-analysis 闸却不带 facts 的夹具（batch15 :369-421、batch18 :147-180、test_audit_release_gate :469-484 等）会整体变红；改为"facts.json 在场即验"，在场性由 A4 finalize 的 `MANDATORY_SEAL_FILES`（`a4_gate.py:52` 含 facts.json）＋发布闸对 A5 seal 的重验（`audit_release_gate.py:1673-1680`）传递保证，登记 P11。②stage2 收口夹具 `test_stage2_closeout.py:47-60` 的 facts/state/identity 用 `ENTITY_ADDR`（0xaaa…）而三账（`build_case` :352-361）用 `0xabc`，新闸必红；改夹具三处地址为 `0xabc`（identity 校验器 `entity_identity_gate.py` 与 `identity_snapshot_receipt.emit_evm` 均无地址格式检查；`build_case` 原始转账夹具本身用 "a"/"b" 过重放），facts 改由 build 生成。③`state_source.json` 只新增**一个**顶层块 `facts_inputs`（`state_from_facts.compile_state` :95-142 按键取值、不展开 source，新块不会漏进 analysis-state.json）。④文档只改 `references/report-template.md:212` 一处（−5 B）；`analyze-workflow.md:147` 不改。

## 0. 开工纪律

- 0.1 工作目录＝`/Users/uravvv/.claude/skills/token-chip-analysis`。开工先跑并贴进 `C_done.md`：`git status --short`（须为空）；`git rev-parse --short HEAD`（须为 `8ead156`）。不符即停工写 `C_done_attempt1_stopped.md`。
...
- 0.8 本段只跑：`python3 -B scripts/tests/test_report_facts.py`、`test_stage2_closeout.py`、`test_repair_batch_d.py`、`test_audit_release_gate.py`、`test_state_from_facts.py`、`test_figures_from_facts.py`、`test_batch15_three_ledgers_frozen.py`、`python3 -B scripts/tests/invariant_scan.py`、`python3 -B scripts/tests/docs_lint.py --all`（若该脚本支持；不支持则跑默认）。不跑 run_all。
...
- 1.4 三账不闭合的案不得生成 facts（build 先跑 `check_three_ledgers(chain=None)`，有错即 exit 2）。
...
### C2 `scripts/report/audit_release_gate.py`：新增 `check_facts_vs_ledgers`，new-analysis 在场即验
...
```python
def check_facts_vs_ledgers(case_dir: Path, facts, errors: list[str]):
    ...
    facts.json 的在场性由 A4 finalize MANDATORY_SEAL_FILES＋A5 seal 重验保证（P11）。"""
...
```python
        # R07（7.2.0）：facts.json 在场即按三账重算复核（在场性由 A4 MANDATORY_SEAL_FILES＋A5 seal 重验保证）
        facts_path = regular_case_path(case_dir, "facts.json") if profile == "new-analysis" else None
        if facts_path is not None and facts_path.is_file():
            check_facts_vs_ledgers(case_dir, load_json(facts_path, errors), errors)
```

`regular_case_path`（`:482`）与 `load_json`（`:406`）为本文件既有。
...
- `:74`（锚 `FIGURES = REPO / "scripts/report/figures_from_facts.py"`）之后加 `FACTS_GATE = REPO / "scripts/report/facts_gate.py"`。
- `:48-49`：`state` 的 `"addresses": [fixture.ENTITY_ADDR]` → `"addresses": ["0xabc"]`（与 `build_case` 三账成员一致）。
- `:52-55`（锚 `    write(case / "facts.json", {` 起四行）整块替换为：

```python
    (case / "peak_evidence.json").write_text('{"peak_raw": "100", "peak_date": "2026-01-01"}\n', encoding="utf-8")
    write(case / "state_source.json", {
        "schema": "analysis-state-source/v1",
        "facts_inputs": {"symbol": "TT", "decimals": 0, "entity_labels": {"e1": "大庄#1"},
                         "peak_overrides": {"e1": {"peak_raw": "100", "peak_date": "2026-01-01",
                                                   "evidence": {"path": "peak_evidence.json",
                                                                "sha256": sha(case / "peak_evidence.json")}}},
                         "metrics": {}}})
    proc = subprocess.run([sys.executable, str(FACTS_GATE), "build", "--case-dir", str(case)],
                          capture_output=True, text=True)
    assert proc.returncode == 0, proc.stdout + proc.stderr
```

- `:58`：identity `rows` 的 `"address": fixture.ENTITY_ADDR` → `"address": "0xabc"`。
- `:123`、`:352`、`:590` 三处 `== 11` → `== 12`（锚见 §0.5，各恰 1 处）。
...
- `:1207`（锚 `    write_json(root / "whale_series.json", [])`）之前插入（`owners_path` 为同函数 `:1056` 已有变量；`sha_file` 为本文件既有）：

```python
    owners = json.loads(owners_path.read_text(encoding="utf-8"))
    if isinstance(owners.get("balances"), dict):
        owners = owners["balances"]
    owner_amount = str(int(str(next(iter(owners.values())))))
    (root / "peak_evidence.json").write_text(json.dumps({"peak_raw": owner_amount}) + "\n", encoding="utf-8")
    write_json(root / "state_source.json", {
        "schema": "analysis-state-source/v1",
        "facts_inputs": {"symbol": "SOLX", "decimals": 0, "entity_labels": {"e1": "e1"},
                         "peak_overrides": {"e1": {"peak_raw": owner_amount, "peak_date": "2026-01-01",
                                                   "evidence": {"path": "peak_evidence.json",
                                                                "sha256": sha_file(root / "peak_evidence.json")}}}}})
    p = subprocess.run([sys.executable, str(HERE.parent / "report/facts_gate.py"), "build",
                        "--case-dir", str(root)], capture_output=True, text=True)
    assert p.returncode == 0, p.stdout + p.stderr
```

RED：基线 facts `entities: {}` 与三账 e1 脱节——改后 `t_b2`（`:1246-1252`）绿例即证明新闸放行 ledger-derived facts；另加一条反例到 `:1252` 之后（同 with 块内）：把 `facts.json` 的 e1 `current_raw` 改 `"1"` 后 `gate.run(root, report, profile="new-analysis")` 须含 `"三账重算值不一致"`，再恢复文件（重写回 build 产物）使后续断言不受影响。若该 with 块内后续逻辑依赖 facts 字节不变（如 a5 seal sha），改为**独立 with 块**重建案子做反例。

**C4-c `scripts/tests/test_report_facts.py`（build/derive/闸 单元）**
...
| 9 | 闸绿例 | build 后 `errors=[]; gate.check_facts_vs_ledgers(root, facts, errors)` → `[]` |
| 10 | 闸拒手改 | facts e1 current_raw "90" → errors 含 "e1"；删 provenance → 含 "provenance"；mode 改 exploration → 含 "exploration"；改 identity_gate total_supply_raw 为 "2000"（不重 build）→ errors 非空（重算 token 不一致或 provenance sha 不一致） |
...
- 登记 `code_change_pending.md`：P11＝闸只在 facts.json 在场时验，在场性依赖 A4 MANDATORY_SEAL_FILES＋A5 seal 重验；P12＝producer.sha256 只记录不比对（facts_gate.py 升级后旧 facts 不强制重 build）；P4 保留（override 数学正确性不验）；APU 0801 再发布须补 state_source.facts_inputs 重 build。
