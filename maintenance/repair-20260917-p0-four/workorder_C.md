# 工单 C（v4）：R07 facts.json 由三账自动生成＋发布闸/收口重算 —— repair-20260917-p0-four 第三段
...
- RED：基线 symbol=123 被 `str()` 吞、metrics 列表/字符串项直通、dual_basis "x" 静默丢弃 → `reject` 抛 "derive_facts 应拒绝该输入"；decimals "18" 基线已拒（文案含 "decimals"）——该变体基线即绿，记为回归例。
