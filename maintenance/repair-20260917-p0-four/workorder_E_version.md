...
## §1 目标
VERSION（权威）= `pyproject.toml` `version` = CHANGELOG 最新条目 = `SKILL.md:23` 注释 全部为 `7.2.0`；CHANGELOG 新增索引一行与详细条目一段，体例与 7.1.3/7.1.1 一致；`test_version_consistency` 绿、`wc -c SKILL.md` 改前后相等（8021）；`changelog_lint`、`docs_lint --all` 因读取 `archive/` 由调度方本机跑（施工方不跑）。
...
- `python3 -B scripts/tests/test_version_consistency.py` exit 0；`wc -c SKILL.md` 改前后均 8021。**不跑** `changelog_lint`、`docs_lint --all`（读 archive/，调度方本机跑并记 75→76）、不跑 run_all（调度方已跑 151/151 并填入 §2）。
