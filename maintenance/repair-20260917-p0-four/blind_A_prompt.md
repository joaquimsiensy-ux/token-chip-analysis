# 盲审 A（r2：r1 唯一 finding A-01＝调度方把 B 段复核提示词与 A 段施工打进同一 commit，属打包偏差非代码缺陷；已拆分为两个 commit，本轮审查范围 f710fad..212ede1 只含 A 段四个白名单文件；r1 报告见 blind_A_reply_r1.md，其余项 r1 已判通过，本轮确认范围修正并复核无新问题） 提示词（只读，施工后独立复核）
...
## 范围
- 审查对象：`git diff f710fad..212ede1 -- scripts/`（施工 commit 范围，由调度方填入）；工单 `maintenance/repair-20260917-p0-four/workorder_A.md`（v4）；施工报告 `A_done.md` 与 `A_red_evidence.txt`。
...
c) **测试真实性**：`A_red_evidence.txt` 里的 RED 是否与基线代码行为一致（可用 `git stash` 以外的方式：`git show f710fad:scripts/report/figures_from_facts.py` 读基线源码静态推演）；十三个用例的断言是否真的区分改前改后；有无为了变绿而弱化的断言。
