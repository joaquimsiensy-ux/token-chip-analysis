# 工单 D（v3）：R09 日级峰值闸——rglob 定位＋补算覆盖收据＋`replay_duck --only-addrs` 生产者 —— repair-20260917-p0-four 第四段
...
- 0.7 先红后绿：D4 新用例改动前先跑取基线结果写 `D_red_evidence.txt`（逐例独立、逐例捕获 AssertionError；按各例**真实基线**如实记录 RED 或 GREEN→GREEN 回归例，不制造 RED），改后取 GREEN。
