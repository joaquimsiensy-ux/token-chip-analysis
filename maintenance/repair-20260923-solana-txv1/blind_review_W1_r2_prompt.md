# 盲审任务 r2：W1+W2 修复（分支 fix/solana-txv1 vs main）

纪律：**只读**，不改文件、不 commit；禁读 `~/.codex`（启动自动披露除外）、`~/Documents`、`~/Desktop`。报告**作为最终回复文本直接输出**，首行 `# 盲审 W1 r2: PASS / FAIL`。
...
补充（r2 专项，仍不读工单/施工报告）：上一轮独立审查者曾指出「认领后的续跑与深验复验路径未复查采纳行 slot 序列是否为候选前缀、`adopted.source` 未绑定到 `pending-<predecessor_plan_digest>`」。本轮须**自己在内存里复现**这两类篡改（候选 `[10,20]` 台账行排成 `[20,10]` 声明 rows=1；source 改空串/错误目录名）并确认生产者 `load_resume_slots`/`_verify_adopted_record` 与深验 `validate_repair_bundle_deep` 均拒绝、未篡改产物仍通过。
