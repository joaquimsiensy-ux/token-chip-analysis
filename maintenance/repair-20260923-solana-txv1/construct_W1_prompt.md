# 施工任务：按工单 W1 施工（分支 fix/solana-txv1）

纪律（优先级高于工单正文，逐条遵守）：
1. **禁读 `~/.codex` 下任何文件**（插件启动自动披露的一次除外，之后不得再读）；禁读 `~/Documents`、`~/Desktop`。
2. 开工先 `git status --short` 必须为空、`git branch --show-current` 必须是 `fix/solana-txv1`；不是就停工写报告。`.staging_b3/` 是被 .gitignore 忽略的测试夹具，不算脏。
3. 工单里任何行号/断言与代码实况不符：**停工**，把不符点写进 `maintenance/repair-20260923-solana-txv1/W1_done_stopped.md`，不要猜着改。
4. 只改工单 §0.3 白名单文件；白名单外需要改动时，停工提问（写进报告），不要自作主张。
5. 边做边 commit：每完成一节（§1、§2、§3、§4、§5）一个 commit，message 中文、前缀 `W1(n):`，结尾附 `Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>`。**不 push**。
6. 先红后绿：红证据先落 `W1_red_evidence.txt` 再改代码。
7. 沙箱离线：不要访问网络；定向测试按工单 0.7 清单跑，尾行贴进 `W1_done.md`；不跑 `run_all.py`。测试带 `MPLCONFIGDIR=$HOME/.matplotlib`。
8. 完成后 `W1_done.md` 必含：改动清单（文件:行）、与工单差异及理由、定向测试尾行、红证据路径、是否读过禁读路径的自报。
9. 施工中如需向调度方提问，把问题写在 `W1_done.md` 末尾「待裁决」段并停下，不要空转。

工单全文如下：

# 工单 W1 v5 — Solana 交易版本上限常量化 ＋ 修复产物「前代认领」机制（→ 9.1.0）

> v4→v5 变更（吸收 codex 复核 r4 `review_W1_r4_report.md`）：①§4⑫ EXDEV 注入**限定于旧 pending 证据→目标 evidence 的链接**（包裹原 `os.link`，其他调用原样转发；`receipt_kernel.publish_exclusive:581` 内部暂存→最终名的链接必须放行），重试时保留该限定注入、只撤销首次证据发布失败注入，并断言重试确实走了原子复制且哈希一致；②措辞：v1 断言写明字段位置、合成向量指定 `coverage=[10,20]`/`beta=[20,30]`、共享 inode 说明限定为硬链接成功的采纳证据。
> v3→v4 变更（吸收 codex 复核 r3 `review_W1_r3_report.md` 5 条）：①EXDEV 回退改为 `publish_exclusive + RawBytes` 原子发布证据并复核哈希，去掉 shutil；提交台账前先 fsync evidence 目录；②§4 明确两行来源构造、⑩只对目标 ledger 注入失败，补提交后普通 resume 恢复向量与复制中断重试向量；③v1 测试包裹 `RepairFixtureTransport.call` 断言后继续调原方法，深拷贝新拉取交易；④摘要一致性向量覆盖非空 beta 并集与当前/前代两个 sha；⑤validator 锚点改 `:1456-1457`，「状态不变」限定于来源与目标 pending，允许 ERROR 回执与不可捕获中断后的暂存残留。
...
- 新增 import：`from producer_history import historical_producer_hashes  # noqa: E402`（LIB 已在 `:20` 进 sys.path）、`import errno`；`:26`（锚 `from receipt_kernel import publish_exclusive, publish_overwrite`）加 `RawBytes`。
...
6. 提交阶段：逐份 `os.link(src, dst)`（dst 已存在且相等则跳过）；仅 `OSError` 且 `errno == errno.EXDEV` 时回退**原子复制** `publish_exclusive(dst, RawBytes(src.read_bytes()))`（暂存后硬链接发布，不留半文件）并复核 dst sha256 == src sha256，其他 `OSError` 原样抛出。全部证据就位后先 `_fsync_dir(pending / "evidence")`（照 `_persist_live_slot:856` 的做法，同步 pending 不等于同步子目录项），再用**原子发布**写台账：`publish_exclusive(pending / "rpc_ledger.jsonl", RawBytes(_jsonl_bytes([new_header] + 采纳行)))`（`receipt_kernel.py:574-587` 先暂存 fsync 再硬链接发布），再 `_fsync_dir(pending)`（`:196`）。`_jsonl_bytes` 重新 canonical 编码：**行字段值不变、字节可不同**。
7. 恢复边界：台账提交前中断 → 目标 ledger 仍不存在，重跑同一 `--resume --adopt-pending` 命令即可（证据 link/原子复制幂等）；提交后中断 → 改用普通 `--resume`。恢复判断依据最终 ledger 是否存在，不依据函数是否正常返回。不可捕获中断（SIGKILL/掉电）后 `publish_exclusive` 的 `.tmp.*` 暂存可能残留，不影响上述判断。
8. 硬链接边界（文档照写）：**硬链接成功的**采纳证据在新旧 pending 间共享 inode（EXDEV 复制出的文件不共享）；发布后 evidence_manifest 深验（`solana_exact_validate.py:1357-1361` → `_repair_ref:892-904` 重算大小与哈希）能发现任何一侧的后续改写，但不能隔离它——本流程及任何后续流程**禁止原地改写已链接证据**（只允许删除目录项或原子替换）。
...
- 造旧 pending（两种来源）：单行来源＝照 E27(a) 跑到配额 exit 3 留一行；**两行来源**＝三候选 slot、前两项成功、第三项配额停止（`repair_slot_responses` 的 `quota=True` 注入）得到两行 pending。之后改名 `pending-<previous digest>`、header plan_digest 改前代值、各行 params_digest 改版本 0 模板 digest；各向量从来源**独立复制**后再改。**已采纳 slot 的响应不得含 version 1 交易**（历史版本 0 成功证据前提）。
...
- **独立验证版本 1 行为**：新拉取的第二个 slot 的 fixture 响应含一笔 `version: 1`、`message.transactionConfig` 的交易（**深拷贝**后再加字段，勿污染旧版本成功证据用的交易对象；`repair_slot_responses:229-263` 与 `RepairFixtureTransport:72-79` 会原样保留这两个字段）；包裹 `RepairFixtureTransport.call(self, kind, body)`：对实际 `reference-getBlock` 调用断言 `body["params"][1]["maxSupportedTransactionVersion"] == 1` 后**继续调用原方法**，并断言确实观察到第二个 slot 的请求；新行 params_digest 与一个**测试内显式写定**（不经 `_rpc_body`）的完整版本 1 body digest 相等。
- 残尾只读：旧台账追加残缺尾行（如 `{"seq":`）后认领仍成功，认领前后旧 ledger 字节哈希不变。
- 最长前缀（用两行来源的独立副本）：首行对齐、第二行 `.ref.json` 的 `raw_response_sha256` 改坏 → 只采纳首行（rows=1），其余 slot 由 fixture 补拉。
- 入口负向（断言「失败前后**旧来源目录与目标 pending 内**的产物文件不变」；CLI `main:1629-1634` 经 `publish_error_receipt:1561-1577` 在 repair parent 写 `ERROR-*.json` 属正常行为，不纳入比较）：①**未登记 producer**：取一个确认不在 ACTIVE 集的 64hex sha，代入当前 plan 算 digest，同步更名旧目录与 header → 拒；②旧 header.reference.endpoint_fingerprint 改动 → 拒；③目标 ledger 已存在（只有 header）→ 拒，且该 ledger 字节不变；④旧 `.ref.json` `raw_response_sha256` 改坏（单行）→ `no adoptable prefix`；⑤不带 `--resume` → 拒；⑥旧目录拷贝到别的 parent → 拒；⑦旧 header 自带 `adopted` → 拒；⑧候选集外 slot：同步改该行 slot、params_digest、两份证据文件名及其内 slot 使身份检查全部对齐，再断言候选前缀检查拒绝。
- 预验/提交边界：⑨（两行来源独立副本）目标 evidence 里预置与旧文件不等的第二个采纳 slot 的 `<slot>.sqd.json` → 预验拒绝且第一个 slot 的文件未被 link；⑩提交前中断：来源准备完毕后，包裹 `publish_exclusive` **仅对目标 `rpc_ledger.jsonl` 路径**抛异常、其他路径调原函数 → 目标 ledger 不存在、证据已 link，重跑同命令成功；⑪提交后中断：让真实发布完整提交认领 ledger 后再注入中断（如剩余 slot 配额 exit 3）→ 再次 `--adopt-pending` 被拒（`adopt requires no target ledger`）、普通 `--resume` 成功、已采纳 slot 不重拉（fixture 不含其响应）、`adopted` 与采纳行不变；⑫复制中断重试：来源准备完成后包裹原始 `os.link`，**仅当 src 位于旧 pending 的 evidence 目录且 dst 位于目标 evidence 目录时**抛 `OSError(errno.EXDEV, …)`，其他调用（含 `receipt_kernel.publish_exclusive:581` 暂存→最终名的同目录链接）原样转发全部参数；首次对某证据 dst 的 `publish_exclusive` 注入失败 → 目标无半文件、ledger 不存在；重跑同命令时**保留限定 EXDEV 替身、只撤销发布失败注入** → 成功，并断言确实经过原子复制分支（如计数/标记）且目标哈希等于源。
- 同步篡改边界（明确记为信任边界，不断言必拒）：同时改行 `result_sha256` 与 `ref.raw_response_sha256` 为同一伪值 → 认领通过，注释说明这属于来源可信前提。
- 深验负向（改文件后同步更新 `bundle.rpc_ledger` 的 size/sha256；`compute_gid:90-92` 排除 rpc_ledger 故不必重算 gid）：①`adopted.predecessor_producer_sha256` 改未登记值 → FAIL 含 "adopted record invalid"；②`adopted.predecessor_plan_digest` 改另一 16hex → 同 FAIL；③某数据行 `endpoint_fingerprint` 改动 → FAIL 含 "reference fingerprint mismatch"；④与 core 一致性：对发布 gen 断言 `_plan_digest_from_generation(bundle, resolution, bundle.producer.sha256) == repair.compute_plan_digest(plan)`；另加一条**合成向量**：构造 `plan_candidates.coverage=[10,20]`、`.beta=[20,30]`（有 beta 独有候选）的 resolution/bundle 片段与对应 plan，对当前 sha 与前代 sha 两个值分别断言 `_plan_digest_from_generation == repair.compute_plan_digest`（防漏合并 beta）。

## 5. 文档（中文，只改既有条目）
- `references/scan-schemas.md` §14.8（`:998-1023`）：表内加「`header.adopted` | object | 否 | 可选；存在时下列五子字段全部必填」＋五行 `header.adopted.{predecessor_plan_digest, predecessor_producer_sha256, rows, source, ts}`（说明：认领自同 parent、登记 ACTIVE 前代 producer 的 pending；续跑/发布/深验均重算前代 digest；来源可信是输入前提，目录归属与深验只验证本案归属及一致性；硬链接成功的采纳证据与旧 pending 共享 inode，禁止原地改写）；不变量 `:1021` 改为「resume 以 (plan_digest, params_digest ∈ 同模板显式版本 0..`SOLANA_MAX_SUPPORTED_TX_VERSION` 集, result_sha256) 判完成；header.adopted 存在时其前 rows 行迁自前代 pending，字段值不变、seq 连续」。
