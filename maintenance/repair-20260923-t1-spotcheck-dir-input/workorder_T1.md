# 工单 T1（v2，融合 codex 复核 r1 全部意见）：time_spotcheck 与发布校验器对「v2 目录输入」的收据绑定对齐 —— repair-20260923-t1-spotcheck-dir-input
...
- 0.5 行号均指基线 f4f80567c21f；施工锚使用目标文件**整行原文**，以 `grep -n -F -x` 核验恰 1 处且行号一致，不符**停工**。`time_spotcheck.py:416`（`    try:`）非唯一，以 `:415` 定位；`shared_release_receipt.py:371、377、997` 为非唯一事实引用，分别按 `bound_case_ref`、`_validated_time_plan_authority` 定位。删除 > 修改 > 新增；新增代码只准放在 §2 指定位置。
...
```
  ...
  说明：①`_expect_reject`（`:73-80`）按 needle 子串匹配 ValueError。②`_shared_authority`（`:148-157`）第二参数即 `inputs.input` 所绑文件。③mock 依赖 `main` 通过模块全局名 `build_envelope`（`:56` 导入、`:417` 调用）；`:398-403` 的 rpc/chain/final-block 前置由 argv 满足，`:409` `assert_distinct_paths` 离线；`fake_envelope` 抛出后被 `:422-424` 捕获返回 1，任何 RPC 之前终止、不发网。④自洽重绑一段中 `new_ref["path"]` 用绝对路径是因 `anchor_plan` 收据本身记绝对路径（§2.1 说明①），`plan_path`/`receipt_path` 的 `_ref` 走相对路径不受影响；`_refresh_receipt`（`:131-136`）重签 plan 输出哈希。若 `validate_receipt` 对绝对路径清单引用另有拒收（`receipt_validate._input_file` 以案根解析），把 `new_ref["path"]` 改回原收据记录的形态（读 `receipt["inputs"]["input_manifest"]["path"]` 原值仅换 size/sha）——施工时按实况取其一并在 done 写明。新用例由 `main`（`:522-525`）自动发现，无需登记。

### 2.4 `references/data-pipeline-evm-recon.md`

- 基线 `:158` 以整行原文为锚（整行 326 B，含 `merged input ` 恰 1 处）：
...
- `CHANGELOG.md:13`（整行锚＝以 `- **9.0.2**（2026-09-19）口径漂移与文档-代码不符审计第二期闭环` 起头、以 `改则变采集器 sha）。` 结尾的该整行，唯一）之前插入一行索引：
...
开工基线两项输出；RED 取证三条摘要（指向 `T1_red_evidence.txt`）；每条施工的实际 diff 行号；§0.8 各命令尾行；三处字节前后（含 §2.4 该行 326→326）；`git diff --stat f4f80567c21f -- scripts references SKILL.md commands-staging VERSION pyproject.toml CHANGELOG.md` 全文（对工作树，非 HEAD），并附 `git status --short` 标明本目录新增的报告与证据文件；与工单差异（若有，含 §2.3 说明④取哪一形态）；末尾披露是否读过禁读路径。不 commit。
