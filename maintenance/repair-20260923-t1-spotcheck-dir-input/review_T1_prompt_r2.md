...
2. 只读、离线、不 commit、不新建/不修改任何文件。报告全文打印到 stdout，首行固定：`# 工单T1复核r2：通过` 或 `# 工单T1复核r2：退回`。
3. 工作目录＝`/Users/uravvv/.claude/skills/token-chip-analysis`，基线＝HEAD（含 f4f80567c21f）。

## 任务
复核 `maintenance/repair-20260923-t1-spotcheck-dir-input/workorder_T1.md`（**v2**，已融合 r1 意见；r1 报告在同目录 `review_T1_reply_r1.md` 可读），逐条审：
g) **r1 吸收核对**：r1 每条退回意见在 v2 是否被正确吸收（含整行锚、文件分支顺序、helper 私有化与 try 复用、0 B 文档替换、RED 三项定义、`_produce_plan(directory=)` 改造、消费者篡改与自洽重绑负例、diff 命令、三处措辞）；v2 新写的 §2.3 mock 接线用例与"自洽重绑"用例在代码事实上是否成立（`main` 是否用模块全局名 `build_envelope`；`receipt_validate` 对绝对路径清单引用是否拒收，即 §2.3 说明④哪一形态成立）。
以下 a)–f) 为 r1 原审项，按 v2 重审：
