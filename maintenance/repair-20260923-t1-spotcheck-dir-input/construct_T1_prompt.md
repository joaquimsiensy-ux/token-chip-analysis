# 施工任务 T1（codex --write，按下方工单 v4 逐条执行）

## 派工基线
- 派工基线：main 分支、HEAD 为包含本提示词文件的最新提交（本提示词入库后 HEAD 才定，故**不以具体 SHA 判定**）；基线判定只看工单 §0.1 两项检查：`git status --short` 为空，且 `git diff --stat f4f80567c21f HEAD -- scripts references SKILL.md commands-staging VERSION pyproject.toml CHANGELOG.md` 为空。工作目录＝`/Users/uravvv/.claude/skills/token-chip-analysis`。开工先 `git rev-parse HEAD` 记入完成报告。
- 本任务是**施工**，不是复核：按工单 §2 逐条落地、按 §0.7 先取 RED、按 §0.8 跑定向测试、按 §3 写完成报告 `T1_done.md` 到工单所在目录 `maintenance/repair-20260923-t1-spotcheck-dir-input/`。
- 纪律以工单 §0 为准（禁读 `~/.codex/`——插件启动搜索若已读 memories 披露一次，之后不再读；白名单；不 commit/push；禁 stash/checkout/reset；锚不符即停工）。工单已由 codex 只读复核通过（`review_T1_reply_r3.md`），施工中若发现工单与代码不符，**停工写 `T1_done_attempt1_stopped.md`**，不得自行改方案。
- 本次为 **attempt 2**（attempt1 按纪律停工于 §0.7，原因已勘误进工单 v4 头部；停工报告 `T1_done_attempt1_stopped.md` 与 `T1_red_evidence.txt` 已入库可读，第②③项 RED 证据可沿用并注明）。停工时报告名用 `T1_done_attempt2_stopped.md`。
- stdout 首行固定 `# 施工 T1：完成` 或 `# 施工 T1：停工`；末尾披露是否读过禁读路径。
