...
7. `只读断言`（不依赖退出码）：
   ...
   - **辅助检查**：夹具全部就绪后做案目录文件清单＋sha 快照（`os.walk(followlinks=False)`；符号链接用 `os.lstat` 记元数据、`os.readlink` 记链接文本，不读取目标内容）；随后对案目录内常规文件与目录 `chmod` 去写位（跳过符号链接，记录原 mode），再跑一遍正例命令均 exit 0；`finally` 按记录恢复原 mode，恢复后再做一次快照比对完全一致。
