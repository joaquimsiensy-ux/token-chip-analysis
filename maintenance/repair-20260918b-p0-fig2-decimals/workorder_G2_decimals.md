# 工单 G2（v1）：EVM decimals 绑定链上观测——observation bundle v2 补 decimals()，闸侧两链族统一读观测 —— repair-20260918b-p0-fig2-decimals 第二段
...
- 0.2 **禁读** `~/.codex/`；禁读 `archive/`、`blind-reviews/`、`.staging_*`、`references/attic.md`、`maintenance/repair-20260918-p0-f04-f07/` 与本目录以外的历史 maintenance 目录；禁读 `/Users/uravvv/Desktop`、`/Users/uravvv/Documents`。
...
- 1.3 旧 v1 bundle **不得**被任何消费者接受：`validate_evm_observation_bundle` 对 schema≠v2 抛 ValueError；`accounting_gate`/`supply_truth_gate`/`shared_release_receipt` 三处 schema 字面量同步 v2（`grep -rn 'evm-observation-bundle/v1' scripts` 改后须为 0 命中）。
...
- `:1356`（锚 `            _require(bundle.get("schema") == "evm-observation-bundle/v1",`）→ v2；`:1761`（同款锚，第 2 处，位于 `validate_accounting_receipt`）→ v2。
...
`:1586-1611`（起锚 `def check_facts_decimals(case_dir: Path, facts, accounting, errors: list[str]):`，止锚 `        errors.append(f"facts.token.decimals={declared!r} 与链上观测 {observed} 不一致——state_source.facts_inputs.decimals 填错")`）整函数替换为：
...
def check_facts_decimals(case_dir: Path, facts, accounting, errors: list[str]):
    ...
    if isinstance(observed, bool) or not isinstance(observed, int):
        errors.append("facts.token.decimals 无链上观测来源可核（accounting_mode.checks.decimals 缺失）")
        return
...
说明：调用点 `:1910` 不动；`_validate_reconciliation_report_once`/`_bound_json_input`/`chain_family` 均为 7.2.1 已用接口。
...
- `test_evm_observation_nonempty_code.py:102`（锚 `    transcript[6]["result"] = "0x"`）→ `[7]`；`:116`（锚 `    transcript[6]["params"] = [TOKEN, hex(AS_OF)]`）→ `[7]`（ContractPool 对一切 eth_call 返 ZERO_WORD → decimals 0，合法）。
...
- `test_review_20260804_p105.py`：在 F05 decimals a/b 两例旁新增 c 例（照 a/b 写法）：复现 review 反例——`fixture_recon_config.json` 改 `decimals=2, total_supply_human="1"`，按 a/b 既有方式重算 balance/supply 收据 `inputs.config` 的 size/sha 与 wrapper sha、`create_bundle(root)`；facts 与 accounting 仍 0 → `gate.run(root, report, profile="new-analysis")` errors 含 `config.decimals` 且含 `与链上观测 0 不一致`（RED：基线 `[]`——review 附录 C 已实证）。若该文件没有现成的"改 config 并重签收据"辅助，允许在本文件内新增私有 helper。
...
- 改后 `grep -rn 'evm-observation-bundle/v1' references SKILL.md commands-staging scripts` 须 0 命中；`docs_lint.py --all` 与契约路由守卫 PASS。
...
①0.1 两条命令输出；②§2 各处 `git diff` 原文（按文件）；③RED 摘要；④0.8 各测试结果尾行（含未能跑的项及原因）；⑤1.1 三个字节数；⑥`git diff --stat`；⑦`grep -rn 'evm-observation-bundle/v1'` 改后输出（须空）；⑧与工单差异/停工点（含 §2.8 各"开工核实"结果）；⑨禁读披露。stdout 首行 `# 施工 G2：完成` 或 `# 施工 G2：停工`。

## 4. 登记不修（`code_change_pending.md` Q5–Q8，调度方维护）

- Solana 侧 checks.decimals↔bundle 相等性另单；非标准 ERC20 在观测阶段 FAIL 属目标行为；存量 EVM 案 v1 bundle BLOCK、迁移＝重跑观测三件＋下游重建；版本档位（v2 schema＝不兼容变更）待用户裁决。
