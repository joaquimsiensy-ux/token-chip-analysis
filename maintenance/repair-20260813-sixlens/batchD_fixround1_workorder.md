...
- 剩余可绕路（N-D2 复核实测后**表述改准**，不得强于实现）：切片内三项仍是独立子串搜——作者在被收据点名的章节里**堆砌三项串但不作真实披露**（如"否认性术语表"写法、location 用通用词命中任意附录）可以过闸，属 F-12 已接受边界同族（validator 是一致性校验器不是真实性证明器）；locations 是冻结的裁决内容（sha 三处咬死），第三方改不了，剩余主体是报告作者本人蓄意伪装。登记见 batchD_ledger 二d。
...
## 收口补丁（盲审终判"可收口"后点名并轮小修：N-D1 必修＋N-D3 顺手＋N-D2 登记）

### N-D1（P2）披露策略名判据改中英文别名族——中文真实披露不再误伤

- **改法**：别名族常量落 `handoff_manifest.FLIP_POLICY_ALIASES`（三处消费同源）：`pro_rata`｜按比例、`fifo`｜先进先出、`lifo`｜后进先出；A5 切片判据改为命中任一别名即该策略在场，报错文案带可写别名指路。位置锚/终点标识/份额三项判据零放宽。
- **测试**：绿例①纯中文披露（盲审变体 D 原文形态——"按比例口径主导终点为 …（50.00%）"）→ DISCLOSED；绿例②中英混排 → DISCLOSED；绿例③既有英文用例零回归（原 good_text 继续在场）；回归红例＝无关附录攻击**仍拒**（别名族不放宽位置锚）；新红例＝中文策略词在场但缺份额仍拒（别名不吞掉份额半边）。
- **文档**：`report-template.md` 补披露章节写法一整句（标题含 locations 位置串＋同段三策略中或英＋终点标识＋两位小数份额，"写在别处不算数"）；`scan-schemas.md §4a` 契约同步别名族。

### N-D3（P3）--report 硬性要求入工作流文档

`analyze-workflow.md` 报告入口段补：new-analysis 发布闸必须带 `--report`（A5 seal 在发布闸内重验，缺 report 时 A5 在场即 fail-closed 拒）。

### N-D2（P3）切片内残余绕路——登记＋表述改准

工单 F-D1 段"那这段事实上就是披露本身"一句已按复核实测改准（否认性术语表/通用词 location 两例可过＝一致性校验器已声明边界的实例，剩余主体是报告作者本人蓄意伪装，F-12 同族）；全文登记入 `batchD_ledger` 二d，含"再收一层"的方向（披露段结构化标记块，R10 设计面）。
