...
- **复核对象**：`06f5784`（基线 `70096b4`）——止损轮，只修 N-C4，工单 `batchC_fixround3_workorder.md`，代码净 +24 行
- **复核方式**：只读＋副本变异，副本 `/private/tmp/batchC_probe/repo4/`（`06f5784` 钉死态）。生产树零改动
- **基线核对（独立复跑）**：`run_all.py` **EXIT=0（"全部通过"）**；`test_repair_batch_c.py` **rc=0，115 checks**（112→115 属实）；`invariant_scan.py` **rc=0**（54/61/45，零变更与工单一致）
- **结论**：**N-C4 CLOSED，无实质新 finding，批 C 可收口。** 剩余残余的"F-12 同族"定性**成立**，但施工方与 `scan-schemas.md` §13 里"伪造整案原始数据后**真跑 producer**"这半句**描述不准确**——实测证明不需要跑 producer，手写一组互相自洽的案内小件（**1470 字节**）即可。这是文档口径更正（台账级），不是闸的缺陷

| 编号 | 判定 | 一句话 |
|---|---|---|
| N-C4 | **CLOSED** | 我轮 2 的攻击 C'（自造原生文件＋state 同步一致＋绑定块自填）转拒于"序列缺 provenance sidecar"；施工方自报的强化变体（sidecar 也自造、输入全绑案内真件、登记面全过）转拒于"末点对账失败"，独立验证**属实**；producer 交叉撕裂另拦一道（超出我给的修法，加分）；误伤三查全过；接线"删掉即红"成立 |

---

## 1. N-C4 终判：攻击重放与最深变体

`check_series_binding` 在重转换比对之后，用编译期同三件纯函数做发布期复算（`load_series_with_sidecar` → `registry_anchor_check` → `endpoint_reconcile`），另加"绑定块 producer 必须与磁盘 sidecar 实物一致"的交叉。实测（单元级直调）：

| 场景 | 结果 |
|---|---|
| **攻击 C'**（轮 2 原样：自造 `camp_series_v2.json`＋state=其转换结果＋绑定块自填，案内无 sidecar 实物） | **拦下**："发布期来源链复算失败：序列缺 provenance sidecar" |
| **强化变体 v1**（sidecar 用公开函数自造、camps_spec／final_balances／replay_stats 全绑案内真件、supply_truth＋preflight 齐全→登记面全过，但序列末点是伪造的） | **拦下**："末点对账失败：「项目方」末点 88.8000% ≠ 重算 10.0000%" |
| 绑定块 producer 与磁盘 sidecar 撕裂 | **拦下** ✓ |
| **★强化变体 v2**（连 `balances_final.json` 也自造，使末点对账自洽——全链七件全部手写且互相自洽） | **★ 放行**（案内伪造件合计 **1470 字节**，**没跑过任何 producer**） |

**误伤三查（裁判点名）独立确认**：①formal 正常产物（案内 sidecar／supply_truth／preflight 全链在场）发布期复算**零 error 放行** ✓；②旧简报型 state（无 `camp_share_series`）**不强加** ✓；③exploration 标记产物**按设计拒** ✓（非误伤）。`test_repair_batch_c` 115 checks 与 run_all 全绿佐证 p105／a4_gate 两处 new-analysis 夹具未被新链波及。

**接线变异法独立重放**（清 `__pycache__` 后）：把三件套整段中和（在 `load_series_with_sidecar` 前插 `return`）→ 攻击 C' 由"拒"转"放行"，**删掉即红成立** ✓，副本逐字节还原。

## 2. 剩余残余定性：认，但措辞要更正

施工方定性：**"此链上不再有可机器闭合的第四层，剩余残余＝F-12 已接受边界同族（保真末点改中间＋伪造整案后真跑 producer）"。**

**我的认定：主体成立，一处措辞必须改。**

**成立的部分**——我逐个查证了"下一层锚"的候选，都不构成可低成本机器闭合的新防线：

- **加验 `channels_preflight.json` 自身的收据链**：preflight 实物确带 `inputs` 三验段，但其 inputs 指向 `blocks.parquet`（24MB 采集原始数据）。三验只比对 path/size/sha，而 sha 是攻击者自己写在 preflight 里的——造一个任意内容的同名文件把 sha 填对即可，成本仅增几十字节。**不是真防线。**
- **交叉 `identity_gate.json`**：实物核查其顶层键为 `schema／chain／share_basis／total_supply_raw／snapshot_binding／state_file／state_sha256／n_addresses／n_flags／rows`——**全是案内自洽字段，不含任何 RPC 会话证据或链上原始证据**。把它拉进序列链只是多一个可伪造的自洽件。**不是真防线。**
- 真正能把成本推上量级的只剩两条路：发布期**重跑 producer**（要重放亿级边数据），或把序列链锚到**案外不可伪造的证据**（真实 RPC 响应／链上数据）。前者工程上不现实，后者属案身份体系（F-12 的地盘）而非"序列来源绑定"的范围。

因此**"此链上不再有可机器闭合的下一层"在'成本可接受'的意义上成立**，F-12 同族的归类**成立**——两者残余都是"一致性校验器无法证伪一组自洽的伪造输入"。

**必须更正的措辞**：残余**不是**"伪造整案后**真跑 producer**"。v2 实测表明：**手写 7 个互相自洽的小 JSON（camps／balances_final／replay_stats／supply_truth／channels_preflight／camp_series／sidecar，合计 1470 字节）就够，producer 一次都不用跑。** 这与我轮 0 抓 F-C3 时的问题同型——**文档宣称的强度高于实际强度**，将来会被引用成"要伪造得跑一遍重放"，从而高估这条边界。

**建议改口（一句话，台账级）**：`references/scan-schemas.md` §13 与批 C 工单里把"剩余残余＝伪造整案原始数据后真跑 producer"改为——

> 剩余残余＝**控制案目录的伪造方手写一组互相自洽的案内小件**（阵营 spec／终态快照／replay_stats／supply_truth／preflight／序列／sidecar，实测约 1.5KB，无需运行 producer），或保真末点只改中间点。发布期复算是一致性校验器，不是真实性证明器；要再进一步只能锚到案外链上证据（属案身份体系，见 F-12）。

## 3. 为什么 v2 不算实质新 finding（划线依据，供裁判复核）

裁判定的规矩是"任何实质新 finding 直接触发 3/3 冻结"，所以这条线我逐条说明：

1. **它不揭示任何闸的实现缺陷。** 三件套在 v2 里**全部按设计正常工作**：sidecar 实物在场检查过了、登记面锚过了、末点对账过了。没有条件式跳过、没有静默放过、没有分支串台——这与我前三轮抓的 F-C1／N-C1／N-C3 有本质区别（那些都是"闸本该拦却没拦"）。
2. **它落在本批从主施工轮起就明确声明的边界内。** 主工单①栏原文"validator 是一致性校验器不是真实性证明器"，与 F-12 已接受边界同款残余——这个声明三轮未变，v2 只是把它的实际成本量化出来。
3. **可执行的收口动作只有文档一句话。** 没有代码可改（改了也只是多一个可伪造的自洽件，见第 2 节的三个候选查证）。凡"唯一动作是改措辞"的，按批 B 以来的惯例归台账级。

**沿用轮 2 的台账项**（已由裁判裁决入 R10，本轮不扩面）：N-C5 的两条——`target.as_of_block` 只验正整数无真实对锚（改成任意别的正整数照过，实测）；sol 侧 `solana-reconcile/v2` 收据 schema 无身份键（`replay_edges.py:166` 实物核实，施工方诚实边界如实）。**补一条**：若将来按 R10 补 sol 侧身份键，需同步把本轮的发布期复算在 sol 分支上验一遍（本轮实测只覆盖 EVM 链路，sol 分支的发布期复算路径未有真实案端到端跑过）。

## 4. 复现件清单（轮 3）

`/private/tmp/batchC_probe/repo4/`（`06f5784` 完整副本，复核期零改动，变异后逐字节还原）＋会话内一次性脚本：`check_series_binding` 七场景（攻击 C'／强化变体 v1／★v2／producer 撕裂／误伤三查）；接线变异法（三件套整段中和→攻击 C' 转绿）；`identity_gate` 与 `channels_preflight` 实物形态核查。

## 5. 终判

**批 C 可收口。**

三轮消化的账：轮 0 抓 6 条（1 P0／2 P1／3 P2），轮 1 关 5 开 1 ＋新 3 条 P2，轮 2 关 4 ＋新 2 条（P2/P3），轮 3 关 1。**当前全部 finding 归零**，最后一条 N-C4 的修法逐字落地且施工方额外加了一道 producer 交叉；三轮里每一轮的绿例、误伤面、变异法我都独立重放过，没有一次出现"为绿改弱断言"或"闸挂可选参数"的回潮。

收口时须一并落实的两件（均台账级、不阻塞）：
1. **改口**：`scan-schemas.md` §13 与批 C 工单的残余边界表述按第 2 节改为准确版本——不要让"真跑 producer"留在文档里被将来引用成这条边界比实际更强。
2. **R10 台账**：N-C5 两条 ＋ 本轮补的"sol 分支发布期复算未经真实案端到端验证"一条。
