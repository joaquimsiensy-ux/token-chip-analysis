...
c) 白名单：`git diff --stat 2a8ee25d3a8b HEAD` 只含 `scripts/lib/net.py`、`scripts/lib/rpc_batch.py`、`scripts/tests/test_batch1_rpc_attestation.py`（maintenance 目录下的 done/evidence 文件除外）；`SKILL.md` 8021 B、`references/**/*.md` 合计 930076 B、`commands-staging/*.md` 合计 8798 B 不变。
...
e) 既有消费者语义未被改动（工单 §1.4 列的 11 处仍只走各自既有 `ok=False` 分支），新增测试的 RED 是否真能在基线（`git stash` 禁用；请用 `git show 2a8ee25d3a8b:scripts/lib/net.py` 等读基线源码做内存对照）上失败。
