...
| Q13 | F04 | `net._run:355` 按 `(_active_index+offset)%n` 选端点、`:372` 循环内回写 `_active_index`：**三端点**时 A/B 都坏则 C 不被访问（A→B→A）失败；更多端点可能重复访问或漏访，顺序取决于端点数与各次结果（四端点 A→B→D 成功、八端点 A→B→D→G→C 成功）——既有缺陷（基线 A/B 超时同样触发），本段使“缺 result 键”也成为触发条件 | 复核 r1 登记、r2 订正（不泛化为“三端点及以上必失败”）；不在本段修（改 `_run` 扩回归面），待日后单独工单；单/双端点不受影响 |
| Q14 | F05 | `report-template.md:278` 允许“双源都无该币 exit 3 回退人工对 Dexscreener 图”，新契约下 ALL_SKIP 收据不能进 −2 收口。受影响范围（r2 订正）：`price_check.py:140` 每次只选一个第二源、不自动换源；所选第二源在全部抽样日期无有效对照（DefiLlama `:108-110`/币安 `:117-122` 无数据，或 `_get:87-100` 重试耗尽）即 ALL_SKIP 退出 3——**四条正式候选链 ETH/BSC/Base/Solana 均可能，且不限币龄**；人工比图不能满足 PASS/WARN 收据契约。Robinhood 类探索链本就不进正式发布，与本轮新增影响无关。不扩大为“任何 SKIP 即拒”（部分 SKIP 仍可汇总 PASS/WARN）；缺 `--binance-symbol` 在 `:143-145` 退出 1 无收据，不属 ALL_SKIP | 本轮按严格口径实施；**待用户裁决**是否另立小工单开人工旁证口子（需新增字段） |
| Q15 | 环境 | 本机 macOS 27.0 上 `system_profiler -xml SPFontsDataType` 稳定返回空 plist，matplotlib 3.11.0 `_get_macos_fonts`（font_manager.py:275）取 `d["_items"]` KeyError——凡从零重建字体缓存必炸；`test_a4_gate.py:307`/`test_distribution_chart.py:167`/`test_batch15_three_ledgers_frozen.py:18` 把 MPLCONFIGDIR 指向临时目录，故 test_stage2_closeout/test_a4_gate 等本机裸跑 BLOCK（基线 8b041842 同样复现，与本轮改动无关；codex 沙箱大概率拦了该命令走空列表分支所以 PASS） | 环境项不改代码：本机跑测试时 `export MPLCONFIGDIR=$HOME/.matplotlib` 沿用现成缓存即过（0918 实测 29/29、23 项）；run_all 同法；待 matplotlib 修复或系统恢复后撤 |
