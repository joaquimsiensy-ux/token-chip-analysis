# 工单 R3：口径漂移与文档-代码不符修复（盲审 R3 消化）v1
...
## §1 硬约束
1.1 字节：`SKILL.md` = 8021、`commands-staging/*.md` = 8798 不变；references 三组 glob 合计 ≤ 929880（基线 929831；本轮三处均为补正代码事实、无可删冗余，允许净增 ≤ 49 B，实测数写入报告）。
...
`references/environment.md:18`。锚：`只从 `~/.config/helius/api-key` 读取` → `默认读 `~/.config/helius/api-keys`（key 池，每行一 key），无可用 key 才回退 `~/.config/helius/api-key`；显式 RPC/keys 文件参数可覆盖`（依据 `scripts/solana/sqd_gap_repair.py:46-47,122-133`）

### D2 slot-index-map 不变量误要求 signature 列单调递增
`references/scan-schemas.md:873`。锚：`三列各自唯一双射且单调递增。` → `三列各自唯一；行按 `sqd_index` 递增。`（依据生产者 `sqd_gap_repair.py:1094-1112` 按 index 排序、消费者 `solana_exact_validate.py:1558-1560` 只查各列唯一与首列有序）
