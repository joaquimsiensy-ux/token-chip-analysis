# 施工 R3 提示词：按 workorder_r3.md v2 施工（写模式）
...
2. 工单全文＝`maintenance/repair-20260916-drift-audit/workorder_r3.md`（v2），逐字执行其 §0–§3。§0.1 的内容基线校验命令须实际运行并把输出贴进 r3_done.md。
...
6. 完成后：把 §3 要求的完成报告写到 `maintenance/repair-20260916-drift-audit/r3_done.md`，并在 stdout 首行打印 `# 施工 R3：完成` 或 `# 施工 R3：停工`，随后打印 `git diff --stat` 与 §1.1 三个字节数。
