# 盲审 R3 提示词：token-chip-analysis 口径漂移与文档-代码不符审计（只读，第三轮）

本轮是第三轮。前两轮报告在 `maintenance/repair-20260916-drift-audit/blind_r1_report.md`、`blind_r2_report.md`，消化工单 `workorder_r1.md`（v3）、`workorder_r2.md`（v2）均已施工落地于当前 HEAD。**已在 R1/R2 报告或工单中出现并已修复的条目不必重报**；请找前两轮没发现的新问题，以及 R1/R2 修复本身是否引入了新的不一致（若有，明确标"Rn 修复引入"）。若确实找不到，首行写 `# 盲审 R3：0 条发现` 并给出覆盖声明——不要为了凑数把范围外问题（结构/门禁有效性/代码质量/预算等）或措辞偏好写成漂移。`code_change_pending.md` 记录的 D1 属已知待决，不必重报。
...
3. 报告全文**打印到 stdout**。首行固定格式：`# 盲审 R3：N 条发现`（N 为整数，0 也要写）。
...
覆盖声明：审过的文档清单；术语表条目数；grep 策略概述；本轮相对 R1/R2 新增的核对方向
