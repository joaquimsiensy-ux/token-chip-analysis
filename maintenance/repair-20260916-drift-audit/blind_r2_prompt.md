# 盲审 R2 提示词：token-chip-analysis 口径漂移与文档-代码不符审计（只读，第二轮）

本轮是第二轮。第一轮报告在 `maintenance/repair-20260916-drift-audit/blind_r1_report.md`，其消化工单在 `workorder_r1.md`（v3），已施工落地于当前 HEAD。**已在 R1 报告/工单中出现并已修复的条目不必重报**；请找 R1 没发现的新问题，以及 R1 修复本身是否引入了新的不一致（若有，明确标"R1 修复引入"）。`code_change_pending.md` 记录的 D1 属已知待决，不必重报。
...
3. 报告全文**打印到 stdout**。首行固定格式：`# 盲审 R2：N 条发现`（N 为整数，0 也要写）。
...
覆盖声明：审过的文档清单；术语表条目数；grep 策略概述；本轮相对 R1 新增的核对方向
