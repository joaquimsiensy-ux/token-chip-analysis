# 工单 R2：口径漂移与文档-代码不符修复（盲审 R2 消化）v1
...
0.2 禁读 `~/.codex/`（启动搜索若已读 memories 如实披露一次，之后不再读）；禁读 `archive/`、`blind-reviews/`、`.staging_*`、`references/attic.md`（含 `wc`/`cat` 通配——统计字节用 `stat` 或 `wc -c` 逐文件，不得让 attic.md 进入任何读取管道）。
...
## §1 硬约束
1.1 字节：`SKILL.md` = 8021 不变；references 三组 glob（`references/*.md references/casebook/*.md references/labels/*.md`）合计 ≤ 929850（基线，须净减或持平，统计用 `stat -f %z` 逐文件求和）；`commands-staging/*.md` = 8798 不变。
...
### D1 V4 手续费公式把输出腿当输入腿
`references/lp-fee-accounting.md:62`。锚：`gross_input_j = 正数一侧的 amount0 或 amount1` → `gross_input_j = 用户付入池子一侧（池流入腿）的绝对值，方向按同 tx Transfer 校准（data-pipeline-robinhood-traps 第 12 条：正数=池流出）`

### D2 "本案回灌"命令实际跨案批扫
`references/retrospective.md:102`。锚：`——本案庄家实体回灌惯犯库（appendix.json / analysis-state.json 为扫描源，筛查不买入的案子复盘时同样回灌；含 manifest 自动落印）` → `——回灌惯犯库（脚本扫描案根父目录——默认 `DEFAULT_ROOT`、可传根目录参数——下全部案的 appendix.json / analysis-state.json 并跨案合并，不只回灌本案；筛查不买入的案子复盘时同样回灌；含 manifest 自动落印）`

### D3 报价小时线格式
`references/data-pipeline-robinhood-channels.md:34`。锚：`cost_engine 的 quote_usd_hour.json 是 dict——两文件格式不同需各自生成` → `cost_engine 的 quote_usd_hour.json 是 [[ts,o,h,l,c],...] 二维 list——两文件列结构不同需各自生成`
...
`references/scan-schemas.md:17`。锚：`各产物一律在场，下文不再逐一重复` → `在 wave-scan／flow-anomaly 产物一律在场（其他产物以各自 schema 表为准），下文不再逐一重复`
