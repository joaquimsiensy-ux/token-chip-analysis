# 工单 R5：v9.0.1 口径漂移与文档-代码不符 3 条纯文本修复 v1
...
## §1 硬约束
1.1 字节：`SKILL.md` = 8021、`commands-staging/*.md` 合计 = 8789 不变；references 三组 glob（`references/*.md references/casebook/*.md references/labels/*.md`）合计 ≤ 929965（基线 929965；四处整行替换＋两处整行删除按 UTF-8 字面模拟净减 356 B → 929609，实测数写入报告，须不高于基线）。
...
```
依据 `scripts/evm/scan_bloxroute_seg.py:45-47`（`<out>.done.json` 跳过已完成段）、`:96-98`、`:108`（失败段只累进 `fails` 并在 `DONE` 行打印，无补扫）、`:81`（时间戳列写空串，不采锚点）、全文无 `scan_meta`；`scripts/evm/scan_transfers.py:71`、`:165-168`（`<chain>_scan_meta.json` 与锚点采集属旧脚本）。`:209` 起始块定位属通用方法不绑脚本，不动。
