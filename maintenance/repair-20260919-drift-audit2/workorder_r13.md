# 工单 R13：v9.0.1 口径漂移与文档-代码不符 1 条纯文本修复 v1
...
## §1 硬约束
1.1 字节：`SKILL.md` = 8021、`commands-staging/*.md` 合计 = 8789 不变；references 三组 glob（`references/*.md references/casebook/*.md references/labels/*.md`）合计 = 929133（基线 929105；一处整行替换按 UTF-8 字面模拟净变动 +28 B；实测数写入报告，须等于该值。为准确性必要的最小增量）。
...
```
→（只在括注内补「；有漏边则优先 exit 2」，其余逐字不动）
```
- **退出码**：0=抽样零漏边；2=发现漏边（对账 gate 语义，报告 missing_detail 带 tx 级证据）；1=运行失败/样本无效。**样本无效机器判据（批 D GPT-F-06 收口，任一命中即 exit 1；有漏边则优先 exit 2）**：任一 getMultipleAccounts 批失败／深挖账户全部 fetch_failed／checked=0 且 closed>0／墙钟截断／undetermined 过半。
```
依据 `scripts/solana/audit_closed_accounts.py:526-529`（`if events["missing"]: LEAK_FOUND/2` 先于 `elif invalid_reasons: INVALID_SAMPLE/1`）、`:71-74`（墙钟截断仅在 status 不属 `{INVALID_SAMPLE, LEAK_FOUND}` 时改 exit 1，保留 LEAK_FOUND/2）。同文件 `:95` 的「过半 undetermined＝样本无效（…exit 1）」是单一判据说明，优先级在 `:96` 一处补即可，不动；`:97` status 契约本身无优先级断言，不动。
