# 工单 R7：v9.0.1 口径漂移与文档-代码不符 3 条纯文本修复 v2（吸收 codex 复核：D1 压缩为"含宏加"、D3 保留中英文空格、§1.1 净变动 0）
...
## §1 硬约束
1.1 字节：`SKILL.md` = 8021、`commands-staging/*.md` 合计 = 8789 不变；references 三组 glob（`references/*.md references/casebook/*.md references/labels/*.md`）合计 = 929528（基线 929528；三处整行替换按 UTF-8 字面模拟净变动 0 B：D1 +36、D2 −30、D3 −6；实测数写入报告，须等于基线）。含宏报告须传 `--facts facts.json`，未传时宏不展开且不因此 WARN；无宏老报告不强制补 facts.json。
...
```
依据 `scripts/report/build_html.py:267`（`--facts` 可选参数，"不给则旧行为不变"）、`:305-306`（legacy-recompile 只拒 `--a4-seal/--a5-seal`，不拒 `--facts`）、`:419-424`（仅 `if a.facts:` 才渲染宏）、`scripts/report/facts_gate.py:557-561`（`state_path` 可为 None）；`references/report-template.md:214`（报告 md 实体数字一律写宏）。措辞"含宏加"而非无条件加：无宏老报告没有 facts.json，硬加报 FileNotFoundError（复核内存复现）。
...
## §2b 范围外残留（登记不修）
`scripts/solana/fetch_sqd_transfers_v2.py:26`、`:1301` 帮助文字仍写"拿到专属端点后 --url 换掉"；该脚本 `:517-518` 对自身全文计算 `collector_sha256`，改帮助文字即改采集器身份，超出本工程纯文本范围，留待日后升版一并处理。
