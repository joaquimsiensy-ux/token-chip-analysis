...
2. 只读、离线、不 commit、不新建文件。报告打印到 stdout，首行：`# 工单R3复核：通过` 或 `# 工单R3复核：退回`。

## 任务
复核 `maintenance/repair-20260919-drift-audit2/workorder_r3.md`（v1；依据同目录 `blind_r3a_report.md`、`blind_r3b_report.md`），逐条审：
...
c) 字节预算按 UTF-8 逐条重算，≤930260 是否成立；表格行 `\|` 转义是否按字面处理；
