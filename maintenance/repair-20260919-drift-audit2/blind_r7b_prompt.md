# 盲审 R7b 提示词：token-chip-analysis v9.0.1 口径漂移与文档-代码不符审计（只读，代码变更区专审路）

本轮是第七轮。前六轮报告 `maintenance/repair-20260919-drift-audit2/blind_r{1,2,3,4,5,6}{a,b}_report.md`（去重后 9＋3＋3＋4＋3＋2 条）已由工单 `workorder_r1.md`（v2）、`workorder_r2.md`（v2）、`workorder_r3.md`（v2）、`workorder_r4.md` 施工落地于当前 HEAD（完成报告 `r1_done.md`…`r6_done.md`）。**前六轮已报并已修的条目不必重报**；请找前六轮没发现的新问题，以及 R1–R6 修复本身是否引入了新的不一致（若有，明确标"Rn 修复引入"）。若确实找不到，首行写 `# 盲审 R7b：0 条发现` 并给出覆盖声明——不要为了凑数把范围外问题或措辞偏好写成漂移。前六轮已覆盖的重点：evm-channels Alchemy 续采去重句、entity-cluster-methods 措辞锁定句、split-run 调用示例、casebook supply-accounting 去重句、evm-channels bloXroute 段、report-template、independent-audit-protocol、evm-recon/channels/sources、evidence-wording、analyze-workflow、scan-schemas 的 SQD 段、research-workflows 外部路、labels/README、robinhood-channels。本轮请做**全量兜底扫**：对每份必审文档逐一过一遍（不限方向），特别是前六轮盲审仍未报出问题的 `context-discipline.md`、`analysis-playbook.md`、`data-pipeline-evm.md`/`data-pipeline-solana.md` 总册、`data-pipeline-solana-scan.md`、`playbook-supply-recon.md`、`lp-fee-accounting.md`、`monitoring-package.md`、`retrospective.md`、`maintenance-review-repair.md`、`environment.md`、`casebook/*`、`token-analyze*.md`（commands-staging）、SKILL.md 本身。
...
3. 报告全文**打印到 stdout**（只读沙箱写不了文件）。首行固定格式：`# 盲审 R7b：N 条发现`（N 为整数，0 也要写）。
