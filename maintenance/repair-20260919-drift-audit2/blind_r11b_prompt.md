...
3. 报告全文**打印到 stdout**（只读沙箱写不了文件）。首行固定格式：`# 盲审 R11b：N 条发现`（N 为整数，0 也要写）。
