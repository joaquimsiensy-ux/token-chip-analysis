# 盲审 R3b 提示词：token-chip-analysis v9.0.1 口径漂移与文档-代码不符审计（只读，代码变更区专审路）

本轮是第三轮。前两轮报告 `maintenance/repair-20260919-drift-audit2/blind_r1a_report.md`、`blind_r1b_report.md`、`blind_r2a_report.md`、`blind_r2b_report.md`（去重后 9＋3 条）已由工单 `workorder_r1.md`（v2）、`workorder_r2.md` 施工落地于当前 HEAD（完成报告 `r1_done.md`、`r2_done.md`）。**前两轮已报并已修的条目不必重报**；请找前两轮没发现的新问题，以及 R1/R2 修复本身是否引入了新的不一致（若有，明确标"Rn 修复引入"）。若确实找不到，首行写 `# 盲审 R3b：0 条发现` 并给出覆盖声明——不要为了凑数把范围外问题或措辞偏好写成漂移。本轮建议换一批核对方向：R1a/R2a 已重点核 report-template、independent-audit-protocol、evm-recon、evm-channels、evidence-wording、analyze-workflow；本轮请加重 `scan-schemas.md` 各 schema 字段/版本号与生产者常量、`split-run.md`/`commands-staging/*.md` 的分段命令与 `scripts/report/*` 子命令参数、`data-pipeline-solana-*.md` 与 `scripts/solana/*` 的 CLI 参数、`labels/README.md`/`MAINTENANCE.md` 与 `scripts/labels/*`、`environment.md` 与 pyproject、`retrospective.md`/`maintenance-review-repair.md` 流程命令。
...
3. 报告全文**打印到 stdout**（只读沙箱写不了文件）。首行固定格式：`# 盲审 R3b：N 条发现`（N 为整数，0 也要写）。
