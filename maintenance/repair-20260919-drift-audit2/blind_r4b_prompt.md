# 盲审 R4b 提示词：token-chip-analysis v9.0.1 口径漂移与文档-代码不符审计（只读，代码变更区专审路）

本轮是第四轮。前三轮报告 `maintenance/repair-20260919-drift-audit2/blind_r{1,2,3}{a,b}_report.md`（去重后 9＋3＋3 条）已由工单 `workorder_r1.md`（v2）、`workorder_r2.md`（v2）、`workorder_r3.md` 施工落地于当前 HEAD（完成报告 `r1_done.md`、`r2_done.md`、`r3_done.md`）。**前三轮已报并已修的条目不必重报**；请找前三轮没发现的新问题，以及 R1–R3 修复本身是否引入了新的不一致（若有，明确标"Rn 修复引入"）。若确实找不到，首行写 `# 盲审 R4b：0 条发现` 并给出覆盖声明——不要为了凑数把范围外问题或措辞偏好写成漂移。本轮再换一批核对方向：前三轮已重点核 report-template、independent-audit-protocol、evm-recon/channels/sources、evidence-wording、analyze-workflow、scan-schemas 的 SQD 段；本轮请加重 `scan-schemas.md` 其余段（reconcile/wave/flow/wrapper/seal/handoff 各 schema 字段与生产者常量）、`playbook-entity-cluster-*.md`/`playbook-state-anomaly.md`/`economic-control-accounting.md` 的阈值与术语互引、`casebook/*.md` 判例与正式方法册的规则一致性、`monitoring-package.md`/`lp-fee-accounting.md`/`research-workflows.md` 与对应脚本、`labels/README.md`/`MAINTENANCE.md` 与 `scripts/labels/*`、`data-pipeline-robinhood-*.md` 探索档描述。
...
3. 报告全文**打印到 stdout**（只读沙箱写不了文件）。首行固定格式：`# 盲审 R4b：N 条发现`（N 为整数，0 也要写）。
