# 工单 R10：v9.0.1 口径漂移与文档-代码不符 1 条纯文本修复 v2（吸收 codex 复核可选压缩）
...
## §1 硬约束
1.1 字节：`SKILL.md` = 8021、`commands-staging/*.md` 合计 = 8789 不变；references 三组 glob（`references/*.md references/casebook/*.md references/labels/*.md`）合计 = 929110（基线 929131；一处整行替换按 UTF-8 字面模拟净变动 -21 B；实测数写入报告，须等于该值）。
