...
## 任务
复核 `maintenance/repair-20260919-drift-audit2/workorder_r1.md`（**v2**）是否正确吸收第一轮复核 `review_wo1_reply.md` 的意见。调度方的处置：D1/D8/D9 照采；D3/D4/D5/D6/D7 采用复核建议文本；D2 采纳"锚去前导空格"，但**不采纳**"其余状态字段见编译器 schema"的写法（无可指的实锚），改为保留字段列举的压缩版；预算改 ≤930160（Fable 本机模拟 +69 B）。逐条核：
a) 每个锚按代码块内整行 `grep -n -F` 恰 1 处、行号一致（尤其 D2/D4/D6/D7 的代码块锚）；
b) 每条替换文本与现行代码事实一致，无新引入不实断言；D4 新 `:215` 与不动的 `:216` 连读是否通顺且无歧义；
c) 字节预算按 UTF-8 逐条重算，≤930160 与 commands-staging = 8789 是否成立；
